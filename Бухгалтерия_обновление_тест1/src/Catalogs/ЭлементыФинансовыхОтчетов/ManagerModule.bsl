...
Процедура УстановитьТекстЯчейки(Форма, Адрес) Экспорт
	...
	Иначе
		...
		Если Расшифровка.ВидЭлемента = ВидЭлемента("ПоказательБюджетов")
			...
			Если ЕстьНастройки = Истина
				...
				ТекущаяОбласть.Текст = ТекущаяОбласть.Текст + Символы.ПС
					+ НСтр("ru='<установлены настройки>';uk='<встановлені настройки>'");
			КонецЕсли;
		ИначеЕсли Расшифровка.ВидЭлемента = ВидЭлемента("СтатьяБюджетов")
			...
			Если Форма.ИспользоватьДляВводаПлана Тогда
				...
				Если ЗначениеЗаполнено(ФормулаАвтозаполнения) Тогда
					ТекущаяОбласть.Текст = ТекущаяОбласть.Текст + Символы.ПС
						+ НСтр("ru='<настроено автозаполнение>';uk='<настроєне автозаповнення>'");
				КонецЕсли;
			Иначе
		...
		ИначеЕсли Расшифровка.ВидЭлемента = ВидЭлемента("ИтогПоГруппе") Тогда
			...
			ТекущаяОбласть.Текст = НСтр("ru='Итог';uk='Підсумок'");
		...
		Если Объект <> Неопределено Тогда
			...
			Исключение
				...
				Если ТипЗнч(ВидОтчета) = Тип("СправочникСсылка.ВидыБюджетов") Тогда
					ИмяСобытияЖурнала = НСтр("ru='Запись вида бюджета';uk='Запис виду бюджету'",КодЯзыка);
					ТекстОшибки = НСтр("ru='При записи вида бюджета %1 не удалось удалить элемент финансового отчета: %2 по причине: %3';uk='При запису виду бюджету %1 не вдалося вилучити елемент фінансового звіту: %2 з причини: %3'",КодЯзыка);
				Иначе
					ИмяСобытияЖурнала = НСтр("ru='Запись вида финансового отчета';uk='Запис виду фінансового звіту'",КодЯзыка);
					ТекстОшибки = НСтр("ru='При записи вида финансового отчета %1 не удалось удалить элемент финансового отчета: %2 по причине: %3';uk='При запису виду фінансового звіту %1 не вдалося вилучити елемент фінансового звіту: %2 з причини: %3'",КодЯзыка);
				КонецЕсли;
...
|редагується в довідниках-власниках 
...
Функция СхемаОтбораПоказателя(СтруктураИменИзмерений, ТипИтога = Неопределено)
			...
			Если ЭтоОборот Тогда
				...
				Для Сч = 1 По 3 Цикл
					...
					ЗаголовокПоля = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Кор. Субконто%1';uk='Кор. Субконто %1'"),
						Сч);
	...
	Если ЭтоОборот Тогда
		...
		|ИЗ
		|	РегистрБухгалтерии.%1.Обороты(, , , , , %2, %3, ) КАК ДанныеРегистра
		|%4";
		ТекстЗапроса = СтрШаблон(ТекстЗапроса, Источник, ИмяИзмерения, ОтборПоКорСчету, ОтборПостроителя);
	Иначе
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ДанныеРегистра.Субконто1,
		...
		|ИЗ
		|	РегистрБухгалтерии.%1.Остатки(, , %2, %3) КАК ДанныеРегистра
		|%4";
		ТекстЗапроса = СтрШаблон(ТекстЗапроса, Источник, ОтборПоВидуСубконто, ИмяИзмерения, ОтборПостроителя);
	КонецЕсли;
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Замена идентификаторов периодов и параметров в текстовых полях в справочнике ""Элементы финансовых отчетов"".
                                   |Замена пустых значений аналитики в справочнике ""Элементы финансовых отчетов"" на единое значение пустой аналитики Неопределено.
                                   |Обновляет элементы финансовых отчетов для бюджетов.'
                                   |;uk='Заміна ідентифікаторів періодів та параметрів у текстових полях у довіднику ""Елементи фінансових звітів"".
                                   |Заміна порожніх значень аналітики у довіднику ""Елементи фінансових звітів"" на єдине значення порожньої аналітики Неопределено.
                                   |Оновлює елементи фінансових звітів для бюджетів.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				...
				НастройкиКомпоновкиДанных = СправочникОбъект.ДополнительныйОтбор.Получить();
				Если ТипЗнч(НастройкиКомпоновкиДанных) = Тип("НастройкиКомпоновкиДанных") Тогда
					ФинансоваяОтчетностьСервер.УстановитьОтбор(
						...
						"ВалютаХранения", 
						"<заполнить_валютахранения>", 
						ВидСравненияКомпоновкиДанных.Равно
					);
					
					СправочникОбъект.ДополнительныйОтбор = Новый ХранилищеЗначения(НастройкиКомпоновкиДанных);
