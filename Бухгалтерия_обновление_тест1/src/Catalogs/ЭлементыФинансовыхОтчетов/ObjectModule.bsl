...
|елемент довідника-власника ""Види бюджетів""'");
		Иначе
			ТекстИсключения = НСтр("ru='Для удаления элементов финансовой отчетности пометьте на удаление
|элемент справочника-владельца ""Виды отчетов""'
|;uk='Для вилучення елементів фінансової звітності позначте на вилучення 
|елемент довідника-власника ""Види звітів""'");
		КонецЕсли;
        ТекстВладельца = НСтр("ru='с наименованием ';uk= 'з найменуванням '") + Владелец.Наименование;
		ТекстИсключения = ТекстИсключения + " " + ТекстВладельца;
	...
	Если ЭтоНовый() Тогда
		УдалитьНеТребуетсяПриведениеСтрокВПараметры = Истина;
	КонецЕсли;
