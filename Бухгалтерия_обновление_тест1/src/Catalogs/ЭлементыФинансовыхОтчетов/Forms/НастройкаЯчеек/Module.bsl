...
Процедура Вставить(Команда)
	...
	Если Не ОбластьВЗонеРедактирования(ЭтаФорма, ТекущаяОбласть) Тогда
		ТекстПредупреждения = НСтр("ru='Невозможно заменить выбранную область';uk='Неможливо замінити вибрану область'");
	...
	Если Отказ Тогда
		ТекстПредупреждения = НСтр("ru='Невозможно заменить выделенную область';uk='Неможливо замінити виділену область'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
...
Процедура КопироватьЗначения(Вырезать = Ложь)
	...
	Если Не ОбластьВЗонеРедактирования(ЭтаФорма, ТекущаяОбласть) Тогда
		ТекстПредупреждения = НСтр("ru='Невозможно скопировать выбранную область';uk='Неможливо скопіювати вибрану область'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	ИначеЕсли ВидЭлемента = ВидЭлемента("ПроизводныйПоказатель") Тогда
		...
		Элементы.Формула.Заголовок = НСтр("ru='Вычисление';uk='Обчислення'");
	...
	ИначеЕсли ВидЭлемента = ВидЭлемента("Группа") Тогда
		...
		Элементы.Формула.Заголовок = НСтр("ru='Настройка';uk='Настройка'");
	...
	Иначе
		...
		Иначе
			...
			Если ИспользоватьДляВводаПлана И ВидЭлемента = ВидЭлемента("СтатьяБюджетов") Тогда
				...
				Элементы.Формула.Заголовок = НСтр("ru='Автозаполнение';uk='Автозаповнення'");
				Формула = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ЭлементОтчета, "Формула");
...
Процедура ПредставлениеОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЭтоПростаяТаблица Тогда
		ТекстПредупреждения = НСтр("ru='Выбор значений ячеек доступен только в сложной таблице';uk='Вибір значень комірок доступний тільки в складній таблиці'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьЭлементОтчета(Область)
	...
	Если Расшифровка.ВидЭлемента = ПредопределенноеЗначение("Перечисление.ВидыЭлементовФинансовогоОтчета.ФормулаПоГруппе")
		...
		ТекстПредупреждения = НСтр("ru='Настройка свойств элемента доступна в виде бюджета';uk='Настройка властивостей елемента доступна у виді бюджету'");
		ПоказатьПредупреждение( , ТекстПредупреждения);
		Возврат;
	КонецЕсли;
