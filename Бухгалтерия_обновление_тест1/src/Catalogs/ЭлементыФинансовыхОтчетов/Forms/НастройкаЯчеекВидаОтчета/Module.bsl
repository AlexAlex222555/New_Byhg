	...
	ИначеЕсли ВидЭлемента = ВидЭлемента("МонетарныйПоказатель") Тогда
		...
		Если ЕстьНастройки = Истина Тогда
			Формула = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 <%2>';uk=' %1 < %2>'"),
				Формула,
				НСтр("ru='установлены дополнительные отборы показателя';uk='встановлені додаткові відбори показника'"));
		КонецЕсли;
...
Процедура ПоказатьСвойстваОбласти(Область)
	...
	Расшифровка = Область.Расшифровка;
	Если Расшифровка = Неопределено Или ТипЗнч(Расшифровка) = Тип("Строка")
			Или Расшифровка.ВидЭлемента = ВидЭлемента("Измерение") Тогда
		Возврат;
	КонецЕсли;
	Если ОбластьВЗонеРедактирования(ЭтаФорма, Область) Тогда
