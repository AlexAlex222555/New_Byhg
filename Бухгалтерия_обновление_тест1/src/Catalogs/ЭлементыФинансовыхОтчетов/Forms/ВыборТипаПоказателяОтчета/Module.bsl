...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ВидПоказателей", ВидПоказателей) Тогда
		ТекстСообщения = НСтр("ru='Непосредственное открытие этой формы не предусмотрено. Открытие данной формы выполняется при начале выбора вида ячейки в форме ""Настройка ячеек вида отчета"" справочника ""Элементы финансовых отчетов"".';uk='Безпосереднє відкриття цієї форми не передбачено. Відкриття даної форми виконується при початку вибору виду комірки в формі ""Настройка комірок виду звіту"" довідника ""Елементи фінансових звітів"".'");
...
Процедура ОбработкаВыбораЭлементаОтчета()
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ВидЭлемента)
		...
		ТекстПредупреждения = НСтр("ru='Элемент не может быть выбран';uk='Елемент не може бути вибраний'");
