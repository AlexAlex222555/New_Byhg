...
Процедура ЗаполнятьПоДефицитуПриИзменении(Элемент)
	...
	Если ЗаполнятьПоДефициту Тогда
		...
		ПланироватьПолуфабрикатыАвтоматически = Объект.ПланироватьПолуфабрикатыАвтоматически;
		//-- НЕ УТ
	КонецЕсли;
	...
	Если Элементы.ТипПлана.СписокВыбора.Количество() = 0 Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеПроизводства") Тогда
			Элементы.ТипПлана.СписокВыбора.Добавить(Перечисления.ТипыПланов.ПланПроизводства, 
				НСтр("ru='Плана производства';uk='Плану виробництва'"));
		КонецЕсли;
		//-- НЕ УТ
	КонецЕсли;
...
Процедура УстановитьВидимость(Форма)
	...
	Если Форма.ОтражаетсяВБюджетировании И Форма.ПравоПросмотраСтатейБюджетов Тогда 
		...
		ИначеЕсли ЭтоПланОстатков Тогда
			...
			Форма.Элементы.ОтражаетсяВБюджетировании.Заголовок = НСтр("ru='Остатки:';uk='Залишки:'");
		...
		ИначеЕсли ЭтоПланВнутреннихПотреблений Тогда
			...
			Форма.Элементы.ОтражаетсяВБюджетировании.Заголовок = НСтр("ru='План внутреннего потребления:';uk='План внутрішнього споживання:'");
			
		КонецЕсли;
		
		Если ЭтоПланОстатков Тогда
			Форма.Элементы.ГруппаОтражениеВБюджетировании.Заголовок = НСтр("ru='Отражать по показателям бюджета:';uk='Відображати за показниками бюджету:'");
		Иначе
			Форма.Элементы.ГруппаОтражениеВБюджетировании.Заголовок = НСтр("ru='Отражать по статьям бюджетирования:';uk='Відображати по статтях бюджетування:'");
		КонецЕсли;
...
Процедура ИнициализироватьНастройкиРежимаЗаполненияПоДефицитам()
	...
	ИначеЕсли Объект.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		...
		Вариант.Представление = НСтр("ru='продукция и полуфабрикаты';uk='продукція і напівфабрикати'");
		Вариант.Описание = НСтр("ru='Номенклатура, которая имеет хотя бы одну спецификацию.';uk='Номенклатура, яка повинна мати хоча б одну специфікацію.'");
		ВариантыЗаполненияСостава.Добавить(Вариант);
