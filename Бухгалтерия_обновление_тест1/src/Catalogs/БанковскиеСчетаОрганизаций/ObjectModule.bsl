...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(КодБанкаПоСчету)Тогда
		...
		ИначеЕсли КодБанкаПоСчету <> СокрЛП(Банк.Код) Тогда
		
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неверно указан банк. Банк с МФО %1 не найден в классификаторе банков.';uk='Невірно вказано банк. Банк з МФО %1 не знайдений в класифікаторі банків.'"), КодБанкаПоСчету);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, , "КодБанка", Отказ);
		КонецЕсли;
