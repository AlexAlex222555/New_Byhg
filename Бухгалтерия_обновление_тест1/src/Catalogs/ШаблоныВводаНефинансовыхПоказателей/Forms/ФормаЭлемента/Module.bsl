...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НоваяСтрока.Представление = НСтр("ru='Табличная часть';uk='Таблична частина'");
	...
	Колонки.Представление = НСтр("ru='Колонки';uk='Колонки'");
	Колонки.ИндексКартинки = 24;
	
	ЭлементыДобавления = НоваяСтрока.ПолучитьЭлементы();
	Строки = ЭлементыДобавления.Добавить();
	Строки.ИмяИзмерения  = "Строки";
	Строки.Представление = НСтр("ru='Строки';uk='Рядки'");
	...
	ПредставлениеЭлемента = НСтр("ru='Не заполнять';uk='Не заповнювати'");
	...
	ЗначениеПараметра = НСтр("ru='Не заполнять';uk='Не заповнювати'");
	...
	ПредставлениеСхемы = НСтр("ru='Произвольная';uk='Довільна'");
	...
	ЗначениеПараметра = НСтр("ru='Произвольная';uk='Довільна'");
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ЗначениеПараметра);
	ОформляемоеПоле = Элемент.Поля.Элементы.Добавить();
	ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("ТаблицаЗаполненияСхемаКомпоновкиДанных");
	Элемент.Использование = Истина;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.РедактироватьВШапке.Найти("НомерПодПериода") <> Неопределено Тогда
		ТекстОшибки = НСтр("ru='Номер периода должен редактироваться в строках или колонках';uk='Номер періоду має редагуватися в рядках або колонках'");
	...
	Для Каждого СтрокаЗаполнения Из ТаблицаЗаполнения Цикл
		...
		КонецЕсли;
		НайденнаяСтрока.ХранилищеСхемыКомпоновкиДанных = Новый ХранилищеЗначения(ХранилищеСхемыКомпоновкиДанных);
		...
		КонецЕсли;
		НайденнаяСтрока.ХранилищеНастроекКомпоновкиДанных = Новый ХранилищеЗначения(ХранилищеНастроекКомпоновкиДанных);
...
Процедура ТаблицаЗаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.СхемаКомпоновкиДанных = "НеЗаполнять" Тогда
		ТекстПредупреждения = НСтр("ru='Выберите схему компоновки данных для заполнения';uk='Виберіть схему компоновки даних для заповнення'");
...
Процедура ПросмотретьФормуДокумент(Команда)
	...
	Если Объект.ПоказателиШаблона.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Должен быть выбран хотя бы один показатель';uk='Повинен бути вибраний хоч би один показник'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
...
Процедура УправлениеФормой()
	...
	Элементы.ПравоШаг1.Доступность = РазрешеноИзменение;
	Элементы.ПравоШаг3.Доступность = РазрешеноИзменение;
	Элементы.ЛевоШаг1.Доступность = РазрешеноИзменение;
	Элементы.ЛевоШаг3.Доступность = РазрешеноИзменение;
...
Процедура УстановитьГиперссылкуЗначения(СтрокаТаблицы)
	...
	Если Элементов.Количество() Тогда
		ТекстЯчейки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='По-умолчанию (%1)';uk='По умовчанню (%1)'"),
