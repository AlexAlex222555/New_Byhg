...
Процедура ЗаполнитьТекстЗапросаДинамическогоСписка(СтруктураДоступныеКонтексты)
	...
	|	ТаблицаСправочникаПереопределяемый.ИмяПредопределенныхДанных%1
	|ИЗ
	|	Справочник.НематериальныеАктивы КАК ТаблицаСправочникаПереопределяемый%2";
	...
	Иначе
		...
		|	ЕСТЬNULL(ПорядокУчетаНМАБУ.Состояние, ЗНАЧЕНИЕ(Перечисление.СостоянияНМА.НеПринятКУчету)) КАК БУСостояние";
