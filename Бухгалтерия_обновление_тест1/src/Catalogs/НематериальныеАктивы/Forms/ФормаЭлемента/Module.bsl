...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ВалютаУпр <> ВалютаРегл Тогда
		Если ВедетсяРегламентированныйУчетВНА Тогда
			Элементы.ДекорацияЗаголовокУУ.Заголовок = СтрШаблон(НСтр("ru='УУ (%1):';uk='УО (%1):'"), Строка(ВалютаУпр));
			Элементы.ДекорацияЗаголовокБУ.Заголовок = СтрШаблон(НСтр("ru='БУ (%1):';uk='БО (%1):'"), Строка(ВалютаРегл));
		Иначе
			Элементы.ДекорацияЗаголовокУУ.Заголовок = СтрШаблон(НСтр("ru='Упр. (%1):';uk='Упр. (%1):'"), Строка(ВалютаУпр));
			Элементы.ДекорацияЗаголовокБУ.Заголовок = СтрШаблон(НСтр("ru='Регл. (%1):';uk='Регл. (%1):'"), Строка(ВалютаРегл));
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ЗаполнитьСведенияОбУчете()
	...
	Если СведенияОбУчете <> Неопределено Тогда
		...
		ЗаголовокСведений = СтрШаблон(НСтр("ru='Место учета (сведения от %1)';uk='Місце обліку (відомості від %1)'"), Формат(СведенияОбУчете.ПериодСведенийОМестеУчета, "ДЛФ=D"));
	...
	Элементы.ГруппаТекущаяСтоимостьЗаголовок.Заголовок       = НСтр("ru='Первоначальная стоимость:';uk='Первісна вартість:'");
	Элементы.ГруппаНакопленнаяАмортизацияЗаголовок.Заголовок = НСтр("ru='Накопленная амортизация:';uk='Накопичена амортизація:'");
	Элементы.ГруппаОстаточнаяСтоимостьЗаголовок.Заголовок    = НСтр("ru='Остаточная стоимость:';uk='Залишкова вартість:'");
	...
	НематериальныеАктивыЛокализация.ЗаполнитьСведенияОбУчете(ЭтаФорма, СведенияОбУчете, ПредставлениеСведений);
	НематериальныеАктивыЛокализация.ЗаполнитьСтоимостьИАмортизацию(ЭтаФорма, СтоимостьИАмортизация);
