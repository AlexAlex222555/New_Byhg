...
Процедура ПросмотретьВидОтчета(Команда)
	...
	Если ДанныеСтроки.ЭтоГруппа = Истина Тогда
		ТекстПредупреждения = НСтр("ru='Просмотр отчета по группе элемента невозможен';uk='Перегляд звіту по групі елемента неможливий'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура СоздатьЭкземплярБюджета(Команда)
	...
	Если Не Элементы.Список.ТекущиеДанные.ИспользоватьДляВводаПлана Тогда
		ТекстПредупреждения = НСтр("ru='Вид бюджета не используется для ввода плановых данных';uk='Вид бюджету не використовується для введення планових даних'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
