...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ТипИзмерения", ТипИзмерения) Тогда
		ТекстСообщения = НСтр("ru='Непосредственное открытие этой формы не предусмотрено. Открытие формы выполняется при добавлении измерения в вид бюджета.';uk='Безпосереднє відкриття цієї форми не передбачено. Відкриття форми виконується при додаванні виміру в вид бюджету.'");
	...
	Если Параметры.ИспользоватьДляВводаПлана Тогда
		...
		Элементы.ВсеЗначения.РасширеннаяПодсказка.Заголовок = НСтр("ru='В бюджете можно будет заполнить аналитику автоматически всеми ее значениями.';uk='У бюджеті можна буде заповнити аналітику автоматично усіма її значеннями.'");
	Иначе
		Элементы.ВсеЗначения.РасширеннаяПодсказка.Заголовок = НСтр("ru='Будут перечислены все значения, по которым есть обороты или остатки';uk='Будуть перераховані всі значення, за якими є обороти або залишки'");
	КонецЕсли;
...
Процедура ДобавитьСтрокуРезультата(Результат, СтатьяПоказательТипИзмерения, ЗначениеАналитики = Неопределено, ЭтоАналитикаПрочее = Ложь, ВсеЗначения = Ложь)
	...
	НаименованиеДляПечати = ?(ЗначениеЗаполнено(ЗначениеАналитики), ЗначениеАналитики, СтатьяПоказательТипИзмерения);
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.ИспользоватьДляВводаПлана Тогда
		Если Не Форма.ФиксированноеЗначение Тогда
			...
			Форма.Элементы.РазрешитьРедактирование.РасширеннаяПодсказка.Заголовок = НСтр("ru='В экземпляр бюджета можно будет добавлять значения аналитики вручную или заполнить автоматически всеми значениями. В вид бюджета будет добавлен элемент <все значения>';uk='У примірник бюджету можна буде додавати значення аналітики вручну або заповнити автоматично усіма значеннями. У вид бюджету буде додано елемент <всі значення>'");
		Иначе
			Форма.Элементы.РазрешитьРедактирование.РасширеннаяПодсказка.Заголовок = НСтр("ru='В экземпляр бюджета можно будет добавлять значения аналитики вручную или заполнить по данным бюджетирования. В вид бюджета будет добавлен элемент <добавляемые и прочие значения>';uk='У примірник бюджету можна буде додавати значення аналітики вручну або заповнити за даними бюджетування. У вид бюджету буде додано елемент <ті, що додаються, і інші значення>'");
		КонецЕсли;
	КонецЕсли;
