...
Процедура ПроверитьДублиСтрокСтатейБюджетовПоАналитике(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого Периодичность Из СтруктураКолонокТаблицыДанных.Периодичности Цикл
		Измерение = "ВыражениеПериода" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
		СтрокаСвертки = СтрокаСвертки + Измерение + ", ";
	КонецЦикла;
...
Процедура ПроверитьНеПересечениеПериодов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого СтрокаСтруктуры Из СтруктураЯчеек Цикл
		...
		Если МассивВышестоящихПериодовКолонки.Количество()
			...
			ТекстСообщения = НСтр("ru='Период не может быть и в строках и в колонках';uk='Період не може бути і в рядках і в колонках'");
		...
		Если НеверныйПорядокПериодов(МассивВышестоящихПериодовКолонки)
			...
			ТекстСообщения = НСтр("ru='Больший период не может входить в меньший';uk='Більший період не може входити в менший'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстСообщения, "");
			Прервать;
	...
	Для Каждого СтрокаСтруктуры Из СтруктураЯчеек Цикл
		...
		Если ЕстьНеделяВДекадах(МассивВышестоящихПериодовСтроки)
			...
			ТекстСообщения = НСтр("ru='Неделя не может входить в декаду';uk='Тиждень не може входити до декади'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстСообщения, "");
			Возврат;
...
Процедура ПроверитьНаличиеЦиклов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Пока НайденныеСтроки.Количество() Цикл
		...
		Если СталоСтрок = ВсегоСтрок Тогда
			...
			ТекстОшибки = НСтр("ru='Обнаружены циклические ссылки в формулах';uk='Виявлено циклічні посилання в формулах'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстОшибки, "");
...
Процедура ПроверитьНаличиеИсточниковДанных(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Если Не ИсточникиДанных.Количество() Тогда
		ТекстОшибки = НСтр("ru='Не выбраны статьи бюджетов или показатели бюджетов в структуре вида бюджета';uk='Не вибрані статті бюджетів або показники бюджетів у структурі виду бюджету'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстОшибки, "");
	КонецЕсли;
	...
	Если РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Документ Тогда
		...
		Если Не ЕстьИсточникДокумент Тогда
			ТекстОшибки = НСтр("ru='Для ввода данных должны быть выбраны статьи бюджета';uk='Для введення даних повинні бути вибрані статті бюджету'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстОшибки, "");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого КлючиИЗначение Из Структура Цикл
		Для Каждого СтрокаОшибки Из КлючиИЗначение.Значение Цикл
			Если КлючиИЗначение.Ключ = "Статьи" Тогда
				ТекстОшибки = НСтр("ru='Для статьи ""%1"" указана недоступная на статье аналитика ""%2""';uk='Для статті ""%1"" вказана недоступна на статті аналітика ""%2""'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ТекстОшибки,
					СтрокаОшибки.СтатьяПоказатель,
					СтрокаОшибки.ВидАналитики);
			Иначе
				ТекстОшибки = НСтр("ru='Для показателя ""%1"" указана недоступная на показателе аналитика ""%2""';uk='Для показника ""%1"" вказана недоступна на показнику аналітика ""%2""'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ТекстОшибки,
					СтрокаОшибки.СтатьяПоказатель,
					СтрокаОшибки.ВидАналитики);
...
Процедура ПроверитьСоответствиеВалютыСтатьям(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого КлючиИЗначение Из Структура Цикл
		Для Каждого СтрокаОшибки Из КлючиИЗначение.Значение Цикл
			Если КлючиИЗначение.Ключ = "Статьи" Тогда
				ТекстОшибки = НСтр("ru='Для статьи ""%1"" недоступен учет по валюте';uk='Для статті ""%1"" недоступний облік по валюті'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					СтрокаОшибки.СтатьяПоказатель);
			Иначе
				ТекстОшибки = НСтр("ru='Для показателя ""%1"" недоступен учет по валюте';uk='Для показника ""%1"" недоступний облік по валюті'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					СтрокаОшибки.СтатьяПоказатель);
			КонецЕсли;
...
Процедура ПроверитьНаличиеСтрокКолонок(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Если Не НайденнаяСтрока.Строки.Количество() Тогда
			...
			ТекстОшибки = НСтр("ru='Строки таблицы не могут быть пустыми';uk='Рядки таблиці не можуть бути порожніми'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстОшибки, "");
			Прервать;
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Если Не НайденнаяСтрока.Строки.Количество() Тогда
			...
			ТекстОшибки = НСтр("ru='Колонки таблицы не могут быть пустыми';uk='Колонки таблиці не можуть бути порожніми'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстОшибки, "");
			Прервать;
...
Процедура ПроверитьПериодичностьЭлементовВидаБюджета(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого Периодичность Из СтруктураКолонок.Периодичности Цикл
		...
		Если ИндексОтчета > ИндексТекущей Тогда
			ТекстОшибки = НСтр("ru='Периодичность, выбираемая в группировках, не может быть больше, чем периодичность самого вида бюджета (%1)';uk='Періодичність, що вибирається в групуваннях, не може бути більше, ніж періодичність самого виду бюджету (%1)'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, 
...
|;uk='У рядках виявлені різнорідні аналітики, або для різних рядків налаштований різний склад статей і показників. 
|Рекомендується налаштовувати однакові аналітики з однаковим складом статей і показників по рядках. Редагувати такі бюджети буде зручніше.'");
...
Процедура НастроитьГруппуЭлементовПериода(ВидБюджета, Элементы, ГраницаФактДанных) Экспорт
	...
	Иначе
		...
		ПолеФормыНадписьВидБюджета = Элементы.НадписьВидБюджета; // ПолеФормы -
		ПолеФормыНадписьВидБюджета.Заголовок = НСтр("ru='Фиксированный бюджет';uk='Фіксований бюджет'");
	...
	Если ВидБюджетаИспользуетГраницуФактическихДанных(ВидБюджета) Тогда
		...
		ПолеФормыГраницаФактДанных.Заголовок = НСтр("ru='Граница факт.данных: %1';uk='Межа факт.даних: %1'");
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ВидыБюджетов.ЗарегистрироватьДанныеДляОчисткиКэшВспомогательныхДанных";
	...
	Читаемые.Добавить(Метаданные.РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.ПолноеИмя());
	Читаемые.Добавить(Метаданные.РегистрыСведений.КэшИменДокументовДляОбработкиПоПравилу.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.КэшИменДокументовДляОбработкиПоПравилу.ПолноеИмя());
	...
	Блокируемые.Добавить(Метаданные.Отчеты.ОборотнаяВедомостьБюджетирования.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.Отчеты.ОборотноСальдоваяВедомостьБюджетирования.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.Отчеты.ПроверкаСвязейПоказателейБюджетов.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.Отчеты.РасшифровкаФормулыБюджетногоОтчета.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.Отчеты.РезультатРаботыПравилПолученияФактическихДанных.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.ПолноеИмя());
	Блокируемые.Добавить(Метаданные.РегистрыСведений.КэшИменДокументовДляОбработкиПоПравилу.ПолноеИмя());
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("33581784-8944-458c-815b-391268856d94");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "Документ.ЭкземплярБюджета,"
		+ "Справочник.ВидыБюджетов,"
		+ "Справочник.ЭлементыФинансовыхОтчетов";
	Обработчик.ИзменяемыеОбъекты = "Справочник.ВидыБюджетов";
	Обработчик.БлокируемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru='Заполняет новые реквизиты вида бюджета значениями по умолчанию. Перезаписывает виды бюджета с признаком СохранятьБюджетВТабличнойФормеВЭкземпляреБюджета, по которым есть экземпляры бюжета, для сброса актуальности кэшированных данных.';uk='Заповнює нові реквізити виду бюджету значеннями по умовчанню. Перезаписує види бюджету з ознакою СохранятьБюджетВТабличнойФормеВЭкземпляреБюджета, за якими існують екземпляри бюджету, для скидання актуальності кешованих даних.'");
...
Процедура ОчиститьКэшВспомогательныхДанных(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось очистить кэш вспомогательных данных по причине: %Причина%';uk='Не вдалося очистити кеш допоміжних даних з причини: %Причина%'");
...
// Обработчик обновления КА.
// Заполняет новые реквизиты вида бюджета значениями по умолчанию.  
// Перезаписывает виды бюджета с признаком СохранятьБюджетВТабличнойФормеВЭкземпляреБюджета, 
// по которым есть экземпляры бюжета, для сброса актуальности кэшированных данных.
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	(ВидыБюджетов.ОсновнойСпособВвода = ЗНАЧЕНИЕ(Перечисление.СпособыРедактированияБюджета.ПустаяСсылка)
	|			ИЛИ ВидыБюджетов.РежимПолученияДанных = ЗНАЧЕНИЕ(Перечисление.РежимПолученияДанныхБюджетирования.ПустаяСсылка))
	|	И НЕ ВидыБюджетов.ЭтоГруппа";
	...
	|ИЗ
	|	Справочник.ВидыБюджетов КАК ВидыБюджетов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыБюджетов.КэшСтатейИПоказателей КАК ВидыБюджетовКэшСтатейИПоказателей
	|		ПО ВидыБюджетов.Ссылка = ВидыБюджетовКэшСтатейИПоказателей.Ссылка
	|ГДЕ
	|	ВидыБюджетовКэшСтатейИПоказателей.Ссылка ЕСТЬ NULL 
	|	И НЕ ВидыБюджетов.ЭтоГруппа";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Справочник.ВидыБюджетов";
	...
	Пока Выборка.Следующий() Цикл
		Попытка	
			...
			Если Не ЗначениеЗаполнено(СпрОбъект.ОсновнойСпособВвода) Тогда
				СпрОбъект.ОсновнойСпособВвода = Перечисления.СпособыРедактированияБюджета.ТабличныйВвод;
				СпрОбъект.ВключитьЗащитуТабличногоДокумента = Истина;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СпрОбъект.РежимПолученияДанных) Тогда
				СпрОбъект.РежимПолученияДанных = Перечисления.РежимПолученияДанныхБюджетирования.Разрешенные;
			КонецЕсли;
			
			Если СпрОбъект.КэшСтатейИПоказателей.Количество() = 0 Тогда
				СпрОбъект.КэшСтатейИПоказателей.Загрузить(ИспользуемыеСтатьиПоказателиВидаБюджета(СпрОбъект.Ссылка));
			КонецЕсли;
		...
		Исключение
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru='Не удалось обработать: %ВидБюджета% по причине: %Причина%';uk='Не вдалося обробити: %ВидБюджета% по причині: %Причина%'");
...
// Возвращает используемые статьи и показатели в виде бюджета
//
// Параметры:
// 	ВидБюджета - СправочникСсылка.ВидыБюджетов - вид бюджета, для которого определяются статьи и показатели
//
Функция ИспользуемыеСтатьиПоказателиВидаБюджета(ВидБюджета) 
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА ЭлементыФинансовыхОтчетов.ВидЭлемента = ЗНАЧЕНИЕ(Перечисление.ВидыэлементовФинансовогоОтчета.СтатьяБюджетов)
	|			ТОГДА РеквизитыСтатья.Значение
	|		КОГДА ЭлементыФинансовыхОтчетов.ВидЭлемента = ЗНАЧЕНИЕ(Перечисление.ВидыэлементовФинансовогоОтчета.ПоказательБюджетов)
	|			ТОГДА РеквизитыПоказатель.Значение
	|		КОГДА ЭлементыФинансовыхОтчетов.ВидЭлемента = ЗНАЧЕНИЕ(Перечисление.ВидыэлементовФинансовогоОтчета.НефинансовыйПоказатель)
	|			ТОГДА РеквизитыНФП.Значение
	|	КОНЕЦ КАК СтатьяПоказатель,
	|	ЕСТЬNULL(РеквизитПривилегированныйРасчет.Значение, ЛОЖЬ) КАК ПривилегированныйРасчет
	|ИЗ
	|	Справочник.ЭлементыФинансовыхОтчетов КАК ЭлементыФинансовыхОтчетов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЭлементыФинансовыхОтчетов.РеквизитыВидаЭлемента КАК РеквизитыСтатья
	|		ПО ЭлементыФинансовыхОтчетов.Ссылка = РеквизитыСтатья.Ссылка
	...
	|		ПО ЭлементыФинансовыхОтчетов.Ссылка = РеквизитыПоказатель.Ссылка
	...
	|		ПО ЭлементыФинансовыхОтчетов.Ссылка = РеквизитыНФП.Ссылка
	|			И (РеквизитыНФП.Реквизит = &Реквизит_НФП)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЭлементыФинансовыхОтчетов.РеквизитыВидаЭлемента КАК РеквизитПривилегированныйРасчет
	|		ПО ЭлементыФинансовыхОтчетов.Ссылка = РеквизитПривилегированныйРасчет.Ссылка
	|			И (РеквизитПривилегированныйРасчет.Реквизит = &Реквизит_ПривилегированныйРасчет)
	|ГДЕ
	|	ЭлементыФинансовыхОтчетов.Владелец = &ВидБюджета
	|	И ЭлементыФинансовыхОтчетов.ВидЭлемента В (ЗНАЧЕНИЕ(Перечисление.ВидыэлементовФинансовогоОтчета.СтатьяБюджетов), ЗНАЧЕНИЕ(Перечисление.ВидыэлементовФинансовогоОтчета.ПоказательБюджетов), ЗНАЧЕНИЕ(Перечисление.ВидыэлементовФинансовогоОтчета.НефинансовыйПоказатель))
	|");
	...
	Запрос.УстановитьПараметр("Реквизит_ПривилегированныйРасчет", ПланыВидовХарактеристик.РеквизитыЭлементовФинансовыхОтчетов.ПривилегированныйРежим);
