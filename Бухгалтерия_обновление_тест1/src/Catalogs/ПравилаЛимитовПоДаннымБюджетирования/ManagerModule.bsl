...
Функция ИерархияАналитикПоПравиламСУказаниемКолонокИсточника(Правила) Экспорт
	...
	Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
		...
		|	Правила.Ссылка КАК Правило,
		|	%1 КАК НомерИзмеренияВидаАналитикиПоУмолчанию,
		|	Правила.СтатьяБюджета.ВидАналитики%2 КАК ИзмерениеВидАналитики,
		|	""Аналитика%2"" КАК ПутьКАналитике
		|ИЗ
		|	Справочник.ПравилаЛимитовПоДаннымБюджетирования КАК Правила
		|ГДЕ
		|	Правила.Ссылка В (&Правила)";
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='В справочнике ""Правила лимитов по данным бюджетирования"" перезаполняет табличную часть ""Иерархия аналитик""';uk='У довіднику ""Правила лімітів за даними бюджетування"" перезаповнює табличну частину ""Ієрархія аналітик""'");
	...
	Блокируемые.Добавить(Метаданные.Справочники.ПравилаЛимитовПоДаннымБюджетирования.ПолноеИмя());
	Обработчик.БлокируемыеОбъекты = СтрСоединить(Блокируемые, ",");
	
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.МоделиБюджетирования.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				...
				Если СправочникОбъект.УдалитьИерархияАналитикПерезаписана
				   И НЕ СправочникОбъект.ИерархияАналитикПерезаписана Тогда
					Для Каждого СтрокаТабличнойЧасти Из СправочникОбъект.ИерархияАналитик Цикл
						СтрокаТабличнойЧасти.Используется = СтрокаТабличнойЧасти.УдалитьИспользуется;
					КонецЦикла;
				...
				ИначеЕсли НЕ СправочникОбъект.ИерархияАналитикПерезаписана Тогда
					...
					ИерархияАналитик.Колонки.Добавить("НомерСтроки", ОписаниеЧисла);
					СчетчикСтрок = 0;
					Для Каждого СтрокаТЧ Из ИерархияАналитик Цикл
						СчетчикСтрок = СчетчикСтрок + 1;
						СтрокаТЧ.НомерСтроки = СчетчикСтрок;
					КонецЦикла;
					СправочникОбъект.ИерархияАналитик.Загрузить(ИерархияАналитик);
					СправочникОбъект.ИерархияАналитикПерезаписана = Истина;
					СправочникОбъект.ПредставлениеАналитик = ПредставлениеАналитик(ИерархияАналитик);
					ОбъектИзменен = Истина;
				КонецЕсли;
