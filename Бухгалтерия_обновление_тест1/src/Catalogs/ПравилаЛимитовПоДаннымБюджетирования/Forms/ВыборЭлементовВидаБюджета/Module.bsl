...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Владелец", МодельБюджетирования) Тогда
		ТекстСообщения = НСтр("ru='Непосредственное открытие этой формы не предусмотрено. Для открытия формы можно воспользоваться командой ""Добавить по виду бюджета"" в форме списка правил лимитов по данным бюджетирования.';uk='Безпосереднє відкриття цієї форми не передбачено. Для відкриття форми можна скористатися командою ""Додати по виду бюджету"" в формі списку правил лімітів за даними бюджетування.'");
...
Процедура Далее(Команда)
	...
	Если Элементы.ГруппаСтраницы.ТекущаяСтраница.Имя = "ВыборВидаБюджета" Тогда
		...
		Если ТекущиеДанные = Неопределено
			...
			ТекстПредупреждения = НСтр("ru='Не выбран вид бюджета';uk='Не вибрано вид бюджету'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
			Возврат;
		КонецЕсли;
		Если ТекущиеДанные.ЭтоГруппа Тогда
			ТекстПредупреждения = НСтр("ru='Создать лимиты по группе видов бюджета нельзя. Выберите вид бюджета.';uk='Створити ліміти по групі видів бюджету не можна. Виберіть вид бюджету.'");
			ПоказатьПредупреждение(,ТекстПредупреждения);
			Возврат;
		КонецЕсли;
...
Функция ВыполненоСозданиеПравилНаСервере()
	...
	Если НайденныеСтроки.Количество() Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			ТекстОшибки = НСтр("ru='Не удалось создать правила лимитов для вида бюджета: %1 модели бюджетирования: %2, по причине: %3';uk='Не вдалося створити правила лімітів для виду бюджету: %1 моделі бюджетування: %2, з причини: %3'",КодОсновногоЯзыка);
			ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					Строка(ВидБюджета),
					Строка(МодельБюджетирования),
					ПредставлениеОшибки);
			СобытиеЖР = НСтр("ru='ПравилаЛимитовПоДаннымБюджетирования.СозданиеПравил';uk='ПравілаЛімітовПоДаннимБюджетірованія.СозданіеПравіл'",КодОсновногоЯзыка);
		...
		КонецПопытки;
		Если ПравилаСозданы Тогда
			ЗаголовокДекорации = НСтр("ru='Правила созданы успешно';uk='Правила створені успішно'")
					+ Символы.ПС + НСтр("ru='Создано правил лимитов: %1';uk='Створено правил лімітів: %1'");
			Элементы.ДекорацияСоздано.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокДекорации,
				Создано);
		Иначе
			ЗаголовокДекорации = НСтр("ru='Не удалось создать правила по причине:';uk='Не вдалося створити правила з причини:'")
					+ Символы.ПС + "%1";
