...
Функция КлючевыеРеквизиты() Экспорт
	...
	Результат.Вставить("ОсновноеИзделиеНоменклатура");
	Результат.Вставить("ОсновноеИзделиеХарактеристика");
...
Функция ПолучитьПартиюПроизводства(ПоляПартии = Неопределено,
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось создать партию производства с параметрами: %ПартияПроизводства% по причине: %Причина%';uk='Не вдалося створити партію виробництва з параметрами: %ПартияПроизводства% з причини: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПартияПроизводства%", НаименованиеПартии);
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Справочник.ПартииПроизводства.ПолучитьПартиюПроизводства';uk='Справочнік.ПартііПроізводства.ПолучітьПартіюПроізводства'",КодОсновногоЯзыка),
...
Функция НаименованиеПартииПроизводства(СтруктураПартии, ПредставлениеГруппыЗатрат = "") Экспорт
	...
	ПредставлениеПартии = НСтр("ru='%ПредставлениеДокумента% (%Номер%.%НомерГруппы%, %Дата%);';uk='%ПредставлениеДокумента% (%Номер%.%НомерГруппы%, %Дата%);'");
	Если ЗначениеЗаполнено(СтруктураПартии.ГруппировкаЗатрат) Тогда
		ПредставлениеПартии = ПредставлениеПартии + " "
			+ НСтр("ru='Группа затрат: %ГруппаЗатрат%';uk='Група витрат: %ГруппаЗатрат%'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчета = ВариантыОтчетовУТПереопределяемый.ДобавитьКомандуСтруктураПодчиненности(КомандыОтчетов);
...
Функция ТекстЗапросаКорректныеПоляПартийПроизводства()
    ...
    |	ВЫБОР
    |		КОГДА НЕ ТИПЗНАЧЕНИЯ(Т.НалоговоеНазначение) = ТИП(Справочник.НалоговыеНазначенияАктивовИЗатрат)
