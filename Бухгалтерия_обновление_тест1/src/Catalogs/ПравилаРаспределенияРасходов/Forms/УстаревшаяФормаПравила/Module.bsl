...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.НазначениеПравила = Перечисления.НазначениеПравилРаспределенияРасходов.РаспределениеСтатейРасходовПоПодразделениям
		...
		ТекстСообщения = НСтр("ru='Не указан тип показателя.';uk='Не вказаний тип показника.'");
...
Функция ПроверитьКорректностьУказанныхМатериалов()
	...
	Если БазаРаспределения = "КоличествоУказанныхМатериалов" Тогда
		...
		Если Выборка.Количество() > 1 Тогда
			...
			ТекстПредупреждения = НСтр("ru='Возможно материалы указаны некорректно. Материалы хранятся в разных измеряемых величинах:';uk='Можливо матеріали вказані некоректно. Матеріали зберігаються в різних вимірюваних величинах:'") + " ";
	...
	Иначе
		...
		ШаблонПредупреждения = НСтр("ru='Невозможно получить коэффициент пересчета для материалов (1%) в 2%.';uk='Неможливо отримати коефіцієнт перерахунку для матеріалів (1%) в 2%.'");
	...
	Если Объект.НазначениеПравила = Перечисления.НазначениеПравилРаспределенияРасходов.РаспределениеСтатейРасходовПоПодразделениям Тогда
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗаголовка,
				Объект.Наименование,
				НСтр("ru='Показатель';uk='Показник'"));
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗаголовка,
				НСтр("ru='Показатель';uk='Показник'"),
