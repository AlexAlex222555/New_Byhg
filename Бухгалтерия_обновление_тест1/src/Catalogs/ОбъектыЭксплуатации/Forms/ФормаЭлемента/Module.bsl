...
Процедура ЗаполнитьСведенияОбУчете()
	...
	ЗаголовокСведений = СтрШаблон(НСтр("ru='Местонахождение (сведения от %1)';uk='Місцезнаходження (відомості від %1)'"), Формат(СведенияОбУчете.ПериодСведенийОМестонахождении, "ДЛФ=D"));
	...
	Элементы.ГруппаТекущаяСтоимостьЗаголовок.Заголовок       = НСтр("ru='Восстановительная стоимость:';uk='Відновлювальна вартість:'");
...
Процедура ПрочитатьПериодическиеРеквизиты()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетВнеоборотныхАктивовПоНаправлениямДеятельности") Тогда
		...
		|ИЗ
		|	РегистрСведений.ПорядокУчетаОС.СрезПоследних(, ОсновноеСредство = &ОсновноеСредство) КАК ПорядокУчетаОССрезПоследних";
