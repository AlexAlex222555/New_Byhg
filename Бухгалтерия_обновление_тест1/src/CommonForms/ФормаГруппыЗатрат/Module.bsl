...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокПолей = ГруппыЗатратСервер.ПереченьПолейПоГруппировкеЗатрат(Параметры.ГруппировкаЗатрат);
	ТекстНеИспользуется = НСтр("ru='<не используется>';uk='<не використовується>'");
