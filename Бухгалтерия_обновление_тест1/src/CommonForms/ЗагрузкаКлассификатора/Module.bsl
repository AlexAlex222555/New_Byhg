	...
	Для Индекс = НачальныйНомер По КонечныйНомер - 1 Цикл
		...
		Если ЗначениеЗаполнено(СтрокаЭлемента.КодРодителя) Тогда
			Если НЕ СтрокаЭлемента.РодительЭтоГруппа Тогда
				ШаблонСообщения = НСтр("ru='Невозможно создать / обновить элемент с кодом %1 т.к. в качестве родителя у него указан элемент (%2)';uk='Неможливо створити / оновити елемент з кодом %1 тому що в якості батьківського у нього вказаний елемент (%2)'");
