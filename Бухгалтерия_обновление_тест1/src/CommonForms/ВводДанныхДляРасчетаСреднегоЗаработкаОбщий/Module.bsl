...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Если ДатаНачалаСобытия >= Дата(2020,12,12) Тогда
			ЗаполнитьФорму1213(ТаблицаНачислений, ТаблицаВремени, ТаблицаИндексации);	
		Иначе	
...
&НаКлиенте
Процедура СреднийЗаработокПередНачаломИзменения(Элемент, Отказ)
	 
	ДанныеСтроки = Элементы.СреднийЗаработок.ТекущиеДанные;
	Если ДанныеСтроки.Идентификатор = "ПремияСуммой" Или ДанныеСтроки.Идентификатор = "ПремияПроцентом" Тогда
		Отказ = Истина
	КонецЕсли;	
...
&НаКлиентеНаСервереБезКонтекста
Функция РезультатРедактированияНачислений1213(Форма)
...
Процедура ЗаполнитьФорму(ДанныеОНачислениях, ДанныеОВремени, ИндексацияСреднегоЗаработка = Неопределено)
	...
	Для Каждого Месяц Из Месяцы Цикл
		...
		КонецЕсли;
		Для Каждого СтрокаТаблицы Из СреднийЗаработок Цикл
			Если СтрокаТаблицы.Свойство("Значение" + ПостфиксКолонки(Месяц)) Тогда
...
&НаСервере
Процедура ЗаполнитьФорму1213(ДанныеОНачислениях, ДанныеОВремени, ИндексацияСреднегоЗаработка = Неопределено)
	...
	Для Каждого Месяц Из Месяцы Цикл
		...
		КонецЦикла;
	КонецЦикла;
	//Для "старых" документов надо убрать годовую премию, заменив на обычную премию:
	Для Каждого СтрокаНачислений Из ДанныеОНачислениях Цикл
		Если СтрокаНачислений.СоставнаяЧасть = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОбщий.ПремияГодоваяПроцентом")
			Тогда СтрокаНачислений.СоставнаяЧасть = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОбщий.ПремияПроцентом");
				 СтрокаНачислений.Год = 0;
				 СтрокаНачислений = ?(СтрокаНачислений.КоличествоМесяцев > 0, СтрокаНачислений.КоличествоМесяцев, 12)
		ИначеЕсли СтрокаНачислений.СоставнаяЧасть = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОбщий.ПремияГодоваяФиксированнойСуммой")
			Тогда СтрокаНачислений.СоставнаяЧасть = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОбщий.ПремияФиксированнойСуммой");
				 СтрокаНачислений.Год = 0;
				 СтрокаНачислений = ?(СтрокаНачислений.КоличествоМесяцев > 0, СтрокаНачислений.КоличествоМесяцев, 12)
	    КонецЕсли;
	КонецЦикла;	
	ЗаполнитьНачисления1213(ДанныеОНачислениях);
...
&НаСервере
Процедура ЗаполнитьНачисления1213(ДанныеОНачислениях) 
...
Процедура РассчитатьСреднийЗаработок(Форма)
	...
	Если Форма.ДатаНачалаСобытия >= Дата(2020,12,12) Тогда
		ДанныеНачислений = РезультатРедактированияНачислений1213(Форма);
	Иначе	
...
Процедура ПрочитатьДанныеУчетаСреднегоЗаработка(ОтборМесяцев = Неопределено, УчитыватьКорректировки = Истина)
	...
	Если ДатаНачалаСобытия >= Дата(2020,12,12) Тогда
		ЗаполнитьФорму1213(ДанныеДляРасчета.ДанныеОНачислениях, ДанныеДляРасчета.ДанныеОВремени, ДанныеДляРасчета.ДанныеОбИндексации);
	Иначе
...
Процедура ДополнитьПредположительно()
	...
	Если ДатаНачалаСобытия >= Дата(2020,12,12) Тогда
		МассивМесяцев.Добавить(ОкончаниеПериодаРасчета);
	Иначе	
		МесяцОбхода = НачалоПериодаРасчета;
	...
	Для Каждого Месяц Из МассивМесяцев Цикл
		...
		Если ДатаНачалаСобытия >= Дата(2020,12,12) И ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление2010") Тогда
			НоваяСтрока.ДатаНачалаПериода = ДобавитьМесяц(НачалоПериодаРасчета,-2);
 			НоваяСтрока.ДатаОкончанияПериода = КонецМесяца(ДобавитьМесяц(НачалоПериодаРасчета,-1));
		Иначе	
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДатаНачалаСобытия >= Дата(2020,12,12) Тогда
			Если ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление100Отпускные")Тогда
				Сумма = Окр(СтрокаНачислений.Сумма*12,2) 
			Иначе
				Сумма = Окр(СтрокаНачислений.Сумма*2,2) 
			КонецЕсли;
			ДанныеНачислений.ЗаполнитьЗначения(Сумма,"Сумма");
		КонецЕсли;
	...
	Запрос.Текст =
		...
		|	ДанныеУчетаВремени.НормаЧасов КАК НормаЧасовПроизводственныйКалендарь,
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			Если ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление100Отпускные") Тогда
				ДанныеВремени.ЗаполнитьЗначения(КалендарныеДниБезУчетаПраздников,"ОтработаноДнейКалендарных");
			Иначе
				Если ВидУчетаВремениДляСредней = ПредопределенноеЗначение("Перечисление.ВидыУчетаВремениДляСредней.ПоРабочимЧасам") Тогда
					НормаЧасовПроизводственныйКалендарь = Выборка.НормаЧасовПроизводственныйКалендарь;
					НормаПоГрафикуЗаПредыдущиеМесяцы = НормаЧасовПроизводственныйКалендарь;
				Иначе	
					НормаДнейПроизводственныйКалендарь = Выборка.НормаДнейПроизводственныйКалендарь;
					НормаПоГрафикуЗаПредыдущиеМесяцы = НормаДнейПроизводственныйКалендарь;
				КонецЕсли;	
	...
	Если ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Постановление2010") Тогда
		Если ДатаНачалаСобытия >= Дата(2020,12,12) Тогда
			Если ЗначениеЗаполнено(Организация.ГрафикРаботыСотрудников)  Тогда 
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	ВТГрафикРаботыСотрудников.ГрафикРаботы КАК ГрафикРаботы
				|ИЗ
				|	ВТГрафикиРаботыСотрудников КАК ВТГрафикРаботыСотрудников
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				...
				Если Выборка.Следующий() Тогда
					Если Выборка.ГрафикРаботы <> Организация.ГрафикРаботыСотрудников Тогда
						...
						СтрокаГрафиков.ГрафикРаботы = Организация.ГрафикРаботыСотрудников;
						...
						|	ВТВремяПоГрафикам.НормаЧасов КАК НормаЧасов
						|ИЗ ВТВремяПоГрафикам КАК ВТВремяПоГрафикам";
						
						Выборка = Запрос.Выполнить().Выбрать();
						Если  Выборка.Следующий() Тогда
							Если ВидУчетаВремениДляСредней = ПредопределенноеЗначение("Перечисление.ВидыУчетаВремениДляСредней.ПоРабочимЧасам") Тогда
								НормаПоГрафикуЗаПредыдущиеМесяцы = Выборка.НормаЧасов;
							Иначе	
								НормаПоГрафикуЗаПредыдущиеМесяцы = Выборка.НормаДней;
							КонецЕсли;
							ДанныеВремени.ЗаполнитьЗначения(НормаПоГрафикуЗаПредыдущиеМесяцы,"СреднемесячноеКоличествоДней");
						КонецЕсли;	
					КонецЕсли	
...
Процедура ЗаполнитьОписаниеСтрок()
	...
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Общий заработок (неиндексируемый)';uk='Загальний заробіток (неіндексований)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ОбщийЗаработок;
	...
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Премия, полностью учитываемая (неиндексируемая)';uk='Премія, повністю обліковується (неіндексується)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияПроцентом;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='	в т.ч. неиндексируемая';uk=' в т. ч. неіндексована'");
	НоваяСтрока.ЗаголовокКоманды = НСтр("ru='Премия, учитываемая частично (неиндексируемая)';uk='Премія, яка обліковується частково (неіндексується)'");
	НоваяСтрока.СоставнаяЧасть = Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияФиксированнойСуммой;
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Норма дней (по произв. календарю)';uk='Норма днів (за вир. календарем)'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Норма часов (по произв. календарю)';uk='Норма годин (за вир. календарем)'");
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Среднемес. к-во дней';uk='Средньоміс. к-ть днів'");
	НоваяСтрока.ДобавлятьПоУмолчанию = Истина;
	НоваяСтрока.Точность = 2;
