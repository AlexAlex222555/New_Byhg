...
|;uk='Ідентифікатор не відповідає правилам іменування перемінних.
| Ідентифікатор не повинен містити пробілів і спеціальних символів і не повинен починатися з цифри.'");
		...
		Если ИдентификаторПеременная Тогда
			...
			Если ЗарезервированныеПеременные.Свойство(Идентификатор) Тогда
				ТекстСообщения = НСтр("ru='Идентификатор аналитики не может совпадать со значением параметра ""%1"".';uk='Ідентифікатор аналітики не може збігатися зі значенням параметра ""%1"".'");
				ОбщегоНазначения.СообщитьПользователю(
			...
			Иначе
				...
				Если НЕ Результат.Пустой() Тогда
					ТекстСообщения = НСтр("ru='В базе данных уже содержится вид аналитики с идентификатором ""%1"". Идентификатор должен быть уникальным';uk='У базі даних вже міститься вид аналітики з ідентифікатором ""%1"". Ідентифікатор повинен бути унікальним'");
					ОбщегоНазначения.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							ТекстСообщения,
							Идентификатор),
