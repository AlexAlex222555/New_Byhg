...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для ИндексСтроки = 0 По СписокКорректировок.Количество() - 1 Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Спецификация) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Не заполнено поле ""Спецификация"" в строке %1.';uk='Не заповнене поле ""Специфікація"" в рядку %1.'"), НомерСтроки);
		...
		Если Не ЗначениеЗаполнено(ДанныеСтроки.ДатаЗапускаВыпуска) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Не заполнена поле ""Дата запуска"" в строке %1.';uk='Не заповнено поле ""Дата запуску"" в рядку %1.'"), НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, ПутьКДанным,, Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Период) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Не заполнена поле ""Дата выпуска"" в строке %1.';uk='Не заповнено поле ""Дата випуску"" в рядку %1.'"), НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, ПутьКДанным,, Отказ);
		КонецЕсли;
		
		Если ДанныеСтроки.ДатаЗапускаВыпуска > ДанныеСтроки.Период
			И Не Отказ Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Некорректно поле ""Дата выпуска"" в строке %1.';uk='Некоректна поле ""Дата випуску"" в рядку %1.'"), НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, ПутьКДанным,, Отказ);
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗакрытьБезВопроса И Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Внесенные изменения будут потеряны. Продолжить?';uk='Внесені зміни будуть втрачені. Продовжити?'");
...
Процедура ПериодПриИзмененииНаСервере(ИдентификаторСтроки, КэшированныеЗначения)
	...
	ПараметрыЗаполнения = УправлениеДаннымиОбИзделияхКлиентСервер.ПараметрыДействияПроверитьЗаполнитьСпецификацию(
		Неопределено, ТекущиеДанные, ПараметрыВыбораСпецификаций);
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма)
	...
	//
	//
	//Если СписокКорректировок[0].ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар Тогда
	//	Элементы.Назначение.ПодсказкаВвода = НСтр("ru='<без назначения>'");
	//Иначе
	//	Элементы.Назначение.ПодсказкаВвода = НСтр("ru='<для товаров>'");
	//КонецЕсли;
	//
	//Если СписокКорректировок[0].ХарактеристикиИспользуются Тогда
...
Процедура УстановитьПараметрыВыбораСпецификаций()
	...
	ПараметрыВыбораСпецификаций.ПараметрыСмещенияПериода = Документы.ПланПроизводства.ПараметрыСмещенияПериодаВыбораСпецификаций(Сценарий);
