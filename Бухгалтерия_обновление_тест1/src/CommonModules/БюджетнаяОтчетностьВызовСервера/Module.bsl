...
Процедура ДобавитьИзмеренияВДеревоНовыхЭлементов(ДеревоЭлементов, СтрокаГруппировки, МодельБюджетирования, ИспользоватьДляВводаПлана, АналитикиШапки)
	...
	ПредставлениеИзмерения = НСтр("ru='Период';uk='Період'");
	...
	ПредставлениеИзмерения = НСтр("ru='Аналитика';uk='Аналітика'");
...
Функция ПериодГруппировкиСоСмещением(Период, ПериодичностьСмещения, ПериодСмещения)
	...
	Если ПериодСмещения > 0 Тогда
		Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"ВЫБРАТЬ ДобавитьКДате(&Период, %1, -%2)",
...
Функция ПолучитьПараметрыОтчета(ПараметрыОтчета, ДанныеЯчеек, СвязаннаяСтрокаДанных, ВидРасшифровки, ИмяПоказателя)
	...
	Для Каждого ВидАналитики Из СтруктураКолонок.ВидыАналитик Цикл
		...
		Если СтрокаСтруктурыЯчеек["Флаг" + Измерение] = "заполнить"
			...
			Если СвязаннаяСтрокаДанных[Измерение] = Неопределено Тогда
				...
				СтрокаПредставление  = НСтр("ru='Аналитика ""%1"" не заполнена';uk='Аналітика ""%1"" не заповнена'");
	...
	Если МинимальнаяПериодичность <> Неопределено Тогда
		ЗначениеПериода = СвязаннаяСтрокаДанных["Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность)];
	...
	Если МинимальнаяПериодичность <> Неопределено Тогда
		...
		Иначе
			МассивИзмеренийКДобавлению.Добавить("Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность));
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ВидРасшифровки = "СтатьяБюджетов" Тогда
		...
		Если ИмяПоказателя = "Сумма" Тогда
			ПолеВыбор.Заголовок = НСтр("ru='Сумма сценария';uk='Сума сценарію'");
		ИначеЕсли ИмяПоказателя = "Количество" Тогда
			ПолеВыбор.Заголовок = НСтр("ru='Количество сценария';uk='Кількість сценарію'");
		Иначе
			ПолеВыбор.Заголовок = НСтр("ru='Значение сценария';uk='Значення сценарію'");
		КонецЕсли;
	...
	ИначеЕсли ВидРасшифровки = "НефинансовыйПоказатель" Тогда
		...
		ПолеВыбор.Использование = Истина;
		ПолеВыбор.Заголовок = НСтр("ru='Значение';uk='Значення'");
