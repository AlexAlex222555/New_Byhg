...
Функция ТекстПоляАналитикиСУсловиемПоКоличествуВидовАналитики(Параметры, НомерТекущейАналитики) Экспорт
	...
	Если ЕстьОшибкиВПараметрах Тогда
		...
		ТекстОшибки = НСтр("ru='Есть ошибки в переданных параметрах';uk='Є помилки в переданих параметрах'",КодЯзыка);
		СобытиеЖР = НСтр("ru='Получение выражения значения аналитики бюджетирования';uk='Отримання вираження значення аналітики бюджетування'",КодЯзыка);
...
//    *ВозвращатьСуммуВВалюте - Булево - Флаг необходимости возвращать сумму в валюте.
...
Функция ФактПоВидуБюджета(ПараметрыДанных, ИсточникиДанных, ВалютаОтчета, РежимПолученияДанных) Экспорт
	...
	Если ИспользоватьАльтернативныйАлгоритм() Тогда
		ТаблицаДанныхПоВидуБюджета = РассчитатьФактПоВидуБюджетаАльтернативный(ПараметрыДанных,
			ИсточникиДанных, ВалютаОтчета, РежимПолученияДанных);
	Иначе
		ТаблицаДанныхПоВидуБюджета = РассчитатьФактПоВидуБюджета(ПараметрыДанных,
			ИсточникиДанных, ВалютаОтчета, РежимПолученияДанных);
	КонецЕсли;
...
Функция ФактическиеДанныеИзИсточников(ИмяСправочникаИсточника, ПараметрыПолученияФакта)
	...
	Если ЭтоОбороты Тогда
		...
		Периодичность = ПараметрыПолученияФакта.Периодичность;
		Если ПоПериодам Тогда
			ВыражениеПериодаПланирования = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
		Иначе
...
Процедура УстановитьВЗапросеВыраженияЗаполненияАналитики(ДокументСсылка, ТекстЗапроса, СтатьяБюджетов, МенеджерОбъекта, ИмяТаблицы = "") Экспорт
	...
	|	СтатьиБюджетов.ВидАналитики%1 КАК ВидАналитики
	|ИЗ
	|	Справочник.СтатьиБюджетов КАК СтатьиБюджетов
	|ГДЕ
	|	СтатьиБюджетов.Ссылка = &СтатьяБюджетов";
	Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
		НовыйТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонТекстаЗапроса,
			НомерАналитики);
...
Процедура УдалитьСохраненныеМакетыБюджетов(Отбор) Экспорт
	...
	Иначе
		...
		УстановитьПривилегированныйРежим(Ложь);
		ТекстОшибки = НСтр("ru='Процедура ""УдалитьСохраненныеМакетыБюджетов"": Неверные параметры вызова';uk='Процедура ""УдалитьСохраненныеМакетыБюджетов"": Невірні параметри виклику'");
		ВызватьИсключение ТекстОшибки;
			...
			Если ЭлементПараметра.ВключатьВДоступныеПоля
			 ИЛИ НЕ ЭлементПараметра.ОграничениеИспользования Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Параметр ""%1"" является зарезервированным, его редактирование пользователем не допускается. Параметр схемы компоновки будет исправлен.';uk='Параметр ""%1"" є зарезервованим, його редагування користувачем не допускається. Параметр схеми компонування буде виправлений.'"),
...
Функция ИменаОбщихПараметровПравилПолученияФакта()
	...
	НеКопируемыеПараметры.Добавить("ТипУпрСуммы");
...
//   *Неоднозначно - Булево - Флаг неоднозначности выбранного выражения аналитики. Если Истина, то в правилах получения
...
Процедура ИнициализироватьКомпоновщикНастроекПравила(Правило, ПараметрыЗаполненияАналитик, НастройкаКомпоновки) Экспорт
	...
	Исключение
		СобытиеЖР = НСтр("ru='Выполнение инициализации компоновщика настроек правила';uk='Виконання ініціалізації компоновщика налаштувань правила'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УстановитьНастройкиЗаполненияАналитикиАвтоматически(ПараметрыЗаполненияАналитик, ВыраженияЗаполненияАналитики = Неопределено) Экспорт
	...
	Если ЭтоАдресВременногоХранилища(ПараметрыЗаполненияАналитик.АдресСхемыКомпоновкиДанных) Тогда
		...
		ШаблонСообщения = НСтр("ru='В источнике есть несколько подходящих вариантов заполнения аналитики ""%1"". Требуется ручной выбор.';uk='У джерелі є декілька відповідних варіантів заповнення аналітики ""%1"". Потрібний ручний вибір.'");
		ШаблонПоля = "НастройкиЗаполненияАналитики[%1].ПредставлениеВыраженияЗаполненияАналитики";
...
|;uk='Перевищено допустиму кількість ітерацій перевірки хешу настойок.
| Виконання перервано для запобігання зациклення механізму запису об''єкта.'");
					ВызватьИсключение ТекстОшибки;
				КонецЕсли;
...
Процедура ФактСтатьиБюджетовПоПравилу(Правило, Параметры, ТаблицаФакта, ДополнятьСценарийИсполнения = Истина) Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипПравилаПолученияФактическихДанныхБюджетирования.ИсполнениеБюджетаИФактическиеДанные) КАК ТипПравила,
	|	ЗНАЧЕНИЕ(Справочник.Сценарии.ИсполнениеБюджета) КАК Сценарий";
	ЗаголовокПоля = НСтр("ru='Тип правила';uk='Тип правила'");
	ФинансоваяОтчетностьСервер.НовоеПолеНабора(
		НаборДанныхСценарии,
		"ТипПравила",
		"ТипПравила",
	...
	ЗаголовокПоля = НСтр("ru='Сценарий';uk='Сценарій'");
	ФинансоваяОтчетностьСервер.НовоеПолеНабора(
		НаборДанныхСценарии,
		"Сценарий",
		"Сценарий",
	...
	Если Параметры.Периодичность <> Неопределено Тогда
		...
		ИмяПоляПериод = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Параметры.Периодичность);
...
|;uk='У схемі джерела даних відсутнє поле періоду.
|Отримання даних в розрізі періоду неможливо.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
|;uk='У схемі джерела даних відсутнє поле реєстратора. 
|Отримання даних в розрізі реєстратора неможливо.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура ФактПоказателяБюджетовПоПравилу(Правило, Параметры, ТаблицаФакта, Знач ПереопределеннаяСКД = Неопределено) Экспорт
	...
	КонецЕсли;     
 	...
 	ПараметрТипУпрСуммы = СхемаКомпоновкиДанных.Параметры.Найти("ТипУпрСуммы");
	Если ПараметрТипУпрСуммы <> Неопределено Тогда
		ПараметрТипУпрСуммы.Значение = Правило.ТипУпрСуммы;
	КонецЕсли;
...
Процедура ЗначенияНефинансовогоПоказателяПоПравилу(Правило, Параметры, ТаблицаЗначений) Экспорт
	...
	Если СхемаКомпоновкиДанных = Неопределено Тогда
		Если НЕ ЗначениеЗаполнено(ШаблонВводаНФП) Тогда
			ШаблонСообщения = НСтр("ru='Не определена схема компоновки данных для нефинансового показателя ""%1""';uk='Не визначена схема компонування даних для нефінансового показника ""%1""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонСообщения, Правило.НефинансовыйПоказатель);
		Иначе
			ШаблонСообщения = НСтр("ru='Не определена схема компоновки данных в шаблоне ввода нефинансовых показателей ""%1"" для нефинансового показателя ""%2""';uk='Не визначена схема компонування даних в шаблоні введення нефінансових показників ""%1"" для нефінансового показника ""%2""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если Не СхемаКомпоновкиДанных.НаборыДанных.Количество() Тогда
		Если НЕ ЗначениеЗаполнено(ШаблонВводаНФП) Тогда
			ШаблонСообщения = НСтр("ru='Отсутствует набор данных в схеме заполнения для нефинансового показателя ""%1""';uk='Відсутній набір даних в схемі заповнення для нефінансового показника ""%1""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Правило.НефинансовыйПоказатель);
		Иначе
			ШаблонСообщения = НСтр("ru='Отсутствует набор данных в схеме заполнения шаблона ввода нефинансовых показателей ""%1"" для нефинансового показателя ""%2""';uk='Відсутній набір даних в схемі заповнення шаблону введення нефінансових показників ""%1"" для нефінансового показника ""%2""'");
	...
	Если ПоляНабора.Найти("ЗначениеПоказателя") = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(ШаблонВводаНФП) Тогда
			ШаблонСообщения = НСтр("ru='В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле с именем ""%2""';uk='У схемі компоновки даних нефінансового показника ""%1"" відсутнє поле з ім''ям ""%2""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				Правило.НефинансовыйПоказатель,
				ИмяПоляНаЯзыкеКода);
		Иначе
			ШаблонСообщения = НСтр("ru='В схеме компоновки данных шаблона ввода нефинансовых показателей ""%1"" для нефинансового показателя ""%2"" отсутствует поле с именем ""%3""';uk='У схемі компоновки даних шаблону введення нефінансових показників ""%1"" для нефінансового показника ""%2"" відсутнє поле з ім''ям ""%3""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ШаблонВводаНФП,
				Правило.НефинансовыйПоказатель,
				ИмяПоляНаЯзыкеКода);
	...
	Если ПоляНабора.Найти("Период") = Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(ШаблонВводаНФП) Тогда
			...
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				...
				ИмяПоляНаЯзыкеКода);
		Иначе
			ШаблонСообщения = НСтр("ru='В схеме компоновки данных шаблона ввода нефинансовых показатей ""%1"" для нефинансового показателя ""%2"" отсутствует поле с именем ""%3""';uk='У схемі компоновки даних шаблону введення нефінансових показників ""%1"" для нефінансового показника ""%2"" відсутнє поле з ім''ям ""%3""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ШаблонВводаНФП,
				Правило.НефинансовыйПоказатель,
				ИмяПоляНаЯзыкеКода);
...
// 	 *ПоляСуммы - Массив из Строка - Массив имен полей сумм.
// 	 *ПоляСуммыВВалюте - Массив из Строка - Массив имен полей сумм в валюте.
// 	 *ПоляКоличества - Массив из Строка - Массив имен полей количества.
//
Процедура ПересчитатьТаблицуДанныхВВалюту(ТаблицаДанных, Параметры, СохранятьВалюту = Ложь, ПоляРесурсов = Неопределено) Экспорт
	...
	НаборТаблицаФакт = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(СхемаКомпоновкиДанных,
	...
	Для каждого Набор Из НаборыКурсовВалют Цикл
		...
		НаборКурсыВалют = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(СхемаКомпоновкиДанных,
			Тип("НаборДанныхОбъектСхемыКомпоновкиДанных"),
			ИмяНабора);
	...
	Если Правило.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.ОперативныйУчет Тогда
		...
		Если РесурсыИсточникаСуммы = Неопределено Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для источника суммы %1 хозяйственной операции %2 не найдены соответствующие ресурсы в источнике данных.';uk='Для джерела суми %1 господарської операції %2 не знайдені відповідні ресурси в джерелі даних.'"), 
...
Функция ПолеРесурсаБухгалтерии(ПолеСумма, ТипИтога) Экспорт
	...
	Возврат СтрЗаменить(ПолеСумма + ОбщегоНазначения.ИмяЗначенияПеречисления(ТипИтога), " ","");
...
Функция УстановитьНаиболееПодходящиеПоляЗаполненияСуммы(ИсточникиСуммы, СхемаКомпоновкиДанных, Настройки, Правило, Параметры)
	...
	Если ИсточникиСуммы.Свойство("Валюта") Тогда
		...
		КонецЕсли;
		Если ПолеНабораДанныхВалюта = Неопределено Тогда
			ТекстОшибки = НСтр("ru='Не определена валюта для получения валютной суммы.';uk='Не визначена валюта для отримання валютної суми.'");
			ВызватьИсключение ТекстОшибки;
		Иначе
...
// * ПериодКурса - Дата -
// * ТребуетсяПересчетПоКурсу - Булево -
...
Процедура ПоставитьДокументВОчередьПроведения(Документ) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		СобытиеЖР = НСтр("ru='Отложенное проведение экземпляров бюджета';uk='Відкладене проведення примірників бюджету'",ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(
			СобытиеЖР, 
			УровеньЖурналаРегистрации.Ошибка,,, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗапуститьОтложенноеПроведениеФоновымЗаданием(Ссылка) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Выполняется отложенное проведение экземпляров бюджета';uk='Виконується відкладене проведення примірників бюджету'");
...
Процедура СохранитьХранимыйМакет(МакетДляХранения, ВспомогательныеДанные, СсылкаНаБюджет) Экспорт
	...
	Иначе
		ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ru='Бюджет';uk='Бюджет'"));
	...
	Если СохраненныеФайлы.Свойство("ВспомогательныеДанные", Файл) Тогда
		...
		РаботаСФайлами.ОбновитьФайл(Файл, ПараметрыФайла);
	Иначе
		ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ru='Вспомогательные данные бюджета';uk='Допоміжні дані бюджету'"));
...
// Возвращает таблицу источников данных.
// 
// Параметры:
// 	ИсточникиДанных - Массив из см. БюджетнаяОтчетностьРасчетКэшаСервер.НовыйИсточник - Источников данных вида бюджета.
// Возвращаемое значение:
// 	ТаблицаЗначений - таблица источников данных:
// 	 *Идентификатор - Строка - Идентификатор источника.
Функция ТаблицаИсточниковДанных(ИсточникиДанных)
	Возврат ОбщегоНазначенияУТ.МассивВТаблицуЗначений(ИсточникиДанных, Истина);
КонецФункции
...
Функция РассчитатьФактПоВидуБюджетаАльтернативный(ПараметрыДанных, Знач ИсточникиДанных, ВалютаОтчета, РежимПолученияДанных)
	...
	ИсточникиДанных = ТаблицаИсточниковДанных(ИсточникиДанных);
    ЕстьКолонкаТипУпрСуммы = (ИсточникиДанных.Колонки.Найти("ТипУпрСуммы") <> Неопределено);
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Иначе
			...
			Если ПолучатьФактическиеДанные Тогда
				...
				Если ЭтоОстатки И ЕстьКолонкаТипУпрСуммы Тогда
					ЗначенияПоУмолчанию.ТипУпрСуммы = ИсточникДанных.ТипУпрСуммы;
				Иначе	
					ЗначенияПоУмолчанию.ТипУпрСуммы = -1;
				КонецЕсли;
	...
	Если ИсключитьСценарий Тогда
		...
		Для каждого ЭлементОтбора Из ЭлементыОтбораОстатки Цикл
			Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
				...
				ГруппаИЛИ.Представление = НСтр("ru='Отбор по сценариям при получении факта применяется только для статей (включая влияющие на показатель статьи) и целевые показатели';uk='Відбір за сценаріями при отриманні факту застосовується тільки для статей (включаючи впливаючі на показник статті) і цільовіих показників'");
...
Функция ВыражениеЗаполненияПоказателяСумма(Знач ПараметрыПолученияФакта, Знач СтрокаДопДанных)
	...
	Если Не ПолеЗаполненияСуммыУстановлено Тогда
		Для каждого ИсточникСуммы Из СтрокаДопДанных.ВыраженияПоказателейСуммы Цикл
			...
			ТребуетсяПересчетПоКурсуМеждународ = Ложь;
...
Процедура ДобавитьКолонкиДополнительныхДанных(ТаблицаДополнительныхДанных, ИндексИменИВидовАналитик)
	...
	Если ТребуетсяЗаполнение Тогда
		...
		ОписаниеТипаТипУпрСуммы = ОбщегоНазначения.ОписаниеТипаЧисло(1,, ДопустимыйЗнак.Любой);
		...
		КолонкиТаблицыДанных.Добавить("ТипУпрСуммы", ОписаниеТипаТипУпрСуммы);
...
Функция ШаблонПараметровИдентификацииВыражений(НастройкиСоставаПолей)
	...
	ЗначенияПоУмолчанию.Вставить("ТипУпрСуммы");
