...
|За рядком %4 оформлено більше, ніж вказано в розпорядженні, на %5 %6'");
...
|;uk='Номенклатура %1, склад %2, підрозділ %3 
|За рядком %4 вже оформлена передача матеріалів в кількості більшій, ніж зазначено в документі, на %5 %6'");
...
|;uk='Кількість за видом робіт ""%1"" 
|перевищує або не відповідає даним розпорядження (на %2 %3)'");
		Иначе
			ШаблонСообщения = НСтр("ru='Выработка сотрудников по виду работ ""%1""
| уже оформлена на %2 %3'
|;uk='Виробіток співробітників за видом робіт  %1"" 
|вже оформлено на %2 %3'");
		КонецЕсли;
...
Процедура ЗапуститьЗаданиеОбработкиОчереди(Очередь, Отбор = Неопределено, Идентификатор = Неопределено) Экспорт
	...
	НаименованиеЗадания = СтрШаблон(НСтр("ru='Обработка очереди <%1>';uk='Обробка черги <%1>'"), Метаданные.РегистрыСведений[Очередь].Синоним);
...
Процедура РассчитатьДолюСтоимости(Объект, ПараметрыРаспределенияЗатрат) Экспорт
	...
	Если Не ПустаяСтрока(ПараметрыРаспределенияЗатрат.ИмяПоляСпособРаспределенияЗатратНаВыходныеИзделия) Тогда
		ВызватьИсключение НСтр("ru='Некорректно переданы параметры распределения затрат.';uk='Некоректно передані параметри розподілу витрат.'");
	КонецЕсли;
...
Процедура Использование_ОформлениеПроизводстваБезЗаказов(ПараметрыОбработчика) Экспорт
	...
	Если РазмерыВременныхТаблиц.ВТНеоформленноеПроизводство = 0 И РазмерыВременныхТаблиц.ВТДокументыПроизводства = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет материалов и работ для списания на производство без заказов';uk='Немає матеріалів і робіт для списання на виробництво без замовлень'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура Оформление_ОформлениеПроизводстваБезЗаказов(ПараметрыОбработчика) Экспорт
	...
	Если НЕ (НастройкиПроизводства.ИспользуетсяПроизводство21 И НастройкиПроизводства.ИспользуетсяПроизводство22) Тогда
		ПараметрыОбработчика.ДанныеЭтапа.Наименование = НСтр("ru='Оформление производства без заказов';uk='Оформлення виробництва без замовлень'");
	КонецЕсли;
...
Процедура ОписаниеПроверок_ОформлениеПроизводстваБезЗаказов(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не списаны материалы (работы) и трудозатраты на производство';uk='Не списані матеріали (роботи) і трудовитрати на виробництво'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должно быть оформлено списание всех материалов (работ) и трудозатрат на производство без заказов.';uk='Повинно бути оформлено списання всіх матеріалів (робіт) і трудовитрат на виробництво без замовлень.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиОформленияПроизводстваБезЗаказов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("НомерГТД", 					НСтр("ru='Номер ГТД';uk='Номер ВМД'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Обнаружены не списанные на производство без заказа материалы (работы) организации ""%1"" на конец периода %2';uk='Виявлені не списані на виробництво без замовлення матеріали (роботи) організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура Использование_ВыработкаСотрудников(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			ПараметрыОбработчика,
			НСтр("ru='Производственный учет не ведется.';uk='Виробничий облік не ведеться.'",ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Если РазмерыВременныхТаблиц.Обороты = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='За указанный период работы сотрудниками не выполнялись, оформление выработки не требуется.';uk='За вказаний період роботи співробітниками не виконувалися, оформлення виробітку не потрібно.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Иначе
		...
		ЗакрытиеМесяцаСервер.ЗафиксироватьРезультатыОбработчикаИспользованияПоОрганизациям(
			ПараметрыОбработчика,
			НСтр("ru='По организации ""%1"" за период %2 выработка оформлена не по всем сотрудникам.';uk='По організації ""%1"" за період %2 виробіток оформлено не по всіх співробітниках.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			Запрос,
...
Функция ВыбратьИзОчереди(Очередь, МенеджерВременныхТаблиц) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	" + СтрСоединить(СписокПолейВыборки, ",") + "
	|ИЗ
	|	ОбрабатываемыеЗадания КАК ОбрабатываемыеЗадания
	|СГРУППИРОВАТЬ ПО
	|	" + СтрСоединить(СписокПолейГруппировки, " , ") + "
	|");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
...
Процедура УдалитьЗадание(Очередь, Задание, МенеджерВременныхТаблиц) Экспорт
	...
	|	ОбрабатываемыеЗадания КАК ОбрабатываемыеЗадания
	|ГДЕ
	|	" + СтрСоединить(СписокПолейОтбора," И ") + "
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Набор = РегистрыСведений[Очередь].СоздатьНаборЗаписей();//РегистрСведенийНаборЗаписей - 
...
Процедура ПроверитьВозможностьВводаНаОсновании(Реквизиты, ХозяйственнаяОперация) Экспорт
	...
	Если НЕ Реквизиты.Проведен Тогда
		ТекстОшибки = НСтр("ru='Невозможно оформить документ на основании непроведенных распоряжений.';uk='Неможливо оформити документ на підставі непроведених розпоряджень.'");
...
|;uk='Немає продукції, доступної для передачі. Для початку процесу передачі продукції 
|необхідно встановити ознаку ""Вироблено"" в етапі виробництва.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	
	Если Реквизиты.КоличествоОрганизаций > 1 Тогда
		ТекстОшибки = НСтр("ru='Невозможно оформить документ по разным организациям.';uk='Неможливо оформити документ по різних організаціях.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли; 
...
|;uk='Неможливо оформити передачу матеріалів, тому що матеріали відвантажуються з різних складів. 
|Для оформлення документа рекомендується використовувати форму ""Отримання і повернення матеріалів"".'");
		Иначе
		//-- Устарело_Производство21
...
|Для оформления документа рекомендуется использовать форму ""Получение и возврат материалов"".'
|;uk='Неможливо оформити передачу матеріалів, тому що матеріали відвантажуються в різні підрозділи. 
|Для оформлення документа рекомендується використовувати форму ""Отримання і повернення матеріалів"".'");
		Иначе
		//-- Устарело_Производство21	
			ТекстОшибки = НСтр("ru='Невозможно оформить передачу материалов, 
...
|Для оформления документа рекомендуется использовать форму ""Получение и возврат материалов"".'
|;uk='Неможливо оформити передачу продукції, тому що продукція передається на різні склади. 
|Для оформлення документа рекомендується використовувати форму ""Отримання і повернення матеріалів"".'");
		Иначе
		//-- Устарело_Производство21	
			ТекстОшибки = НСтр("ru='Невозможно оформить передачу продукции, 
|т.к продукция передается на разные склады.'
|;uk='Неможливо оформити передачу продукції, 
|тому що продукція передається на різні склади.'");
		//++ Устарело_Производство21	
		КонецЕсли;
		//-- Устарело_Производство21
		ВызватьИсключение ТекстОшибки;
...
|Для оформления документа рекомендуется использовать форму ""Получение и возврат материалов"".'
|;uk='Неможливо оформити передачу продукції, тому що продукція передається в різні підрозділи. 
|Для оформлення документа рекомендується використовувати форму ""Отримання і повернення матеріалів"".'");
		Иначе
		//-- Устарело_Производство21	
			ТекстОшибки = НСтр("ru='Невозможно оформить передачу продукции, 
...
|;uk='Неможливо оформити передачу продукції, 
|тому що продукція передається в різні підрозділи.'");
		//++ Устарело_Производство21	
		КонецЕсли;
		//-- Устарело_Производство21
		ВызватьИсключение ТекстОшибки;
...
Процедура ВыполнитьПодстановкуОтборовВТекстЗапросаДинамическогоСписка(ТекстЗапроса, СписокОтборов, ИмяТаблицы = "ОсновнаяТаблица") Экспорт
	...
	Для каждого Отбор Из СписокОтборов Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru='Передана несуществующая операция отбора!';uk='Передана неіснуюча операція відбору!'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
...
Функция СобытиеРасчетНормативнойДлительности() Экспорт
	...
	Возврат НСтр("ru='НСИ производства.Расчет нормативной длительности';uk='НДІ виробництва. Розрахунок нормативної тривалості'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеСозданиеСпецификации() Экспорт
	
	Возврат НСтр("ru='НСИ производства.Создание спецификации';uk='НДІ виробництва. Створення специфікації'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЗаменаСпецификаций() Экспорт
	
	Возврат НСтр("ru='Межцеховое управление.Замена спецификаций';uk='Міжцехове управління. Заміна специфікацій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеПланированиеГрафикаПроизводства() Экспорт
	
	Возврат НСтр("ru='Межцеховое управление.Планирование графика производства';uk='Міжцехове управління. Планування графіка виробництва'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
КонецФункции

Функция СобытиеРедактированиеЦепочкиЭтапов() Экспорт
	
	Возврат НСтр("ru='Межцеховое управление.Редактирование цепочки этапов';uk='Міжцехове управління..Редагування ланцюжка етапів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеПередачаЭтаповКВыполнению() Экспорт
	
	Возврат НСтр("ru='Межцеховое управление.Передача этапов к выполнению';uk='Міжцехове управління. Передача етапів до виконання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеПометкаНаУдалениеЭтапов() Экспорт
	
	Возврат НСтр("ru='Межцеховое управление.Пометка на удаление этапов производства';uk='Міжцеховее управління..Помітка на вилучення етапів виробництва'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеРасчетОчередиПроизводственныхОпераций() Экспорт
	
	Возврат НСтр("ru='Внутрицеховое управление.Формирование очереди производственных операций';uk='Внутрішньоцехове управління. Формування черги виробничих операцій'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЗаполнениеЭтаповПоПроизводственнымОперациям() Экспорт
	
	Возврат НСтр("ru='Внутрицеховое управление.Заполнение этапов по производственным операциям';uk='Внутрішньоцехове управління. Заповнення етапів за виробничими операціями'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеРасчетЗначенияСвойстваПриАвтовыборе() Экспорт
	
	Возврат НСтр("ru='НСИ производства.Расчет значения свойства при автовыборе';uk='НДІ виробництва. Розрахунок значення властивості при автовиборі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеАвтовыборХарактеристикиПоАлгоритму() Экспорт
	
	Возврат НСтр("ru='НСИ производства.Автовыбор характеристики по алгоритму';uk='НДІ виробництва. Автовибір характеристики за алгоритмом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеРасчетКоличестваПоФормуле() Экспорт
	
	Возврат НСтр("ru='НСИ производства.Расчет количества по формуле';uk='НДІ виробництва. Розрахунок кількості за формулою'",ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеОбработкаОчередиЗаданий() Экспорт
	
	Возврат НСтр("ru='Обработка очереди заданий';uk='Обробка черги завдань'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
//                                           		  Ложь: все строки
// 	СтруктураПолейТаблицы - Структура        		- см. ПроизводствоСервер.СтруктураПолейТаблицы
...
Функция ПоместитьВоВременноеХранилищеДанныеСтрок(
	...
	Объект = ?(ЗначениеЗаполнено(ИмяОбъекта), Форма[ИмяОбъекта], Форма);
	ТаблицаДанных = Объект[ИмяТаблицы];
	...
	Таблица = ?(
		ТипЗнч(ТаблицаДанных) = Тип("ТаблицаЗначений"), 
			ТаблицаДанных.СкопироватьКолонки(ОсновныеПоля), 
			ТаблицаДанных.Выгрузить(Новый Массив, ОсновныеПоля));// ТаблицаЗначений
