...
Функция ТекстДанныеПоМатериальнымЗатратам(ДляРасшифровки)
	...
	Если ДляРасшифровки Тогда
		...
		|		ДД.Партия,
        |		СпрПартииПроизводства.НалоговоеНазначение,
		|		ЕСТЬNULL(СпрПартииПроизводства.Назначение.НаправлениеДеятельности,
...
Процедура ЗаполнитьРасчетнуюПартиюРаспределенияНоменклатурыНаПроизводство(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	Если РасчетнаяПартия.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ПотреблениеАвто Тогда
		...
		РасчетнаяПартия.ИдентификаторСтроки = Строка(Новый УникальныйИдентификатор);
		
	Иначе
		
		ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход, "РасчетЗавершен, Организация, ХозяйственнаяОперация, Количество,
			|Стоимость, СтоимостьРегл, ПостояннаяРазница");
...
Процедура ПолучитьДанныеДляРаспределенияНоменклатурыНаВыпуск(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПолучитьДанныеЭтапаРасчета(
		ПараметрыРасчета,
		ТекстЗапросаДляРаспределенияНоменклатурыНаВыпуск(ПараметрыРасчета),
		ПараметрыНумерации);
		
	Если ПараметрыРасчета.РежимОтладки Тогда
		
		ОстаткиМатериалов                             = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ОстаткиМатериалов");
		ОстаткиМатериаловСводно                       = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ОстаткиМатериаловСводно");
		...
		РаспределенныеОтчеты                          = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "РаспределенныеОтчеты");
		...
		ПланФактМатериалов                            = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ПланФактМатериалов");
...
Функция ТекстЗапросаДляРаспределенияНоменклатурыНаВыпуск(ПараметрыРасчета = Неопределено) Экспорт
...
Функция ТекстЗацикленнаяНоменклатураНаВыпуск()
    ...
    |	Продукция.НалоговоеНазначение                                        КАК КорНалоговоеНазначение,
	|	Продукция.АналитикаФинансовогоУчета                                  КАК КорАналитикаФинансовогоУчета,
	|	НЕОПРЕДЕЛЕНО                                                         КАК СтатьяКалькуляции,
...
Функция ТекстРаспределениеНоменклатурыНаВыпускПоПродукции() // использует Продукция
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                                                КАК КорАналитикаУчетаПартий,
    |	ЕСТЬNULL(Движения.НалоговоеНазначение, ДД.НалоговоеНазначение) 		 КАК КорНалоговоеНазначение,
...
Процедура ЗаполнитьРасчетнуюПартиюРаспределениеНоменклатурыНаВыпуск(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	Если Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Расчетное Тогда
		...
		ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход, "РасчетЗавершен,ВидЗапасов,АналитикаУчетаПартий,
            |АналитикаФинансовогоУчета,НалоговоеНазначение,РазделУчета");
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход, "РасчетЗавершен,СтатьяКалькуляции,ВидЗапасов,АналитикаУчетаПартий,
            |АналитикаФинансовогоУчета,НалоговоеНазначение,АналитикаУчетаНоменклатуры,РазделУчета");
...
Процедура ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета, ТекстЗапроса) Экспорт
	...
	ИначеЕсли ИмяЭтапа = "РаспределениеНоменклатурыНаВыпуск" Тогда
		ТекстЗапроса = ТекстЗапросаДляРаспределенияНоменклатурыНаВыпуск(ПараметрыРасчета);
	КонецЕсли;
