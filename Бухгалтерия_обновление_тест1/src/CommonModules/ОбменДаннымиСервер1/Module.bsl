...
Функция ШаблонИмениФайлаСообщения(ТекущийУзелПланаОбмена, УзелИнформационнойБазы, ЭтоИсходящееСообщение, Транслитерация = Ложь, ИспользоватьВиртуальныйКодУзлаПриПолучении = Ложь) Экспорт
	...
	Если Транслитерация Тогда
		...
		ИмяФайлаСообщения = ИмяФайлаСообщения;
	КонецЕсли;
...
Процедура ПриЗаписиСправочникОбъектПриЗаписи(Источник, Отказ) Экспорт
	ТекущийУзел = ПланыОбмена.ОбменУправлениеПредприятиемМагазины.ЭтотУзел();
	УзелОбмена = ПланыОбмена.ОбменУправлениеПредприятиемМагазины.НайтиПоКоду("ЦБ"); // узел услуг
	массивузлов = ПолучитьСписокУзлов(); 	
	Если УзелОбмена = ТекущийУзел Тогда  
		Если Источник.ПометкаУдаления Тогда
			ПланыОбмена.УдалитьРегистрациюИзменений(массивузлов,Источник.Ссылка);
		Иначе                                                        
			ПланыОбмена.ЗарегистрироватьИзменения(массивузлов,Источник.Ссылка);
			Попытка
				ПланыОбмена.ЗарегистрироватьИзменения(массивузлов,Источник.НоменклатураГТД.КодУКТВЭД);   
			Исключение
			КонецПопытки;
		КонецЕсли;	     
	КонецЕсли;	
...
Функция ПолучитьСписокУзлов()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОбменУправлениеПредприятиемМагазины.Ссылка
		|ИЗ
		|	ПланОбмена.ОбменУправлениеПредприятиемМагазины КАК ОбменУправлениеПредприятиемМагазины
		|ГДЕ
		|	ОбменУправлениеПредприятиемМагазины.Ссылка <> &Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", ПланыОбмена.ОбменУправлениеПредприятиемМагазины.НайтиПоКоду("ЦБ")); 
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Масс.Добавить(ВыборкаДетальныеЗаписи.ссылка);
	КонецЦикла;
...
Процедура ОбработкаПроведенияДокументОбъектОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	ТекущийУзел = ПланыОбмена.ОбменУправлениеПредприятиемМагазины.ЭтотУзел();
	УзелОбмена = ПланыОбмена.ОбменУправлениеПредприятиемМагазины.НайтиПоКоду("ЦБ"); // узел услуг
