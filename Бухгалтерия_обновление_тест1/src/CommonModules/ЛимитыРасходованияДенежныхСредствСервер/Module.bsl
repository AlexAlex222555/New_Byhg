...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если ТипЗнч(Документ) = Тип("ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств")
			...
			//++ НЕ УТ
			ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования") Тогда
...
Процедура ПослеЗаписиДвиженийДокумента(Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	// Контроль лимитов по данным бюджетирования при проведении заявки на расходование ДС.
	Ошибки = Неопределено;
...
//	 *ОтборПоАналитикамПлана - Массив структур - отбор по вышестоящим аналитикам плана (если задан). 
...
//	 *ОтборПоАналитикамФакта - Массив структур - отбор по вышестоящим аналитикам факта (если задан).
//	   **ИмяИзмеренияНомерАналитики - Строка - Имя измерения или путь к нумерованной аналитике (вида "Аналитика1", здесь номер тот, который задан в статье бюджетов).
//	   **Значение - Характеристика.АналитикиСтатейБюджетов - Значение измерения или аналитики.
//
// Возвращаемое значение:
	...
	Если ПродолжитьРасчет Тогда
		...
		Для Каждого СтрокаПланФакта Из ПланФактПоПравилам Цикл
			Если СтрокаПланФакта.СуммаПревышения = 0 Тогда
				Продолжить;
			КонецЕсли;
			ИерархияАналитик = СработавшиеЛимиты.ИерархияАналитикПоПравилам.Получить(СтрокаПланФакта.Правило);
...
|;uk='Така заявка не потрапляє ні в одне ""Дозволяюче"" правило лімітування. 
|Заявка може бути тільки понад-лімітної.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "",ТекстСообщения, "");
...
|Заявка может быть только сверх-лимитной.'
|;uk='Така заявка не потрапляє у жодне правило лімітування. 
...
Функция ТекстСообщенияПроверкиПревышенияЛимитовПоДаннымБюджетирования(ИерархияАналитик, СтрокаПланФакта, ДатаПроверки, КэшПредставлений)
	...
	Если ПредставлениеПравилаЛимитов = Неопределено Тогда
		ПредставлениеПравилаЛимитов = ПрименяемоеПравилоЛимита.Наименование;
	...
	Если ПравоИспользованияОтчета Тогда
		Если ЗначениеЗаполнено(ПредставлениеАналитикЛимита) Тогда
			ТекстСообщения = НСтр("ru='Общее превышение на %1 %4 лимита ""%5"" по аналитикам запланированного бюджета %6.';uk='Загальне перевищення на %1 %4 ліміту ""%5"" по аналітикам запланованого бюджету %6.'");
		Иначе
			ТекстСообщения = НСтр("ru='Общее превышение на %1 %4 лимита ""%5"".';uk='Загальне перевищення на %1 %4 ліміту ""%5"".'");
		КонецЕсли;
		Если СтрокаПланФакта.СуммаПлан = 0 Тогда
			ТекстСообщения = ТекстСообщения
				+ Символы.ПС
				+ НСтр("ru='Лимит в периоде %7 не установлен.';uk='Ліміт в періоді %7 не встановлено.'");
		ИначеЕсли СтрокаПланФакта.СуммаМожноСписать > 0 Тогда
			ТекстСообщения = ТекстСообщения
				+ Символы.ПС
				+ НСтр("ru='Всего лимит в периоде %7 - %2 %4. Можно списать %3 %4.';uk='Всього ліміт у періоді %7 - %2 %4. Можна списати %3 %4.'");
		Иначе
			ТекстСообщения = ТекстСообщения
				+ Символы.ПС
				+ НСтр("ru='Всего лимит в периоде %7 - %2 %4. Лимит израсходован.';uk='Всього ліміт у періоді %7 - %2 %4. Ліміт витрачений.'");
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАналитикЛимита) Тогда
			ТекстСообщения = НСтр("ru='Общее превышение лимита ""%5"" по аналитикам запланированного бюджета %6 за период %7.';uk='Загальне перевищення ліміту ""%5"" по аналітикам запланованого бюджету %6 за період %7.'");
		Иначе
			ТекстСообщения = НСтр("ru='Общее превышение лимита ""%5"" за период %7.';uk='Загальна перевищення ліміту ""%5"" за період %7.'");
		КонецЕсли;
		Если СтрокаПланФакта.СуммаМожноСписать > 0 Тогда
			ТекстСообщения = ТекстСообщения
			+ Символы.ПС
			+ НСтр("ru='Можно списать %3 %4.';uk='Можна списати %3 %4.'");
		Иначе
			ТекстСообщения = ТекстСообщения
			+ Символы.ПС
			+ НСтр("ru='Лимит израсходован.';uk='Ліміт витрачений.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РассчитатьБлижайшиеАналитикиПланаСУчетомИерархии(ФактическиеДанные, ПравилаЛимитов, ПравилаЛимитовСДопДанными, ПлановыеДанные)
	...
	ИерархияАналитикПоПравилам = ПравилаЛимитов.ИерархияАналитикПоПравилам;
	СценарииПравил = ПравилаЛимитов.СценарииПравил;
	МаксимальноеКоличествоАналитик = БюджетированиеКлиентСервер.МаксимальноеКоличествоАналитик();
...
Функция ПлановыеДанныеДляПланФакта(ПравилаЛимитовСДопДанными, ФактическиеДанные, ДополнительныеПараметры)
	...
	Если ИспользоватьНесколькоВалют Тогда
		...
		ПолеРасчетнойВалюты.Выражение = "&ЦелеваяВалютаПлан1";
...
Функция СвернутыйФактДляПланФакта(ПравилаЛимитовСДопДанными, ФактическиеДанные, ПлановыеДанные, ДополнительныеПараметры)
	...
	Если ИспользоватьНесколькоВалют Тогда
		...
		ПолеРасчетнойВалюты.Выражение = "&ЦелеваяВалютаФакт1";
...
Функция ДанныеДополнительныхПолей(ПараметрыПолученияФакта, ВспомогательныеДанные)
	...
	ИерархияАналитикПоПравилам = ВспомогательныеДанные.ИерархияАналитикПоПравилам;
...
// 	ДействующиеЛимиты - Структура - см. ДействующиеЛимиты().
...
// 	 *ОтборПоАналитикамФакта - Массив структур - массив элементов отбора по аналитикам факта, если они заданы. Необязательное свойство.
...
//			** Значение - Массив из Строка - массив строк, описывающих примененные индексы.
//
