...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ЕстьДекларации, ВозвратныеОтходы
	Возврат 
        ...
        |		 И ЕСТЬNULL(КорПартияПроизводства.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка))
        |				<> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА КорПартияПроизводства.НалоговоеНазначение 
		//-- НЕ УТ
		|		КОГДА НЕ Т.СлужебноеВидДвиженияПриход
		...
		//++ НЕ УТ
		//++ Устарело_Производство21
        |		КОГДА НЕ ПередачаВПроизводство.НалоговоеНазначение ЕСТЬ NULL
        |			ТОГДА
        |				ВЫБОР КОГДА ПередачаВПроизводство.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |					ТОГДА ПередачаВПроизводство.НалоговоеНазначение
        |					ИНАЧЕ УчетныеПолитикиОрганизаций.НалоговоеНазначениеПоУмолчанию
        |				КОНЕЦ
		//-- Устарело_Производство21
        |		КОГДА НЕ ДвижениеПродукцииИМатериалов.НалоговоеНазначение ЕСТЬ NULL
        |			ТОГДА
        |				ВЫБОР КОГДА ДвижениеПродукцииИМатериалов.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |					ТОГДА ДвижениеПродукцииИМатериалов.НалоговоеНазначение
        |					ИНАЧЕ УчетныеПолитикиОрганизаций.НалоговоеНазначениеПоУмолчанию
        |				КОНЕЦ
		//-- НЕ УТ
		...
		//++ НЕ УТ
		//++ Устарело_Производство21
        |		КОГДА НЕ ПередачаВПроизводство.НалоговоеНазначение ЕСТЬ NULL И НЕ Т.СлужебноеВидДвиженияПриход
        |		 И ПередачаВПроизводство.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА ПередачаВПроизводство.НалоговоеНазначение
		|		КОГДА ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ПеремещениеМатериаловВПроизводстве)
        |			ТОГДА ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
		//-- Устарело_Производство21
        |		КОГДА НЕ ДвижениеПродукцииИМатериалов.НалоговоеНазначение ЕСТЬ NULL И НЕ Т.СлужебноеВидДвиженияПриход
        |		 И ДвижениеПродукцииИМатериалов.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА ДвижениеПродукцииИМатериалов.НалоговоеНазначение
		//-- НЕ УТ
		|		КОГДА Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноСписанияНаРасходы)
        |			ТОГДА ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
		...
		//++ НЕ УТ
		|		КОГДА НЕ Т.СлужебноеВидДвиженияПриход
        |		 И ЕСТЬNULL(КорПартияПроизводства.НалоговоеНазначение, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка))
