	...
	Если ТипЗнч(Элемент) = Тип("ДанныеФормыЭлементДерева")
		...
		Иначе
			...
			Если СтруктураЭлемента.ВидЭлемента = ВидЭлемента("СтатьяБюджетов")
				...
				СтруктураЭлемента.Вставить("ДополнительныйРеквизит_НижняяГраницаДанных", ПеременныеПериодов.НачалоПериодаДанных.Имя);
				СтруктураЭлемента.Вставить("ДополнительныйРеквизит_ВерхняяГраницаДанных", ПеременныеПериодов.КонецПериодаДанных.Имя);
				СтруктураЭлемента.Вставить("ДополнительныйРеквизит_НачалоПериодаГруппировки", ПеременныеПериодов.ПериодГруппировки.Имя);
				СтруктураЭлемента.Вставить("ДополнительныйРеквизит_КонецПериодаГруппировки", ПеременныеПериодов.ПериодГруппировки.Имя);
	...
	Если ПредставлениеПустогоЗначения = Неопределено Тогда
		...
		Если ПредставлениеИзмеренияВидаАналитики = Неопределено Тогда
			Если ТипЗнч(ИзмерениеВидАналитики) = Тип("Строка") Тогда
				...
				Если ИзмерениеВидАналитики = "Организация" Тогда
					ПредставлениеИзмеренияВидаАналитики = НСтр("ru='Организации';uk='Організації'",КодЯзыка);
				ИначеЕсли ИзмерениеВидАналитики = "Подразделение" Тогда
					ПредставлениеИзмеренияВидаАналитики = НСтр("ru='Подразделения';uk='Підрозділи'",КодЯзыка);
				ИначеЕсли ИзмерениеВидАналитики = "Сценарий" Тогда
					ПредставлениеИзмеренияВидаАналитики = НСтр("ru='Сценарии';uk='Сценарії'",КодЯзыка);
				ИначеЕсли ИзмерениеВидАналитики = "Валюта" Тогда
					ПредставлениеИзмеренияВидаАналитики = НСтр("ru='Валюты';uk='Валюти'",КодЯзыка);
		...
		Если ИзмерениеВидАналитики = "" ИЛИ ЭтоНумерованнаяАналитика Тогда
			Если Режим = "Все" Тогда
				ПредставлениеПустогоЗначения = НСтр("ru='<все>';uk='<всі>'",КодЯзыка);
			ИначеЕсли Режим = "ДобавляемыеИПрочие" Тогда
				ПредставлениеПустогоЗначения = НСтр("ru='<добавляемые и прочие>';uk='<ті, що додаються і інші>'",КодЯзыка);
			Иначе
				ПредставлениеПустогоЗначения = НСтр("ru='<прочие>';uk='<інші>'",КодЯзыка);
			КонецЕсли;
		Иначе
			Если Режим = "Все" Тогда
				Шаблон = НСтр("ru='<все %1>';uk='<всі %1>'",КодЯзыка);
			ИначеЕсли Режим = "ДобавляемыеИПрочие" Тогда
				Шаблон = НСтр("ru='<добавляемые и прочие %1>';uk='<що додаються та інші %1>'",КодЯзыка);
			Иначе
				Шаблон = НСтр("ru='<прочие %1>';uk='<інші %1>'",КодЯзыка);
			КонецЕсли;
			ПредставлениеПустогоЗначения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Шаблон,
				НРег(ПредставлениеИзмеренияВидаАналитики));
		КонецЕсли;
		Если НЕ КэшПредставлений = Неопределено Тогда
...
Функция ПредставлениеИзмеренияБюджетирования(ИмяИзмерения, КодЯзыка = "") Экспорт
	...
	Если ИмяИзмерения = "Организация" Тогда
		ПредставлениеИзмерения = НСтр("ru='Организация';uk='Організація'",КодЯзыка);
	ИначеЕсли ИмяИзмерения = "Подразделение" Тогда
		ПредставлениеИзмерения = НСтр("ru='Подразделение';uk='Підрозділ'",КодЯзыка);
	ИначеЕсли ИмяИзмерения = "Сценарий" Тогда
		ПредставлениеИзмерения = НСтр("ru='Сценарий';uk='Сценарій'",КодЯзыка);
	ИначеЕсли ИмяИзмерения = "Валюта" Тогда
		ПредставлениеИзмерения = НСтр("ru='Валюта';uk='Валюта'",КодЯзыка);
	ИначеЕсли ИмяИзмерения = "Период" Тогда
		ПредставлениеИзмерения = НСтр("ru='Период';uk='Період'",КодЯзыка);
	ИначеЕсли ИмяИзмерения = "НомерПодПериода" Тогда
		ПредставлениеИзмерения = НСтр("ru='№ подпериода';uk='№ підперіоди'",КодЯзыка);
	ИначеЕсли ПредставлениеНумерованнойАналитики <> Неопределено Тогда
...
Функция ПеременныеПериодовБюджетирования(ИсключаяСкобки = Ложь, КодЯзыка = "") Экспорт
	...
	ЗначениеИПредставление.Вставить("Представление", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ИсключаяСкобки, "%1", "[%1]"),
			НСтр("ru='Начало периода данных';uk='Початок періоду даних'",КодЯзыка)));
	...
	ЗначениеИПредставление.Вставить("Представление", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ИсключаяСкобки, "%1", "[%1]"),
			НСтр("ru='Конец периода данных';uk='Кінець періоду даних'",КодЯзыка)));
	...
	ЗначениеИПредставление.Вставить("Представление", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ИсключаяСкобки, "%1", "[%1]"),
			НСтр("ru='Граница факт.данных';uk='Межа факт.даних'",КодЯзыка)));
	...
	ЗначениеИПредставление.Вставить("Представление", 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ИсключаяСкобки, "%1", "[%1]"),
			НСтр("ru='Период группировки';uk='Період групування'",КодЯзыка)));
...
//
// Возвращаемое значение:
//  Соответствие - Соответствие представлений переменных периодов и их значений.
...
//
// Возвращаемое значение:
//  Соответствие  - Соответствие значений переменных периодов и их представлений.
...
Функция ПеременныеБюджетныхОтчетов(ИсключаяСкобки = Ложь, КодЯзыка = "") Экспорт
	...
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Вид отчета';uk='Вид звіту'",КодЯзыка));
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Текущая дата и время';uk='Поточна дата і час'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Период отчетности';uk='Період звітності'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Конечная дата периода отчета';uk='Кінцева дата періоду звіту'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Организация';uk='Організація'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Подразделение';uk='Підрозділ'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Сценарий';uk='Сценарій'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Статус документа';uk='Статус документа'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
	...
	ЗначениеИПредставление = Новый Структура;
	ЗначениеИПредставление.Вставить("Имя", Имя);
	ЗначениеИПредставление.Вставить("Представление", НСтр("ru='Ответственный';uk='Відповідальний'",КодЯзыка));
	Результат.Вставить(ИмяПараметра, ЗначениеИПредставление);
...
Функция ПредставленияНумерованныхАналитик(КодЯзыка = "")
	...
	Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
		ПредставлениеНумерованнойАналитики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Аналитика%1';uk='Аналітика %1'",КодЯзыка), 
			НомерАналитики);
...
Функция ПредставлениеСмещения(Форма, ЗначениеПустое = Ложь) Экспорт
	...
	Иначе
		...
		Иначе
			Представление = НСтр("ru='Данные смещены на %1 вперед';uk='Дані зміщені на %1 вперед'");
			ПредставлениеПериод = БюджетированиеКлиентСервер.ПериодЦифрамиПериодичностьПрописью(-Период, Периодичность);
			ПредставлениеПериод = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Представление, ПредставлениеПериод);
		КонецЕсли; 
	КонецЕсли;
...
Функция ПреобразоватьРасшифровку(ОбъектИПараметрыРасчета, Расшифровка, РежимПоиска = Ложь, ИмяПоказателя = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из Расшифровка Цикл
		Если ЛеваяЧастьИмениСовпадает(КлючИЗначение.Ключ, "Период") Тогда
	...
	Если ИспользуемыеПериоды.Количество() Тогда
		...
		КолонкаПериод = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность);
