...
#Область СписаниеВПроизводство 
	...
	ТекстСписанияВПроизводство = СтрШаблон(ТекстСписанияВПроизводство, 
		НСтр("ru='Списание на расходы товаров';uk='Списання на витрати товарів'",ЯзыкСодержания));
    
#КонецОбласти
	
#Область ВыпускПродукции  
	ТекстВыпускПродукции = 
	"ВЫБРАТЬ //// Выпуск продукции (Дт 23 :: Кт 23)
	|
	|	Операция.Ссылка												КАК Ссылка,
	|	Операция.Дата												КАК Период,
	|	Операция.Организация										КАК Организация,
	...
	ТекстВыпускПродукции = СтрШаблон(ТекстВыпускПродукции, 
...
#Область ВыпускПродукцииНематериальныеЗатраты
	...
	// Выпуск собственной продукции, приход нематериальных затрат (Дт 23 :: Кт 23).
	"ВЫБРАТЬ
	...
	ТекстВыпускПродукцииНематериальныеЗатраты = СтрШаблон(ТекстВыпускПродукцииНематериальныеЗатраты, 
		НСтр("ru='Выпуск продукции';uk='Випуск продукції'",ЯзыкСодержания));
 
#КонецОбласти
	
#Область ЗакупкаУслуги 
	ЗакупкаУслуги = "
	...
	|
	|	МАКСИМУМ(
	|		ВЫБОР КОГДА Операция.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая)
	|		ТОГДА
	...
	ЗакупкаУслуги = СтрШаблон(ЗакупкаУслуги, 
		НСтр("ru='Поступление услуг';uk='Надходження послуг'",ЯзыкСодержания));
 
#КонецОбласти
	

#Область ТекстЗакупкаНДСОжидаемый
	...
	|	ЕСТЬNULL(СУММА(Суммы.СуммаНДСУпрМУ), МАКСИМУМ(Операция.СуммаНДС)/ КурсВалютыУпрУчета.Курс) КАК СуммаУУ,
	|
	...
	ТекстЗакупкаНДСОжидаемый = СтрШаблон(ТекстЗакупкаНДСОжидаемый, 
		НСтр("ru='НДС при приобретении';uk='ПДВ при придбанні'",ЯзыкСодержания));
 
#КонецОбласти

#Область СписаниеВПроизводствоМатериаловДавальца
	...
	ТекстСписанияВПроизводствоМатериаловДавальца = СтрШаблон(ТекстСписанияВПроизводствоМатериаловДавальца, 
...
#Область ТекстВыпускВозвратныхОтходов 
	...
	// Выпуск возвратных отходов (Дт 23 :: Кт 23).
	"ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстЗакупкаНДСОжидаемый);
