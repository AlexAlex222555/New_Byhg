...
Функция ПерезаполнитьНакладнуюПоОрдерам(Объект,НалогообложениеНДС) Экспорт
	...
	//++ НЕ УТ
	ИначеЕсли Объект.Свойство("ПередачаПоЗаказам") Тогда
		СтруктураПараметров.Вставить("ЗаполнятьНесобраннымиТоварами", Не Объект.ПередачаПоЗаказам);
	//-- НЕ УТ
	Иначе
...
Процедура СформироватьЗапросКонтрольЗавершенияРасчетов(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	РасчетыСКлиентамиОстатки.КОплатеОстаток КАК КОплатеОстаток
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентами.Остатки(, ОбъектРасчетов.Объект = &Ссылка) КАК РасчетыСКлиентамиОстатки
		|;
		|";
