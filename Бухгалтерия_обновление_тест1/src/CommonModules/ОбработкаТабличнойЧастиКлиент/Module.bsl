...
Функция НеобходимВызовСервера(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакЦеховаяКладовая", СтруктураПараметровДействия) Тогда
		...
		ИмяПоляСклад = "Склад";
		Если СтруктураПараметровДействия <> Неопределено И СтруктураПараметровДействия.Свойство("ИмяПоляСклад") Тогда
			ИмяПоляСклад = СтруктураПараметровДействия.ИмяПоляСклад;	
		КонецЕсли;	
			  
    	Если КэшированныеЗначения.СвойстваСкладов.Получить(ТекущаяСтрока[ИмяПоляСклад]) = Неопределено Тогда
			Возврат Истина;
    	КонецЕсли;
