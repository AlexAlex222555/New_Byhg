...
#Область ТекстЗадолженностьПередКомитентом
	...
	|	ЕСТЬNULL(ВидыЗапасовТоваров.Договор.ГруппаФинансовогоУчета, Операция.ГруппаФинансовогоУчета) КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	|	ЕСТЬNULL(ВидыЗапасовТоваров.Договор.Подразделение, Операция.Подразделение) КАК ПодразделениеДт,
	|	ЕСТЬNULL(ВидыЗапасовТоваров.Договор.НаправлениеДеятельности, Операция.НаправлениеДеятельности) КАК НаправлениеДеятельностиДт,
...
#Область ТекстНалоговыйКредитУсловнаяПродажа // (Дт 6441 :: Кт 68ОК, 6435)
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='НДС: налоговый кредит';uk='ПДВ: податковий кредит'",ЯзыкСодержания),НСтр("ru='Условная продажа';uk='Умовний продаж'",ЯзыкСодержания));
	ТекстыОтражения.Добавить(ТекстЗапроса);
