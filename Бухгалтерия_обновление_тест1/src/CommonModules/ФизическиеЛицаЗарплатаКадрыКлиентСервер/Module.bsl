...
Функция ПредставлениеВозраста(ДатаРождения, ДатаВозраста) Экспорт
	...
	Если Возраст >= 1 Тогда
		Возврат СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru=';%1 год;;%1 года;%1 лет;';uk='; %1 рік ;; %1 роки; %1 років;'"), Возраст);
	КонецЕсли;
	...
	Если Возраст = 0 Тогда
		Возврат НСтр("ru='Меньше месяца';uk='Менше місяця'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru=';%1 месяц;;%1 месяца;%1 месяцев;';uk=';%1 місяць ;;%1 місяці;%1 місяців;'"), Возраст);
