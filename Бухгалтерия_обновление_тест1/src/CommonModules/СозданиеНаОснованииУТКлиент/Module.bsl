...
Процедура СозданиеВыработкиСотрудников(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	Если ТипЗнч(МассивСсылок[0]) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		...
		Для Каждого Строка Из Массив Цикл
			...
			Если Не Реквизиты.Проведен Тогда
				ТекстОшибкиПроведен = НСтр("ru='Документ %Документ% не проведен. Ввод на основании непроведенного документа запрещен.';uk='Документ %Документ% не проведено. Введення на підставі непроведенного документа заборонене.'");
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Операция недоступна для выбранного объекта.';uk='Операція недоступна для вибраного об''єкта.'");
