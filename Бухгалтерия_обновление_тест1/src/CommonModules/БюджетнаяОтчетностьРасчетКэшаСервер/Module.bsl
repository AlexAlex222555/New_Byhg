...
Функция ДанныеОтчетаДляОформления(ПараметрыТаблицы, КэшДопРеквизиты) Экспорт
	...
	Для Каждого ЭлементОтчета Из МассивЭлементов Цикл
		...
		Если ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Период Тогда
			...
			ИмяИзмерения = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
...
// 
// Параметры:
// 	ИсточникОтбор - ГруппаЭлементовОтбораКомпоновкиДанных, Произвольный -
// 	ИмяЭлемента - Строка -
// 	НайденныйОтбор - СписокЗначений, Массив, Неопределено - Описание
// 	СУчетомВложенных - Булево - Описание
// Возвращаемое значение:
// 	Булево - Описание
...
Процедура ДобавитьИсточник(СтруктураКэша, Источники, ОтобраннаяСтрока, СтруктураКолонок, ДополнительныеПараметрыИсточника)
	...
	Если ЭлементОтчета.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
		ТипУпрСуммы = ДополнительныеПараметрыИсточника.ТипУпрСуммы;
		СтруктураИсточника.Вставить("ТипУпрСуммы", ТипУпрСуммы);
	КонецЕсли;
...
Функция СравнитьИсточники(СтруктураКэша, Источник, ОтобраннаяСтрока, СтруктураКолонок, ДополнительныеПараметрыИсточника)
	...
	ИначеЕсли ВидЭлементаСравнения = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов
		...
		Если ВидЭлементаСравнения = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
			ТипУпрСуммы = ДополнительныеПараметрыИсточника.ТипУпрСуммы;
			Если ТипУпрСуммы <> Источник.ТипУпрСуммы Тогда 
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ИсточникДокумент Тогда
		...
		ОтборЭлемента = ЗначениеРеквизитаЭлементаВидаБюджета(
				ЭлементОтчета, "ДополнительныйОтбор", КэшРеквизитов).Получить(); // НастройкиКомпоновкиДанных, КоллекцияЭлементовОтбораКомпоновкиДанных - 
		УбратьИзОтбораОпределенныеИзмерения(ОтобраннаяСтрока, ОтборЭлемента, СтруктураКэша);
...
Процедура ЗаполнитьИсточникиПоВидуЭлемента(Источники, СтруктураКэша, ОписаниеСтруктурыЭлементовБюджета, ВидЭлемента, ПараметрыЗаполненияИсточников)
	...
	Для Каждого ОтобраннаяСтрока Из ОтобранныеСтроки Цикл
		...
		Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
			ТекстЗапроса = "
			|ВЫБРАТЬ 
			|	ПравилаПолученияФактаПоПоказателямБюджетов.ТипУпрСуммы КАК ТипУпрСуммы
			|ИЗ
			|	Справочник.ПравилаПолученияФактаПоПоказателямБюджетов КАК ПравилаПолученияФактаПоПоказателямБюджетов
			|ГДЕ
			|   ПравилаПолученияФактаПоПоказателямБюджетов.ПоказательБюджетов = &ПоказательБюджетов
			|";
			Запрос = Новый Запрос(ТекстЗапроса);
			Запрос.УстановитьПараметр("ПоказательБюджетов", ЭлементИсточника);
			Выборка = Запрос.Выполнить().Выбрать();
			ТипУпрСуммы = -1;
			Если Выборка.Следующий() Тогда
				ТипУпрСуммы = Выборка.ТипУпрСуммы;
			КонецЕсли;       
			ДополнительныеПараметрыИсточника.Вставить("ТипУпрСуммы", ТипУпрСуммы);
        КонецЕсли;
			...
			Если НЕ КорневойЭлемент = Неопределено Тогда
				...
				Если ЯчейкаСложнойТаблицы = Неопределено Тогда
					...
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не удалось получить ячейку сложной таблицы по элементу отчета %1.';uk='Не вдалося отримати комірку складної таблиці по елементу звіту %1.'",КодЯзыка),
						ОписаниеСтруктурыЭлементовБюджета.ВидБюджета);
					ЗаписьЖурналаРегистрации(
						НСтр("ru='Получение описания вида бюджета';uk='Отримання опису виду бюджету'",КодЯзыка),
...
Функция ВыражениеПравилаПериодаГруппировки(ИмяПоля, ЭлементОтчета, СтруктураКолонок, КэшРеквизитов)
	...
	ПеременныеПериодов = БюджетнаяОтчетностьКлиентСервер.ПеременныеПериодовБюджетирования();
	ВыражениеЗаменыПериодаГруппировки = "ЛеваяТаблицаПериодов.Период"
		+ ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность);
	...
	Пока ЗначениеЗаполнено(Элемент) Цикл
		...
		ИначеЕсли ВидЭлементаСравнения = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение Тогда
			...
			ИначеЕсли ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Период Тогда
				...
				ИмяПериода = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Период);
				НоваяСтрока["ФлагПериод" + ИмяПериода] = "заполнить";
				
				ЗначениеПериода = ФинансоваяОтчетностьСервер.ЗначениеДополнительногоРеквизита(Элемент, "ЗначениеПериода", КэшДопРеквизитов);
				Если ЗначениеПериода = Неопределено Тогда
					НоваяСтрока["ВыражениеПериода" + ИмяПериода] = -1;
				Иначе
					НоваяСтрока["ВыражениеПериода" + ИмяПериода] = ЗначениеПериода;
				КонецЕсли;
				...
				Для Сч = НайденныйИндекс + 1 По УпорядоченныеПериодичности.ВГраница() Цикл
					...
					ИмяПериода = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Период);
					КолонкаПериод = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Период);
					Если НоваяСтрока.Владелец().Колонки.Найти(КолонкаПериод) <> Неопределено Тогда
						Если НоваяСтрока["Флаг" + КолонкаПериод] <> "заполнить" Тогда
							НоваяСтрока["Флаг" + КолонкаПериод] = "заполнить";
							НоваяСтрока["ВыражениеПериода" + ИмяПериода] = -1;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
	...
	ИначеЕсли Не ЗначениеЗаполнено(Элемент) Тогда // расчет по подчиненным строкам
		...
		Для Каждого Строка Из ПодчиненныеСтроки Цикл
			...
			СтрокаОперанда.ИмяОперанда = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вложенная строка #%1';uk='Вкладений рядок # %1'"),
...
Функция ПолучитьИмяИзмерения(КэшДопРеквизиты, Элемент)
	...
	ИначеЕсли Элемент.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение Тогда
		...
		ИначеЕсли ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Период Тогда
			...
			Возврат "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
...
Функция НовыйСтруктураЯчеек(ОписаниеОтчета, МассивОсновныхКолонок = Неопределено, МассивКолонокТаблицыИсточников = Неопределено)
	...
	Для Каждого Измерение Из ОписаниеОтчета.Периодичности Цикл
		...
		СтруктураЯчеек.Колонки.Добавить("Период" + ИмяПериода, ТипКолонкиСтруктурыЯчеек("Период"), ИмяПериода);
		СтруктураЯчеек.Колонки.Добавить("ФлагПериод" + ИмяПериода, ТипКолонкиСтруктурыЯчеек("Флаг"));
		СтруктураЯчеек.Колонки.Добавить("ВыражениеПериода" + ИмяПериода, ТипКолонкиСтруктурыЯчеек("Количество"));
		МассивОсновныхКолонок.Добавить("ВыражениеПериода" + ИмяПериода);
		МассивОсновныхКолонок.Добавить("Период" + ИмяПериода);
		МассивОсновныхКолонок.Добавить("ФлагПериод" + ИмяПериода);
	КонецЦикла;
...
Процедура ДобавитьРесурсыИВычисляемыеПоля(СКД, СтруктураКолонок, ПоляИсточников, Остатки)
	...
	Если СтруктураКолонок <> Неопределено Тогда
		...
		Для Каждого Периодичность Из СтруктураКолонок.Периодичности Цикл
			Периодичности.Добавить("Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность));
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьСКДИтоговыеДанныеОстатки(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных)
	...
	Для Каждого СтрокаПериодичности Из Периодичности Цикл
		...
		ИмяПериода     = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Измерение);
		КолонкаПериода = "Период" + ИмяПериода;
		...
		Для Сч = 0 По ВГраница Цикл
			...
			Если СтруктураКолонокТаблицыДанных.Периодичности.Найти(Измерение) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			КолонкаПериода = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Измерение);
	...
	Если СтруктураКолонокТаблицыДанных.Периодичности.Количество() Тогда
		...
		Порядок.Поле = Новый ПолеКомпоновкиДанных("Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность));
...
Функция ПолучитьСКДИтоговыеДанныеОбороты(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, ЕстьХранимыеПравила, ЕстьПривилегированныйРежим)
	...
	Для Каждого СтрокаПериодичности Из Периодичности Цикл
		...
		ИмяПериода    = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Измерение);
		КолонкаПериод = "Период" + ИмяПериода;
		...
		Для Сч = 0 По ВГраница Цикл
			...
			Если СтруктураКолонокТаблицыДанных.Периодичности.Найти(Измерение) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			КолонкаПериод = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Измерение);
...
Процедура ДобавитьКолонкиКоличестваВКолонки(Таблица, ПараметрыУчетаПоКолонкам, СтрокаДобавления = Неопределено)
	...
	Если Не СтрокаДобавления.Строки.Количество() Тогда
		...
		Если ЕстьКоличество Тогда
			...
			НоваяСтрока.НаименованиеДляПечати = НСтр("ru='Количество';uk='Кількість'");
		КонецЕсли;
		
		Если ЕстьЗначение Тогда
			НоваяСтрока = СтрокаДобавления.Строки.Добавить();
			НоваяСтрока.ИмяИзмерения = "#Показатель_Значение";
			НоваяСтрока.НаименованиеДляПечати = НСтр("ru='Значение';uk='Значення'");
		КонецЕсли;
		...
		Если ДобавитьСумму Тогда
			...
			НоваяСтрока.НаименованиеДляПечати = НСтр("ru='Сумма';uk='Сума'");
		КонецЕсли;
...
Функция ТекстНесовместимыхЭлементов()
	...
	Возврат НСтр("ru='В подчиненных строках есть несовместимые элементы';uk='У підпорядкованих рядках є несумісні елементи'");
...
Функция ДоступныеДляСтрокиБюджетаКолонкиСпискаВвода(СведенияОБюджетеДляРасчета, ЭлементОтчета, ВышележащиеЭлементы)
	...
	Если ЭлементОтчета.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение Тогда
		...
		Если ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Период Тогда
			...
			ЭлементыТекущегоУровня.НастройкиКолонок.Вставить("Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность),
...
Функция РассчитатьКолонкиСпискаВвода(СведенияОБюджетеДляРасчета, ЭлементыДерева, СоответствиеРезультат, ВышележащиеЭлементы = Неопределено)
	...
	Для Каждого КлючИЗначение Из СоответствиеУровня Цикл
		...
		Если МинимальнаяПериодичность <> Неопределено Тогда
			Измерения = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность);
		КонецЕсли;
