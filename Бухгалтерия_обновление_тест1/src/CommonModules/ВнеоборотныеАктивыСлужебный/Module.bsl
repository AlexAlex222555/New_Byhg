...
Процедура ПроверитьВозможностьПринятияКУчетуОС(Объект, Отказ) Экспорт
	...
	МестонахождениеВТабличнойЧасти = Ложь;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.УжеПринятоКУчету Тогда
			Если ЗначениеЗаполнено(Выборка.ДокументСнятияСУчета) Тогда
				ТекстСообщения = НСтр("ru='Основное средство ""%1"" было снято с учета в организации %2. Повторное принятие к учету в ту же организацию не поддерживается.';uk='Основний засіб "" %1"" було знято з обліку в організації %2. Повторне прийняття до обліку в ту ж організацію не підтримується.'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Представление, Объект.Организация);
			ИначеЕсли ЭтоВводОстатков Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='По основному средству ""%1"" уже введены остатки.';uk='За основним засобом "" %1"" вже введені залишки.'"), Выборка.Представление);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Основное средство ""%1"" уже принято к учету.';uk='Основний засіб ""%1"" вже прийнято до обліку.'"), Выборка.Представление);
			КонецЕсли; 
...
Процедура ПроверитьРеквизитыОСПриПринятииКУчету(Объект, ИмяТЧ, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НеЗаполненИнвентарныйНомер Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='В карточке основного средства ""%1"" необходимо заполнить инвентарный номер.';uk='У картці основного засобу ""%1"" необхідно заповнити інвентарний номер.'"), Выборка.Представление);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.УжеПринятоКУчету Тогда
			...
			Если ЗначениеЗаполнено(Выборка.ДокументСписания) Тогда
				ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" был списан в организации %2. Повторное принятие к учету в ту же организацию не поддерживается.';uk='Нематеріальний актив "" 1"" був списаний в організації %2. Повторне прийняття до обліку в ту ж організацію не підтримується.'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Представление, Объект.Организация);
			ИначеЕсли ЭтоВводОстатков Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru='По нематериальному активу ""%1"" уже введены остатки.';uk='За нематеріальним активом "" %1"" вже введені залишки.'"), Выборка.Представление);
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Нематериальный актив ""%1"" уже принят к учету.';uk='Нематеріальний актив ""%1"" вже прийнято до обліку.'"), Выборка.Представление);
			КонецЕсли;
...
Процедура ПроверитьЧтоОСПринятыКУчету(Объект, Отказ) Экспорт
	...
	ИначеЕсли ТипДокумента <> Тип("ДокументОбъект.ИзменениеПараметровОС2_4") Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СостоянияПринят.Найти(Выборка.СостояниеБУ) <> Неопределено
			...
			Если Выборка.МестонахождениеОрганизация <> Объект.Организация Тогда
				...
				ТекстСообщения = НСтр("ru='Основное средство ""%1"" числится в другой организации ""%2"".';uk='Основний засіб ""%1"" числиться в іншій організації ""%2"".'");
			...
			Если ИмяРеквизитаПодразделение <> Неопределено
				...
				ЕстьОшибкиМестонахождения = Истина;
				ТекстСообщения = НСтр("ru='Основное средство ""%1"" числится в другом подразделении ""%2"".';uk='Основний засіб ""%1"" числиться в іншому підрозділі ""%2"".'");
			...
			Если ЕстьРеквизитМОЛ 
				...
				Если ЗначениеЗаполнено(Выборка.МестонахождениеМОЛ) Тогда
					ТекстСообщения = НСтр("ru='Основное средство ""%1"" числится за другим МОЛ ""%2"".';uk='Основний засіб ""%1"" числиться за іншим МВО ""%2"".'");
					ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Представление, Выборка.МестонахождениеМОЛПредставление);
				Иначе
					ТекстСообщения = НСтр("ru='Основное средство ""%1"" не числится за МОЛ.';uk='Основний засіб ""%1"" не числиться за МВО.'");
		...
		Если НЕ ЕстьОшибкиМестонахождения Тогда
			...
			Если ДоступенВыборОтраженияВУчетах Тогда
				...
				Если СостоянияПринят.Найти(Выборка.СостояниеБУ) = Неопределено
					...
					ТекстСообщения = НСтр("ru='Основное средство ""%1"" не принято к учету на %2 в регламентированном учете.';uk='Основний засіб ""%1"" не прийнято до обліку на %2 в регламентованому обліку.'");
				...
				ИначеЕсли СостоянияПринят.Найти(Выборка.СостояниеУУ) = Неопределено
					...
					ТекстСообщения = НСтр("ru='Основное средство ""%1"" не принято к учету на %2 в управленческом учете.';uk='Основний засіб ""%1"" не прийнято до обліку на %2 в управлінському обліку.'");
					
				Иначе
					ТекстСообщения = НСтр("ru='Основное средство ""%1"" не принято к учету на %2.';uk='Основний засіб ""%1"" не прийнято до обліку на %2.'");
				КонецЕсли; 
...
// 	Отказ - Булево -
Процедура ПроверитьЧтоОСПолученыВАренду(Объект, Отказ) Экспорт
	...
	ТекстЗапроса = ВнеоборотныеАктивыЛокализация.ТекстЗапросаПроверитьЧтоОСПолученыВАренду();
	Если ТекстЗапроса = Неопределено Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru='Основное средство ""%1"" не поступало в аренду на %2.';uk='Основний засіб ""%1"" не надходило в оренду на %2.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Представление, Формат(Объект.Дата, "ДЛФ=D"));
...
Процедура ПроверитьЧтоНМАПринятыКУчету(Объект, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СостоянияПринят.Найти(Выборка.СостояниеУУ) = Неопределено
			...
			ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" не принят к учету на %2.';uk='Нематеріальний актив ""%1"" не прийнятий до обліку на %2.'");
		...
		ИначеЕсли Объект.ОтражатьВРеглУчете
			...
			ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" не принят к учету на %2 в регламентированном учете.';uk='Нематеріальний актив ""%1"" не прийнятий до обліку на %2 в регламентованому обліку.'");
		...
		ИначеЕсли Объект.ОтражатьВУпрУчете
			...
			ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" не принят к учету на %2 в управленческом учете.';uk='Нематеріальний актив ""%1"" не прийнятий до обліку на %2 в управлінському обліку.'");
		...
		Иначе
			...
			Если Выборка.Организация <> Объект.Организация
				...
				ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" принят к учету в другой организации %2.';uk='Нематеріальний актив ""%1"" прийнято до обліку в іншій організації %2.'");
			...
			Если Выборка.Подразделение <> Объект.Подразделение
				...
				ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" принят к учету в другое подразделение %2.';uk='Нематеріальний актив ""%1"" прийнято до обліку в інший підрозділ %2.'");
...
Функция СообщитьЕслиОСНеПринятоКУчету(ОсновноеСредство, Дата) Экспорт
	...
	Если ПервоначальныеСведения.СостояниеБУ = Перечисления.СостоянияОС.НеПринятоКУчету
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Основное средство ""%1"" не принято к учету.';uk='Основний засіб ""%1"" не прийнято до обліку.'"), Строка(ОсновноеСредство));
	...
	ИначеЕсли ПервоначальныеСведения.СостояниеБУ = Перечисления.СостоянияОС.СнятоСУчета
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Основное средство ""%1"" снято с учета.';uk='Основний засіб ""%1"" знято з обліку.'"), Строка(ОсновноеСредство));
		ВызватьИсключение ТекстСообщения;
...
Функция СообщитьЕслиНМАНеПринятКУчету(НематериальныйАктив, Дата) Экспорт
	...
	Если ПервоначальныеСведения.СостояниеБУ = Перечисления.СостоянияНМА.НеПринятКУчету
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Нематериальный актив ""%1"" не принят к учету.';uk='Нематеріальний актив ""%1"" не прийнятий до обліку.'"), Строка(НематериальныйАктив));
		ВызватьИсключение ТекстСообщения;
	...
	ИначеЕсли ПервоначальныеСведения.СостояниеБУ = Перечисления.СостоянияНМА.Списан
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Нематериальный актив ""%1"" снят с учета.';uk='Нематеріальний актив ""%1"" знято з обліку.'"), Строка(НематериальныйАктив));
		ВызватьИсключение ТекстСообщения;
...
// 
// Параметры:
// 	ПараметрыРеквизитовОбъекта - Массив из см. ВнеоборотныеАктивыКлиентСервер.НовыйПараметрРеквизитаОбъекта - Параметры реквизитов.
...
Функция ТекстРаспределенияСуммы(ИмяПоля, ИмяТаблицы = Неопределено) Экспорт

	Если ИмяТаблицы = Неопределено Тогда
		ИмяТаблицы = "СпособыОтраженияРасходов";
	КонецЕсли; 
	
	ТекстЗапроса =	
	"	((ВЫРАЗИТЬ(&СуммаПереопределяемый
	|				* (&БазаНакопленнаяПереопределяемый + &БазаПереопределяемый) 
	|					/ &СуммаБазыПереопределяемый КАК ЧИСЛО(31,2)))
	|			- (ВЫРАЗИТЬ(&СуммаПереопределяемый * &БазаНакопленнаяПереопределяемый / &СуммаБазыПереопределяемый КАК ЧИСЛО(31,2))))";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&БазаНакопленнаяПереопределяемый", ИмяТаблицы + ".БазаНакопленная");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СуммаБазыПереопределяемый", ИмяТаблицы + ".СуммаБазы");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&БазаПереопределяемый", ИмяТаблицы + ".База");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СуммаПереопределяемый", ИмяПоля);
	
	Возврат ТекстЗапроса;

КонецФункции

// Формирует текст запроса для формирования движений в регистре "ПартииПрочихРасходов" на основнии расчета амортизации.
// 
// Параметры:
...
Процедура ТекстЗапросаПараметрыРасчетаАмортизации(ИмяРегистра, ИмяТаблицы, ДобавитьТекстДляДоначисления, ОбъектУчета, СписокЗапросов, ВычислениеПолей = Неопределено) Экспорт
	...
	Если ДобавитьТекстДляДоначисления Тогда
		...
		Если ОбъектУчета = "ОсновноеСредство" Тогда
			...
			|				И ТаблицаРегистра.ОсновноеСредство = ОбъединениеОС.ОсновноеСредство
...
Процедура ТекстЗапросаТаблицаВтПервоначальныеСведенияОС(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	|	ПервоначальныеСведенияОС.Партнер КАК Партнер,
...
Процедура ТекстЗапросаТаблицаВтПорядокУчетаНМАБУ(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	|						СписокНМА.НематериальныйАктив
	|					ИЗ
	|						втСписокНМА КАК СписокНМА)) КАК ПорядокУчетаНМАБУ
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура ТекстЗапросаТаблицаВтСписокНМА(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	//++ Локализация
	Если ВРег(ИмяДокумента) = ВРег("Документ.ПринятиеКУчетуНМА") 
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.ПереоценкаНМА")
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.СписаниеНМА")
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.ПеремещениеНМА")
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.МодернизацияНМА")
...
Процедура ЗарегистрироватьПредупреждениеВыполненияРасчета(ЭтапРасчета, Период, Организация, ОписаниеПредупреждения, ГруппаПредупреждений = "", Объект = Неопределено, Отказ = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ГруппаПредупреждений) Тогда
		ГруппаПредупреждений = НСтр("ru='При выполнении операции возникли предупреждения';uk='При виконанні операції виникли попередження'",ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ИнициализироватьХозяйственныеОперацииИДокументыВнеоборотныхАктивов(ХозяйственныеОперацииИДокументы) Экспорт
	...
	Строка.ЗаголовокРабочегоМеста	 	= НСтр("ru='внутренние потребления товаров';uk='внутрішні споживання товарів'");
	...
	Строка.ЗаголовокРабочегоМеста	 	= НСтр("ru='приобретение услуг и прочих активов';uk='придбання послуг і інших активів'");
	Строка.ДобавитьКнопкуСоздать        = Истина;
...
Процедура УстановитьЗначенияПараметровПроведенияПриПеремещении(Реквизиты, ЗначенияПараметровПроведения) Экспорт
	...
	ШаблонСтроки = НСтр("ru='Перемещение из ""%1""';uk='Переміщення з ""%1""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	ШаблонСтроки = НСтр("ru='Перемещение в ""%1""';uk='Переміщення в ""%1""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Если ЗначениеЗаполнено(Реквизиты.ПодразделениеПолучатель) Тогда
...
|;uk='Не вдалося виконати обробник оновлення об''єкта метаданих ""%1"".
...
Функция ЗаголовокНадписиОДокументеВДругомУчете(РеквизитыДокумента)
	...
	ТекстПринятУУ = СтрШаблон(НСтр("ru='В управленческом учете создан документ %1';uk='В управлінському обліку створений документ %1'"), ПредставлениеДокумента);
	ТекстПринятБУ = СтрШаблон(НСтр("ru='В регламентированном учете создан документ %1';uk='В регламентованому обліку створений документ %1'"), ПредставлениеДокумента);
	...
	ПредставлениеЗаказа = СтрШаблон(НСтр("ru='№%1 от %2';uk='№%1 від %2'"), 
	...
	Если НЕ РеквизитыДокумента.Проведен Тогда
		...
		ЗаголовокНадписи.Добавить(Новый ФорматированнаяСтрока(
									НСтр("ru='не проведен';uk='не проведений'"),, ЦветаСтиля.ЦветОсобогоТекста));
	КонецЕсли;
...
Функция НайтиДокументВДругомУчетеИСформироватьЗаголовок(Объект, ПоказатьИнформациюЕслиНеСоздан)
	...
	ПредставлениеДокумента = ПредставлениеДокумента(МетаданныеДокумента.Синоним);
	ТекстНеПринятУУ = СтрШаблон(НСтр("ru='В управленческом учете не создан документ %1';uk='В управлінському обліку не створений документ %1'"), ПредставлениеДокумента);
	ТекстНеПринятБУ = СтрШаблон(НСтр("ru='В регламентированном учете не создан документ %1';uk='У регламентованому обліку не створений документ %1'"), ПредставлениеДокумента);
	ТекстОформить = НСтр("ru='Создать';uk='Створити'");
...
Функция ПредставлениеДокумента(СинонимДокумента)
	...
	Если СтрЗаканчиваетсяНа(СинонимДокумента, НСтр("ru='ОС';uk='ОЗ'")) Тогда
		ПредставлениеДокумента = СокрЛП(Лев(СинонимДокумента, СтрДлина(СинонимДокумента) - 2));
	ИначеЕсли СтрЗаканчиваетсяНа(СинонимДокумента, НСтр("ru='НМА';uk='НМА'")) Тогда
