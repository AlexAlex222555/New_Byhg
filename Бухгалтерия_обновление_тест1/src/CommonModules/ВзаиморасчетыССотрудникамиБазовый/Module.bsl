...
Функция ПечатьПлатежнаяВедомость(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаШапок.Следующий() Цикл
		...
		//++ НЕ БЗК
		ВыборкаСчетов=ВзаиморасчетыССотрудниками.ВедомостьДанныеДляПечатиСчетаУчетов(ВыборкаШапок.Ссылка);
		ВыборкаСчетов.Свернуть("СчетУчета");
		ВыборкаСчетов.Сортировать("СчетУчета Возр");
		СтрокаСчетУчета="";
		Для каждого СчетУчета из ВыборкаСчетов Цикл
			СтрокаСчетУчета=СтрокаСчетУчета+?(ВыборкаСчетов.Индекс(СчетУчета)=0,СчетУчета.СчетУчета.Код,", "+СчетУчета.СчетУчета.Код);
		КонецЦикла;
		ОбластьМакетаШапкаДокумента.Параметры.Счет=?(ЗначениеЗаполнено(СтрокаСчетУчета),СтрокаСчетУчета,"");
		//-- НЕ БЗК
