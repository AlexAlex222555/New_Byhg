...
Функция ТекстОтраженияВРеглУчетеУКР() Экспорт
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	ВЫБОР КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров)
	|		ТОГДА ""%1""
	|		ИНАЧЕ ""%2""
	...
	|ГДЕ
	|	УчетнаяПолитикаОрганизаций.СборкаТоваровЧерезСчет23
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Списание собственных комплектующих на сборку';uk='Списання власних комплектуючих на збирання'",ЯзыкСодержания),
		НСтр("ru='Списание комплекта для разборки';uk='Списання комплекту для розбирання'",ЯзыкСодержания));
...
#КонецОбласти
	...
	#Область ТекстСписаниеНаРасходыКомиссионныхКомплектующих   // (Дт  :: Кт 0221, 0241)
	ТекстЗапроса =
...
#КонецОбласти
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Оприходование собранного комплекта';uk='Оприбуткування зібраного комплекту'",ЯзыкСодержания),
		НСтр("ru='Оприходование комплектующей';uk='Оприбуткування комплектуючої'",ЯзыкСодержания));
...
#КонецОбласти
	...
	|			И (Строки.РазделУчета = Стоимости.РазделУчета)
	|ГДЕ
 	|	НЕ УчетнаяПолитикаОрганизаций.СборкаТоваровЧерезСчет23
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Сборка товаров';uk='Збирання товарів'",ЯзыкСодержания));
...
#КонецОбласти
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)
	|	КОНЕЦ КАК ВидСчетаКт,
	|	ВЫБОР КОГДА ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) ССЫЛКА Справочник.Партнеры
	|				ИЛИ ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) ССЫЛКА Справочник.Организации ТОГДА
	|		ЕСТЬNULL(Стоимости.ГруппаФинансовогоУчетаРасчетов, СписываемаяНоменклатура.ГруппаФинансовогоУчетаРасчетов)
	|	ИНАЧЕ
	|		ЕСТЬNULL(Стоимости.ГруппаФинансовогоУчета, СписываемаяНоменклатура.КорГруппаФинансовогоУчета)
	|	КОНЕЦ КАК АналитикаУчетаКт,
	|	ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) КАК МестоУчетаКт,
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	ВЫБОР КОГДА ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) ССЫЛКА Справочник.Партнеры
	|				ИЛИ ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) ССЫЛКА Справочник.Организации ТОГДА
	|		ЕСТЬNULL(Стоимости.ПодразделениеАналитики, СписываемаяНоменклатура.КорПодразделениеАналитики)
	|	ИНАЧЕ
	...
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,
	|	НЕОПРЕДЕЛЕНО КАК СчетКт,
	|	ЕСТЬNULL(Стоимости.Номенклатура, СписываемаяНоменклатура.КорНоменклатура) КАК СубконтоКт1,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) ССЫЛКА Справочник.Партнеры
	|				ИЛИ ЕСТЬNULL(Стоимости.Склад, СписываемаяНоменклатура.КорСклад) ССЫЛКА Справочник.Организации
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Разборка товаров';uk='Розбирання товарів'",ЯзыкСодержания));
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР() Экспорт
	...
	|	Строки.ГруппаФинансовогоУчетаРасчетов КАК ГруппаФинансовогоУчетаРасчетов,
	|	Строки.Склад КАК Склад,
	|	Строки.КорСклад КАК КорСклад,
	|	Строки.КорГруппаФинансовогоУчета КАК КорГруппаФинансовогоУчета,
	...
	|	Строки.АналитикаДоходов КАК АналитикаДоходов,
	|	Строки.СтатьяДоходов КАК СтатьяДоходов,
	...
	|	Строки.КорГруппаПродукции КАК КорГруппаПродукции,
	|	Строки.КорПодразделениеАналитики КАК КорПодразделениеАналитики,
	...
	|			И (ЕСТЬNULL(ВидыЗапасовСписание.АналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) = Строки.НаправлениеДеятельности)
	...
	|	Строки.КорПодразделениеАналитики,
	|	СУММА(СтрокиСКоличеством.Количество)
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСтроки КАК Строки
	|		ПО (Строки.Ссылка = ДокументыКОтражению.Ссылка)
	...
	|			И (ЕСТЬNULL(ВидыЗапасовОприходование.АналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) = Строки.НаправлениеДеятельности)
	|		ЛЕВОЕ СОЕДИНЕНИЕ УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
	|		ПО ДокументыКОтражению.Ссылка = УчетнаяПолитикаОрганизаций.Ссылка
	|			И ДокументыКОтражению.Период = УчетнаяПолитикаОрганизаций.Период
	|ГДЕ
