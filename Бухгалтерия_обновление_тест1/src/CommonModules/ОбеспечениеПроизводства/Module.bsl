...
Функция ТекстЗапросаПараметрыОбеспечения(ИмяТаблицыМатериалы, ИмяПоляСпособПолученияМатериала = Неопределено, ИмяПоляПодразделение = Неопределено, ВариантЗаполненияОбеспечения = Неопределено) Экспорт
	...
	Иначе
		...
		//-- Устарело_Производство21
			...
			|		
			|		КОГДА НЕ ПереопределениеОбеспечения1.ВариантОбеспечения ЕСТЬ NULL
			|			И ПереопределениеОбеспечения1.ВариантОбеспечения <> НЕОПРЕДЕЛЕНО ТОГДА
			|
			|			ПереопределениеОбеспечения1.ВариантОбеспечения
			|		
			|		КОГДА НЕ ПереопределениеОбеспечения2.ВариантОбеспечения ЕСТЬ NULL
			|			И ПереопределениеОбеспечения2.ВариантОбеспечения <> НЕОПРЕДЕЛЕНО ТОГДА
			|
			|			ПереопределениеОбеспечения2.ВариантОбеспечения
			|		
		...
		Иначе // По настройкам передачи в производство
			...
			|
			|			ПереопределениеОбеспечения.ВариантОбеспечения
			|
			|		КОГДА МатериалыИУслуги.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара) ТОГДА
			|			
			|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)
