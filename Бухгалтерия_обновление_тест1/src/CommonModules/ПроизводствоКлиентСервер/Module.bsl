...
Функция РасшифровкаВходящегоОстатка(Параметры) Экспорт
	...
	Если Параметры.ВходящийОстаток = 0
		...
		Возврат НСтр("ru='Движения номенклатуры не обнаружено.';uk='Рухи номенклатури не виявлені.'");
	Иначе
		ШаблонРасшифровки = НСтр("ru='Входящий остаток (<b>%1</b> %2) =';uk='Вхідний залишок (<b>%1</b> %2) ='");
	КонецЕсли;
	
	
	Если Не Параметры.НачальныйОстаток = 0 Тогда
		ШаблонРасшифровки = ШаблонРасшифровки + " " + НСтр("ru='остаток на начало месяца (<b>%3</b> %2)';uk='залишок на початок місяця (<b>%3</b> %2)'");
	КонецЕсли;
	
	Если Не Параметры.Поступило = 0 Тогда
		ШаблонРасшифровки = ШаблонРасшифровки + ?(Параметры.НачальныйОстаток = 0, " ", " + ")
												+ НСтр("ru='поступило (<b>%4</b> %2)';uk='надійшло (<b>%4</b> %2)'");
	КонецЕсли;
	...
	Если Не Параметры.Передано = 0 Тогда
		ШаблонРасшифровки = ШаблонРасшифровки + ?(Параметры.НачальныйОстаток = 0 И Параметры.Поступило = 0, " ", " - ")
												+ НСтр("ru='передано (<b>%5</b> %2)';uk='передано (<b>%5</b> %2)'");
	КонецЕсли;
...
//  НастройкиЗаполнения - Соответствие         - Ключ:     Имя проверяемого реквизита ТЧ
...
Функция СвернутьКолонкуВСтроку(Знач Массив, Знач КоличествоВыводимыхЭлементов = 1) Экспорт
	...
	Если Массив.Количество() > 0 Тогда
		...
		Возврат СтрСоединить(СписокДляВывода, ", ") + ?(Дополнение > 0, " " + СтрШаблон(НСтр("ru='и еще %1';uk='і ще %1'"), Дополнение), "");
...
Функция ЗаголовокТабличнойЧастиПоТипуПроцесса(ТипПроизводственногоПроцесса, КодЯзыка = "") Экспорт
	...
	ИначеЕсли ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка") Тогда
		Заголовок = НСтр("ru='Разбираемые изделия';uk='Розбирані вироби'",КодЯзыка);
