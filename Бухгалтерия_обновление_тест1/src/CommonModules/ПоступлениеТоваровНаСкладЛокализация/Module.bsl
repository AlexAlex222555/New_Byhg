...
Функция ТекстОтраженияВРеглУчетеУКР()
	...
	#Область ПоступлениеЦенностей  // (Дт 26, 28 :: Кт *.НП)
	ЗакупкаЦенности = "
	|ВЫБРАТЬ // Поступление товаров на склад или цеховую кладовую (Дт 26, 28 :: Кт *.НП)
	|
	...
	ЗакупкаЦенности = СтрШаблон(ЗакупкаЦенности, 
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР()
	...
	|	ВЫБОР
	|		КОГДА Себестоимость.ХозяйственнаяОперация =ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка) 
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
