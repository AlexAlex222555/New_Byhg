...
Процедура ОбработкаПроверкиЗаполненияОтборов(ОтчетОбъект, Отказ) Экспорт
	...
	Для каждого Отбор Из ОтборыВерхнегоУровня Цикл
		...
		ТекстСообщения = НСтр("ru='Поле с реквизитами субконто ""%1"" нельзя использовать в групповом отборе с иными полями.';uk='Поле з реквізитами субконто ""%1"" не можна використовувати в груповому відборі з іншими полями.'");
...
Функция ТипыСвязанныеСОрганизацией() Экспорт
	...
	//++ НЕ УТ
	Результат = НастройкаСчетовУчета.ТипыСвязанныеСОрганизацией();
	//-- НЕ УТ
	Возврат Результат;
...
Процедура СохранитьТипФайлаТабличногоДокументаПоУмолчанию(ТипФайла) Экспорт
	...
	ОписаниеНастройки.Представление = НСтр("ru='Значение по-умолчанию для формата в диалоге выбора файла при сохранении отчета';uk='Значення за замовчуванням для формату в діалозі вибору файлу при збереженні звіту'");
...
Процедура ОбработкаПроверкиЗаполнения(ОтчетОбъект, Отказ, Проверки = Неопределено) Экспорт
	...
	Если Проверки.Свойство("СписокВидовСубконто") И Проверки.СписокВидовСубконто Тогда
		...
		Если ОтчетОбъект.СписокВидовСубконто.Количество() > МаксКоличествоСубконто Тогда
			ТекстСообщения = НСтр("ru='Выбрано слишком много видов субконто, максимально допустимо %1';uk='Обрано занадто багато видів субконто, максимально допустиме %1'");
	...
	Если ПоляТекущейРасшифровки.Количество() > 0
		...
		ИЛИ ПоляТекущейРасшифровки.Количество() = 1) Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеПоля) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЗначениеПоля)) Тогда
				ВозможныеРасшифровки.Добавить(ПоляТекущейРасшифровки[0].Значение, СтрШаблон(НСтр("ru='Открыть ""%1""';uk='Відкрити ""%1""'"), ПоляТекущейРасшифровки[0].Значение));
			КонецЕсли;
...
Функция ОписаниеОтборовОтчета(ПараметрыОтчета, КомпоновщикНастроек)
	...
	ТекстПодразделение = "";
	Если ПараметрыОтчета.Свойство("Подразделение") И ЗначениеЗаполнено(ПараметрыОтчета.Подразделение) Тогда
		ТекстПодразделение = СтрШаблон(НСтр("ru='Подразделение Равно ""%1""';uk='Підрозділ Дорівнює ""%1""'"), ПараметрыОтчета.Подразделение);
	КонецЕсли;
...
Процедура ДополнитьТекстОтбора(ОсновнойТекстОтбор, ДополнительныйТекстОтбор)
	...
	Если Не ПустаяСтрока(ОсновнойТекстОтбор) И Не ПустаяСтрока(ДополнительныйТекстОтбор) Тогда
		ОсновнойТекстОтбор = ДополнительныйТекстОтбор + " " + НСтр("ru='И';uk='І'") + " " + ОсновнойТекстОтбор;
