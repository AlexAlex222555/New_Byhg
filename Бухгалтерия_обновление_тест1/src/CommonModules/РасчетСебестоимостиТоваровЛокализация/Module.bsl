...
#Область ТекстПогрешностьРасчетаСебестоимостиТовары
	...
	|	ВЫБОР
	|		КОГДА Строки.Склад ССЫЛКА Справочник.Партнеры
	|			ТОГДА ЕСТЬNULL(Строки.Договор.ГруппаФинансовогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	...
	|	ВЫБОР
	|		КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.СобственныеТоварыВПути) ИЛИ Строки.Склад ССЫЛКА Справочник.Партнеры
...
#Область ТекстПогрешностьРасчетаСебестоимостиПереработка
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Погрешность расчета себестоимости 
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Строки.Организация КАК Организация,
...
#Область ТекстПогрешностьРасчетаРезервов
	...
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеКт,	
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДругиеЗатратыОперационнойДеятельности) КАК СчетКт,
	|
...
#КонецОбласти
	...
	ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстЗапроса,
		НСтр("ru='Погрешность расчета себестоимости (товары)';uk='Похибка розрахунку собівартості (товари)'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru='Погрешность расчета себестоимости (переработка)';uk='Похибка розрахунку собівартості (переробка)'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru='Погрешность расчета себестоимости (НЗП)';uk='Похибка розрахунку собівартості (НЗВ)'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru='Погрешность расчета себестоимости (резервы)';uk='Похибка розрахунку собівартості (резерви)'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru='Погрешность расчета себестоимости (забаланс)';uk='Похибка розрахунку собівартості (забаланс)'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР() Экспорт
	...
	|		КОГДА Движения.АналитикаУчетаНоменклатуры.МестоХранения ССЫЛКА Справочник.Партнеры
	|			ТОГДА Движения.ВидЗапасов.Договор
