...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
    ...
    ВнеоборотныеАктивы.ДобавитьЭтап_НачислениеАмортизацииОС(ТаблицаЭтапов, ТекущийРодитель);
    ВнеоборотныеАктивы.ДобавитьЭтап_НачислениеАмортизацииНМА(ТаблицаЭтапов, ТекущийРодитель);
    ВнеоборотныеАктивы.ДобавитьЭтап_ФормированиеДвиженийПоВнеоборотнымАктивам(ТаблицаЭтапов,ТекущийРодитель);
	...
	//++ НЕ УТ
	УчетНДСУП.ДобавитьЭтап_КорректировкаНалоговогоНазначенияКапитальныхИнвестиций(ТаблицаЭтапов,ТекущийРодитель);
	//-- НЕ УТ
    //++ НЕ УТ
    ВнеоборотныеАктивы.ДобавитьЭтап_РасчетСтоимостиВнеоборотныхАктивов(ТаблицаЭтапов, ТекущийРодитель);
    //-- НЕ УТ
...
Процедура Использование_ПереходНаНовыеРежимыРаботы(ПараметрыОбработчика) Экспорт
	...
	Если ПроизводствоСервер.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство21 Тогда
		...
		Если ПроизводствоСервер.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство22 Тогда
			ТекстПояснения = НСтр("ru='Завершить переход на управление производством версии 2.5 и выключить функциональную опцию ""Производство (версия 2.1)""';uk='Завершити перехід на управління виробництвом версії 2.5 і вимкнути функціональну опцію ""Виробництво (версія 2.1)""'",ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			ТекстПояснения = НСтр("ru='Производство (версия 2.5)';uk='Виробництво (версія 2.5)'",ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
//
//
//			НСтр("ru='Учет необоротных активов (версия 2.5)'", ОбщегоНазначения.КодОсновногоЯзыка()),
//
//			ТекстГиперссылкиПоУмолчанию,
...
|по яких ще є залишки в регістрі ""Товари організацій"" і які повинні бути списані при витрачанні.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Цены возвратных отходов в документе ""Отчет переработчика""';uk='Ціни зворотних відходів в документі ""Звіт переробника""'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Цена должна быть заполнена.';uk='Ціна повинна бути заповнена.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
