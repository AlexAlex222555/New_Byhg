...
//  * ПараметрыСмещенияПериода - Структура - структура параметров смещения
...
Функция ПараметрыВыбораСпецификаций() Экспорт
	...
	// В случае заполнения - структура с ключами: Календарь, ПолеДлительность
	ПараметрыВыбора.Вставить("ПараметрыСмещенияПериода", Неопределено);
...
// 
// Возвращаемое значение:
//  Массив из см. СоздатьОписаниеФункцииКонструктораФормул - описание функций внешних модулей.
//
Функция ОписаниеФункцийКонструктораФормул() Экспорт
	...
	Описание = СоздатьОписаниеФункцииКонструктораФормул(
					"ВыборПоПорогу",,
					НСтр("ru='Выбор по порогу';uk='Вибір за порогом'"),
					НСтр("ru='ВыборПоПорогу';uk='ВиборПоПорогу'"),
					НСтр("ru='ВыборПоПорогу(,,,)';uk='ВиборПоПорогу (,,,)'"));
...
Процедура ЗаполнитьСпособПолученияМатериалаПоУмолчанию(ДанныеСтроки) Экспорт
	...
	СтруктураЗначений.Вставить("СпособПолученияМатериалаРедактирование", НСтр("ru='Обеспечивать';uk='Забезпечувати'"));
...
Функция ПредставлениеЕдиницыИзмеренияПартииВыпуска(ОписаниеПартииВыпуска = Неопределено, Количество = 0) Экспорт
	...
	Если ОписаниеПартииВыпуска <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ЕдиницаИзмеренияПартииВыпуска) Тогда
			Если ИспользуетсяОписаниеПартииВыпуска И ОписаниеПартииВыпуска.ВыпускПроизвольнымиПорциями Тогда
				Результат = НСтр("ru='ед. хранения';uk='од. зберігання'");
			Иначе
				Результат = ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
								Количество,
								НСтр("ru='партия';uk='партія'"),
								НСтр("ru='партии';uk='партії'"),
								НСтр("ru='партий';uk='партій'"),
								"ж");
			КонецЕсли;
		Иначе
	...
	Иначе
		...
		Возврат НСтр("ru='единиц/партий изделий';uk='одиниць/партій виробів'");
	КонецЕсли;
...
Функция ПредставлениеСтатусаСпецификации(Статус, НачалоДействия = '00010101', КонецДействия = '00010101', СтрокаСтатус = "") Экспорт
	...
	Если Статус = ПредопределенноеЗначение("Перечисление.СтатусыСпецификаций.Действует") Тогда
		Если НачалоДействия <> '00010101' И КонецДействия <> '00010101' Тогда
			СтрокаПериодДействия = " " + СтрШаблон(НСтр("ru='с %1 по %2';uk='з %1 по %2'"), Формат(НачалоДействия, "ДЛФ=D"), Формат(КонецДействия, "ДЛФ=D"));
		ИначеЕсли НачалоДействия <> '00010101' Тогда
			СтрокаПериодДействия = " " + СтрШаблон(НСтр("ru='с %1';uk='з %1'"), Формат(НачалоДействия, "ДЛФ=D"));
		ИначеЕсли КонецДействия <> '00010101'  Тогда
			СтрокаПериодДействия = " " + СтрШаблон(НСтр("ru='по %1';uk='по %1'"), Формат(КонецДействия, "ДЛФ=D"));
		КонецЕсли;
	КонецЕсли;
...
Функция СписокУсловийОтбораПоСвойствамНоменклатуры()
	...
	Условие = Новый Структура(Реквизиты, "Равно",             НСтр("ru='Равно';uk='Дорівнює'"),               Ложь,   "%1 = %2");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "НеРавно",           НСтр("ru='Не равно';uk='Не дорівнює'"),            Ложь,   "%1 <> %2");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "Больше",            НСтр("ru='Больше';uk='Більше'"),              Истина, "%1 > %2");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "БольшеИлиРавно",    НСтр("ru='Больше или равно';uk='Більше або дорівнює'"),    Истина, "%1 >= %2");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "Меньше",            НСтр("ru='Меньше';uk='Менше'"),              Истина, "%1 < %2");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "МеньшеИлиРавно",    НСтр("ru='Меньше или равно';uk='Менше або дорівнює'"),    Истина, "%1 <= %2");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "ВСписке",           НСтр("ru='В списке';uk='У списку'"),            Ложь,   "%1 В (%2)");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "НеВСписке",         НСтр("ru='Не в списке';uk='Не в списку'"),         Ложь,   "НЕ %1 В (%2)");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "ВИнтервале",        НСтр("ru='В интервале (>,<)';uk='В інтервалі (>, <)'"),   Истина, "%1 > %2 И %1 < %3");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "ВИнтервалеВключая", НСтр("ru='В интервале (>=,<=)';uk='В інтервалі (> =, <=)'"), Истина, "%1 >= %2 И %1 <= %3");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "Заполнено",         НСтр("ru='Заполнено';uk='Заповнено'"),           Ложь,   "НЕ %1 В (НЕОПРЕДЕЛЕНО, ЛОЖЬ, 0)");
	Результат.Добавить(Условие);
	Условие = Новый Структура(Реквизиты, "НеЗаполнено",       НСтр("ru='Не заполнено';uk='Не заповнено'"),        Ложь,   "%1 В (НЕОПРЕДЕЛЕНО, ЛОЖЬ, 0)");
	Результат.Добавить(Условие);
