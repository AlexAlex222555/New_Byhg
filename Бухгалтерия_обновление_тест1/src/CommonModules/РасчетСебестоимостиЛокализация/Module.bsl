...
Процедура РаспределениеПартийНДС(ПараметрыРасчета, СтрокаСхемыРасчета, НомерЭтапа) Экспорт
	...
	ИначеЕсли НомерЭтапа = 2 Тогда
		...
		// Формирует движения по регистрам:
        // - ДетализацияПартийТоваровДляНДС2_4
        РасчетСебестоимостиНДС.ПодготовкаДанныхДляУчетаНДС2_4ПоПартиямПрочихРасходов(ПараметрыРасчета);
		//-- НЕ УТ
		...
		//++ НЕ УТ
        // Этап 13
		// Формирует документы КорректировкаНалоговогоНазначенияКапитальныхИнвестиций с движениями по регистрам:
        // - ПартииПрочихРасходов
        // - ПрочиеАктивыПассивы
        // - ПрочиеРасходы.
		РасчетСебестоимостиНДС.КорректировкаНалоговогоНазначенияКапитальныхИнвестиций(ПараметрыРасчета);
		//-- НЕ УТ
...
Процедура РаспределениеТрудозатрат(ПараметрыРасчета) Экспорт
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		ОстаткиТрудозатратНаПодразделениях                          = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ОстаткиТрудозатратНаПодразделениях");
		ВыпускиБезЗаказов                                           = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВыпускиБезЗаказов");
		ПриходыБезЗаказов                                           = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ПриходыБезЗаказов");
		...
		ДанныеДоРасчетаЦепочек                                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "Данные");
	КонецЕсли;
...
Процедура РаспределениеТрудозатратНаВыпуск(ПараметрыРасчета) Экспорт
	...
	//++ Локализация
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "РаспределениеТрудозатратНаВыпускУП21УП22");
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		...
		втРаспределенныеТрудозатраты                                = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "втРаспределенныеТрудозатраты");
		Продукция                                                   = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "Продукция");
		ДанныеДоРасчетаЦепочек                                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "Данные");
	КонецЕсли;
...
Функция ОписаниеЦепочекРаспределенияТрудозатрат(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
        Перечисления.ТипыЗаписейПартий.Потребление, 	 		"Организация, Подразделение, ВидРабот");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.Потребление,
        Перечисления.ТипыЗаписейПартий.ОстатокБезЗаказа, 		"Организация, Подразделение, ВидРабот");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.Потребление,
        Перечисления.ТипыЗаписейПартий.ПартияБезЗаказа,  		"Организация, Подразделение, ВидРабот");
	
	// ПотреблениеАвто <- (Распределение)
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеПриемника(ОписаниеЦепочек,
        Перечисления.ТипыЗаписейПартий.ПотреблениеАвто,			"Организация, Подразделение, ВидРабот");
	РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.ПотреблениеАвто,
        Перечисления.ТипыЗаписейПартий.Распределение, 	 		"Организация, Подразделение, ВидРабот");
...
// Вт ОстаткиТрудозатратНаПодразделениях, ВыпускиБезЗаказов, ПриходыБезЗаказов, КРаспределению.
// 
...
Функция ОписаниеДвиженийРаспределенияТрудозатратНаВыпуск(ПараметрыРасчета)
	...
	ОписаниеДвижений = РасчетСебестоимостиПрикладныеАлгоритмы.ОписаниеДвижений();
	ОписаниеДвижений.Вставить("Контекст", "РаспределениеТрудозатратНаВыпускУстаревшее");
	...
	ОписаниеДвижений.Вставить("Показатели",	"Количество, НормативнаяСтоимость, Стоимость, СтоимостьРегл");
	ОписаниеДвижений.Вставить("БазисПрихода", "Знаменатель");
	ОписаниеДвижений.Вставить("БазисРасхода", "Знаменатель");
	ОписаниеДвижений.Вставить("КлючРасхода", "ДокументИсточник");
	ОписаниеДвижений.Вставить("ПолеПорядка", "Период");
...
Функция ТекстТрудозатратыВыпускиБезЗаказов2_2()
	Возврат
		...
		|	ЕСТЬNULL(РаботыДляДавальца.РазделУчета, ДД.РазделУчета) КАК КорРазделУчета,
		|	ЕСТЬNULL(РаботыДляДавальца.ВидЗапасов, ДД.ВидЗапасов) КАК КорВидЗапасовПродукции,
		|	ВЫБОР КОГДА ДД.Организация В (&ОрганизацииСФИФОСкользящая)
		|		ТОГДА ЕСТЬNULL(РаботыДляДавальца.АналитикаУчетаПартий, ДД.АналитикаУчетаПартий)
		|	ИНАЧЕ
		|		НЕОПРЕДЕЛЕНО
...
Функция ВосстановитьДвиженияДокументовПоРегиструНакопления(Регистратор) Экспорт
	...
	Если ЭтоРегламентнаяОперация
	 //++ НЕ УТ
     ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.КорректировкаНалоговогоНазначенияКапитальныхИнвестиций") 
	 //-- НЕ УТ
	 Тогда
...
Процедура ДополнитьТипыДокументовОСиНМА(ТипыРегистраторов) Экспорт
	...
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.ИзменениеПараметровНМА"));
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.КорректировкаНалоговогоНазначенияОС"));
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.МодернизацияОС"));
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.МодернизацияНМА"));
	...
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.ПеремещениеНМА"));
	...
	ТипыРегистраторов.Добавить(Тип("ДокументСсылка.РемонтОС"));
	//-- Локализация
КонецПроцедуры

Процедура ОбработатьДокументОтчетПереработчикаПередВосстановлениемДвижений(Регистратор) Экспорт
	...
	Если ДокументОбъект <> Неопределено Тогда             
		...
		Если ДокументОбъект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки Тогда
			ДокументОбъект.Услуги.Очистить();
			ДанныеСтроки = ДокументОбъект.Услуги.Добавить();
			ЗаполнитьЗначенияСвойств(ДанныеСтроки, ДокументОбъект);
			Если ДокументОбъект.Продукция.Количество() = 1 Тогда
				ДанныеСтроки.НомерГруппыЗатрат = ДокументОбъект.Продукция[0].НомерГруппыЗатрат;
			КонецЕсли;
			ДокументОбъект.МаксимальныйНомерГруппыЗатрат = ДанныеСтроки.НомерГруппыЗатрат;
		КонецЕсли;   
		
		Если Не ДокументОбъект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства Тогда
			Документы.ОтчетПереработчика.СформироватьПартииПроизводства(ДокументОбъект);
		КонецЕсли;
...
Процедура ТекстЗапросаДляРасчетаЭтапа(ИмяЭтапа, ПараметрыРасчета, ТекстЗапроса) Экспорт
	...
	ИначеЕсли ИмяЭтапа = "РаспределениеТрудозатратНаВыпускУстаревшее" Тогда
...
Процедура ЗарегистрироватьКОтражениюВРегламентированномУчете(ПараметрыРасчета)
	...
	РезультатЗапроса = РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина, "ДокументыКОтражению", Истина);
