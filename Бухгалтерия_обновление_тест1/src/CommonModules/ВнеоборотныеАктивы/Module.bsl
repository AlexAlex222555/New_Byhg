...
Функция ЗапуститьРасчетАмортизацииВФоне(ПараметрыРасчета, УникальныйИдентификатор) Экспорт
	...
	РезультатРасчета = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
						...
						ПараметрыРасчета,
						НСтр("ru='Расчет амортизации';uk='Розрахунок амортизації'"));
	...
	Если ПараметрыРасчета.ФактическаяСтоимость Тогда
		...
		Для каждого СтрокаДокумента Из ПараметрыРасчета.ТаблицаОбъектовУчета Цикл			
			...
			Для Сч = 0 По ДанныеПоАналитке.РасходыПоОбъектуУчета.Количество()-1 Цикл
				...
				РасходыПоОбъектуУчета.ПостояннаяРазница = 0;
				РасходыПоОбъектуУчета.ВременнаяРазница = 0;
...
Процедура СформироватьЗаданияКРасчетуАмортизацииОС(ДокументСсылка, ДанныеТаблиц)
	...
	ПорядокНачисленияПоСтандартамРегл = 
		(Константы.ПорядокУчетаВНАВУпрУчете.Получить() = Перечисления.ПорядокУчетаВНАВУпрУчете.ПоСтандартамРегл);
	...
	ЗарегистрироватьДокумент = 
		ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаСтоимостиИАмортизацииОС")
		И НЕ ПорядокНачисленияПоСтандартамРегл;
...
Процедура СформироватьЗаданияКРасчетуАмортизацииНМА(ДокументСсылка, ДанныеТаблиц)
	...
	// Поэтому задание формируется всегда.
	ЗарегистрироватьДокумент = 
		ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаСтоимостиИАмортизацииНМА")
		И НЕ ПорядокНачисленияПоСтандартамРегл;
...
Процедура СформироватьЗаданияКРасчетуСтоимостиВНА(ДанныеТаблиц) Экспорт
	...
	ОпределитьИзмененияВРегистреПрочиеРасходы(ДанныеТаблиц);
...
Процедура ДобавитьПредупреждениеЕслиСведенияНеАктуальны(СведенияАктуальны, ТабличныйДокумент) Экспорт
	...
	ОбластьПримечания.Текст = НСтр("ru='Стоимость и амортизация не актуальны, необходимо выполнить закрытие месяца (это сообщение не выводится на печать)';uk='Вартість і амортизація не актуальні, необхідно виконати закриття місяця (це повідомлення не виводиться на друк)'");
...
Функция НайтиОбъектыЭксплуатацииПоШтрихкодам(ДанныеШтрихкодов, ПараметрыПодбора = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Объект эксплуатации с инвентарным номером %1 не найден';uk='Об''єкт експлуатації з інвентарним номером %1 не знайдений'");
	Для каждого Штрихкод Из Результат.НеНайдены Цикл
		ТекстСообщения = СтрШаблон(ШаблонСообщения, Штрихкод);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения); 
	КонецЦикла; 
	
	ШаблонСообщения = НСтр("ru='Объект эксплуатации ""%1"" не может быть добавлен, т.к. он не удовлетворяет условиям выбора';uk='Об''єкт експлуатації ""%1"" не може бути доданий, тому що він не задовільняє умовам вибору'");
...
Функция НайтиОсновныеСредстваПоШтрихкодам(ДанныеШтрихкодов, ПараметрыПодбора = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Основное средство с инвентарным номером %1 не найдено';uk='Основний засіб з інвентарним номером %1 не знайдений'");
	Для каждого Штрихкод Из Результат.НеНайдены Цикл
		ТекстСообщения = СтрШаблон(ШаблонСообщения, Штрихкод);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения); 
	КонецЦикла; 
	
	ШаблонСообщения = НСтр("ru='Основное средство ""%1"" не может быть добавлено в документ, т.к. оно не удовлетворяет условиям выбора';uk='Основний засіб "" %1"" не може бути додано в документ, тому що він не задовольняє умовам вибору'");
	Для каждого СсылкаНаОбъект Из Результат.НеПодходят Цикл
		ТекстСообщения = СтрШаблон(ШаблонСообщения, СсылкаНаОбъект);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, СсылкаНаОбъект); 
	КонецЦикла; 
...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru='Количество одновременно выполняемых заданий расчета амортизации';uk='Кількість одночасно виконуваних завдань розрахунку амортизації'");
...
|Количество заданий рекомендуется настраивать в зависимости от конфигурации сервера СУБД и серверов BAF.'
|;uk='Розрахунок амортизації в закритті місяця може виконуватися одночасно відразу за кількома пакетам амортизації. 
|Збільшення кількості завдань може зменшити час розрахунку амортизації. 
|Кількість завдань рекомендується налаштовувати в залежності від конфігурації сервера СУБД і серверів BAF.'");
...
Процедура ПроверитьСоответствиеДатыВерсииУчета(Объект, ЭтоДокументВерсии2_4, Отказ) Экспорт
	...
	Если ИспользуетсяУправлениеВНА_2_4 И НЕ ЭтоДокументВерсии2_4 Тогда
		...
		ТекстСообщения = НСтр("ru='Дата документа должна быть до %1 (начало ведения учета необоротных активов версии 2.5).
|После этой даты необходимо использовать документы, расположенные в разделе ""Необоротные активы"".'
|;uk='Дата документа повинна бути до %1 (початок ведення обліку необоротних активів версії 2.5).
|Після цієї дати необхідно використовувати документи, розташовані в розділі ""Необоротні активи"".'");
	...
	ИначеЕсли НЕ ИспользуетсяУправлениеВНА_2_4 И ЭтоДокументВерсии2_4  Тогда
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ИзменениеПараметровОС2_4")
			...
			Если НачалоДня(Объект.Дата) < НачалоДня(ДатаНачалаУчета-1) Тогда
				...
				ТекстСообщения = НСтр("ru='Если документ оформлен для перехода на учет версии 2.5, то его дата должна быть %1.
|В остальных случаях дата документа должна быть после %2 (начало ведения учета необоротных активов версии 2.5).
|До этой даты необходимо использовать документы, расположенные в разделе ""Регламентированный учет"".'
|;uk='Якщо документ оформлений для переходу на облік версії 2.5, то його дата повинна бути %1. 
|В інших випадках дата документа повинна бути після %2 (початок ведення обліку необоротних активів версії 2.5). 
|До цієї дати необхідно використовувати документи, розташовані в розділі ""Регламентований облік"".'");
				ТекстСообщения = СтрШаблон(ТекстСообщения, Формат(ДатаНачалаУчета-1, "ДЛФ=D"), Формат(ДатаНачалаУчета, "ДЛФ=D"));
			КонецЕсли; 
			
		Иначе	
			
			ТекстСообщения = НСтр("ru='Дата документа должна быть после %1 (начало ведения учета необоротных активов версии 2.5).
|До этой даты необходимо использовать документы, расположенные в разделе ""Регламентированный учет"".'
|;uk='Дата документа повинна бути після %1 (початок ведення обліку необоротних активів версії 2.5).
|До цієї дати необхідно використовувати документи, розташовані в розділі ""Регламентований облік"".'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Формат(ДатаНачалаУчета, "ДЛФ=D"));
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ОписаниеВарианта = ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "");
	ОписаниеВарианта.Включен = НЕ ВедетсяРегламентированныйУчетВНА;
	ОписаниеВарианта.Описание = НСтр("ru='Какие основные средства числятся в организации?';uk='Які основні засоби числяться в організації?'");
	#КонецОбласти
...
Процедура ПроверитьОтсутствиеДублейВТабличнойЧасти(Объект, ИмяТабличнойЧасти, ИмяРеквизита, Отказ, ШаблонСообщения = Неопределено) Экспорт
	...
	Если ШаблонСообщения = Неопределено Тогда
		Если ИмяТабличнойЧасти = "ОС" Тогда
			ШаблонСообщения = НСтр("ru='Основное средство ""%1"" уже выбрано в строке %2.';uk='Основний засіб ""%1"" вже вибрано в рядку %2.'");
		...
		Иначе
			ШаблонСообщения = НСтр("ru='Значение ""%1"" уже выбрано в строке %2.';uk='Значення ""%1"" вже вибрано в рядку %2.'");
		КонецЕсли;
	КонецЕсли; 
...
Процедура СформироватьДвиженияПриОбновленииИБ(ИмяРегистра, Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "РегистрСведений." + ИмяРегистра;
	...
	|	ЕСТЬNULL(РеестрДокументов.ДатаДокументаИБ, ДАТАВРЕМЯ(1, 1, 1)) КАК Период
	|ИЗ
	|	ВТДляОбработки КАК ВТДляОбработки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО РеестрДокументов.Ссылка = ВТДляОбработки.Регистратор
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Регистратор, "Проведен") = Истина Тогда
				...
				ТаблицыДляДвижений = МенеджерДокумента.ДанныеДокументаДляПроведения(Выборка.Регистратор, ИмяРегистра);
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось обработать записи регистра ""%1"": %2 по причине: %3';uk='Не вдалося обробити записи регістру ""%1"": %2 по причині: %3'"),
			...
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение, Выборка.Регистратор.Метаданные(), Выборка.Регистратор, ТекстСообщения);
...
Процедура ДобавитьЭтап_ЗаполнениеФактическойСтоимостиВНАПриОтключенномУчетеСебестоимости(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.ТекстВыполнить     = НСтр("ru='Заполнить';uk='Заповнити'");
...
Процедура Использование_ЗаполнениеФактическойСтоимостиВНАПриОтключенномУчетеСебестоимости(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4() Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Учет необоротных активов отключен.';uk='Облік необоротних активів відключений.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетСебестоимости") 
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='При включенном учете себестоимости заполнение стоимости не требуется.';uk='При включеному обліку собівартості заповнення вартості не потрібне.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если Обработки.ЗаполнениеФактическойСтоимостиВНАПриОтключенномУчетеСебестоимости.ЕстьДокументыКОбработке() Тогда
		...
		ТекстСообщения = НСтр("ru='Обнаружены документы, в которых стоимость, указанная в документе, отличается от фактической.';uk='Виявлено документи, в яких вартість, зазначена в документі, відрізняється від фактичної.'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет документов, в которых требуется заполнить стоимость.';uk='Немає документів, в яких потрібно заповнити вартість.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура Использование_ФормированиеДвиженийПоВнеоборотнымАктивам(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ИспользуетсяУправлениеВНА_2_4(ПараметрыОбработчика.ПараметрыРасчета.Период) Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			ПараметрыОбработчика,
			НСтр("ru='Учет необоротных активов версии 2.5 не используется.';uk='Облік необоротних активів версії 2.5 не використовується.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если НЕ ВыполненоОтложенноеФормированиеДвиженийПоВНА(ПараметрыОбработчика) Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет документов по которым требуется сформировать движения.';uk='Немає документів за якими потрібно сформувати рухи.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли; 
...
Процедура Использование_РасчетСтоимостиВнеоборотныхАктивов(ПараметрыОбработчика) Экспорт
	...
	Если НЕ РасчетСебестоимостиПовтИсп.ФормироватьДвиженияПоРегистрамСебестоимости(ПараметрыОбработчика.ПараметрыРасчета.Период) Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			ПараметрыОбработчика,
			НСтр("ru='Учет себестоимости отключен, расчет стоимости недоступен.';uk='Облік собівартості відключений, розрахунок вартості недоступний.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если НЕ ВыполненРасчетСтоимостиВнеоборотныхАктивов(ПараметрыОбработчика) Тогда
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет документов по которым требуется расчет стоимости.';uk='Немає документів за якими потрібно провести розрахунок вартості.'",ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
...
Функция ХозяйственныеОперацииПоКоторымРассчитываетсяСтоимость() Экспорт
	...
	Операции.Добавить(Перечисления.ХозяйственныеОперации.МодернизацияНМА);
...
Функция УсловияПереходаНаУчет2_4() Экспорт
	...
	Если НЕ РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22() Тогда
		...
		УсловияВключенияУчет2_4 = УсловияВключенияУчет2_4 + Символы.ПС
			+ НСтр("ru='- установить опции ""Партионный учет"" значение ""Версия 2.1""
|(раздел ""НСИ и администрирование"" - ""Финансовый результат и контроллинг"")'
|;uk='- встановити опції ""Партіонний облік"" значення ""Версія 2.1""
|(розділ ""НСІ та адміністрування"" - ""Фінансовий результат і контролінг"")'");
	Иначе
		...
		Если ЗначениеЗаполнено(ДатаОпции) И ДатаОпции > МинимальнаяДатаНачалаУчета2_4 Тогда
			...
			ШаблонСообщения = НСтр("ru='Начало учета должно быть не раньше даты начала применения партионного учета версии 2.1: %1';uk='Початок обліку має бути не раніше дати початку застосування партійного обліку версії 2.1: %1'");
...
|(раздел ""НСИ и администрирование"" - ""Финансовый результат и контроллинг"")'
|;uk='- включити опцію ""Інші доходи і витрати в валюті регл. обліку"" 
|(розділ ""НДІ і адміністрування"" - ""Фінансовий результат і контролінг"")'");
	КонецЕсли;
...
|;uk='- включити опцію ""Управлінський облік за правилами міжнародного фінансового обліку"" 
|(розділ ""НДІ і адміністрування"" - ""Фінансовий результат і контролінг"" - ""Доп. настройки обліку собівартості"")'");
	Иначе
		ДатаОпции = РасчетСебестоимостиПовтИсп.ДатаНачалаВеденияУправленческогоУчетаОрганизаций();
		Если ЗначениеЗаполнено(ДатаОпции) И ДатаОпции > МинимальнаяДатаНачалаУчета2_4 Тогда
			МинимальнаяДатаНачалаУчета2_4 = ДатаОпции;
			ШаблонСообщения = НСтр("ru='Начало учета должно быть не раньше даты начала ведения управленческого учета по правилам международного финансового учета: %1';uk='Початок обліку має бути не раніше дати початку ведення управлінського обліку за правилами міжнародного фінансового обліку: %1'");
			КомментарийМинДатаНачалаУчета2_4 = СтрШаблон(ШаблонСообщения, Формат(МинимальнаяДатаНачалаУчета2_4, "ДЛФ=D"));
		КонецЕсли; 
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(УсловияВключенияУчет2_4) Тогда
		КомментарийУчет2_4 = НСтр("ru='Для включения возможности учета версии 2.5 необходимо:';uk='Для включення можливості обліку версії 2.5 необхідно:'") + УсловияВключенияУчет2_4;
	КонецЕсли; 
...
Функция СоздатьОчередьЗаданий(ПараметрыРасчета, ИзмененныеПакеты)
	...
	Для каждого ПакетГоловнойОрганизации Из ПакетыАмортизацииПоГоловнымОрганизациям Цикл
		...
		Если ПакетГоловнойОрганизации.ЕстьОбособленныеПодразделения Тогда
			НаименованиеФоновогоЗадания = НСтр("ru='Расчет амортизации по организации ""%1"" и ее филиалам.';uk='Розрахунок амортизації по організації ""%1"" та її філіям.'");
		Иначе
			НаименованиеФоновогоЗадания = НСтр("ru='Расчет амортизации по организации ""%1"".';uk='Розрахунок амортизації по організації ""%1"".'");
		КонецЕсли; 
...
|;uk='У основного засобу %1 попередня сума витрат (регл. облік) %2 (%3) не повинна перевищувати фактичну %4 (%5).
|Необхідно змінити попередню суму витрат в документі %6.'");
...
|Необходимо изменить предварительную сумму затрат в документе %6.'
|;uk='У основного засобу %1 попередня сума витрат (упр. облік) %2 (%3) не повинна перевищувати фактичну %4 (%5). 
|Необхідно змінити попередню суму витрат в документі %6.'");
...
|;uk='У основного засобу %1 відсутні фактичні витрати на модернізацію (регл. облік).
|Необхідно перевірити документи, якими були відображені витрати.'");
...
|;uk='У основного засобу %1 відсутні фактичні витрати на модернізацію (упр. облік).
|Необхідно перевірити заповнення в документах, якими були відображені витрати.'");
	Иначе
...
|;uk='У документі для основного засобу %1 вказана вартість (регл. облік) %2 (%3), що перевищує фактичну %4 (%5). 
|Необхідно змінити вартість в документі %6.'");
...
|Необходимо изменить стоимость в документе %6.'
|;uk='У документі для основного засобу %1 вказана вартість (упр. облік) %2 (%3), що перевищує фактичну %4 (%5).
|Необхідно змінити вартість в документі %6.'");
...
|Необходимо проверить документы, которыми были отражены затраты.'
|;uk='За основним засобом %1 відсутні фактичні витрати (регл. облік), пов''язані з його придбанням.
|Необхідно перевірити документи, якими були відображені витрати.'");
...
|Необходимо проверить документы, которыми были отражены затраты.'
|;uk='За основним засобом %1 відсутні фактичні витрати (упр. облік), пов''язані з його придбанням.
|Необхідно перевірити документи, якими були відображені витрати.'");
...
|;uk='У нематеріального активу %1 попередня вартість (регл. облік) %2 (%3) не повинна перевищувати фактичну %4 (%5).
|Необхідно змінити попередню вартість в документі %6.'");
...
|Необходимо изменить предварительную стоимость в документе %6.'
|;uk='У нематеріального активу %1 попередня вартість (упр. облік) %2 (%3) не повинна перевищувати фактичну %4 (%5).
|Необхідно змінити попередню вартість в документі %6.'");
...
|Необходимо проверить документы, которыми были отражены затраты.'
|;uk='По необоротному активу %1 відсутні фактичні витрати (регл. облік), пов''язані з його придбанням. 
|Необхідно перевірити документи, якими були відображені витрати.'");
...
|Необходимо проверить документы, которыми были отражены затраты.'
|;uk='По необоротному активу %1 відсутні фактичні витрати (упр. облік), пов''язані з його придбанням.
|Необхідно перевірити документи, якими були відображені витрати.'");
...
Функция ОтложенноеФормированиеДвижений(ПараметрыВыполнения, МассивСсылок, Отказ = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЗагрузитьТаблицыДвижений(ТаблицыДляДвижений, ДокументОбъект.Движения, Истина);
			УправленческийУчетПроведениеСервер.ОбновитьДвиженияАктивовПассивов(Выборка.Ссылка, ДокументОбъект.Движения);
...
|%2'
|;uk='Не вдалося сформувати руху по документу %1, через:
|%2'");
...
Процедура ПроверитьОтложенноеФормированиеДвижений(ПараметрыВыполнения, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ОписаниеПроблемы = НСтр("ru='В документе Реализация услуг и прочих активов №%1 от %2 для необоротного актива %3 необходимо указать статью расходов, на которую будет списана стоимость.';uk='У документі Реалізація послуг та інших активів №%1 від %2 для необоротного активу %3 необхідно зазначити статтю витрат, на яку буде списана вартість.'");
...
Процедура ОпределитьИзмененияВРегистреПрочиеРасходы(ДанныеТаблиц)
...
Функция ИдентификаторыДокументовПоКоторымФормируютсяОтложенныеДвижения() Экспорт
	...
	ВнеоборотныеАктивыЛокализация.ДополнитьСписокТиповДокументовПоКоторымФормируютсяОтложенныеДвижения(СписокТипов);
...
Процедура ЗагрузитьТаблицыДвижений(ТаблицыДляДвижений, Движения, ЗагружатьПустойНабор = Ложь)
