...
Функция ПредставлениеДокументаПриПроведении(Документ) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперации", МетаданныеДокумента) Тогда
		ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Проведение документа: %1 (%2)';uk='Проведення документа: %1 (%2)'"), СокрЛП(Документ), Документ.ВидОперации);
	Иначе
		ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Проведение документа: %1';uk='Проведення документа: %1'"), СокрЛП(Документ));
	КонецЕсли;
...
Функция ПолучитьСоответствиеСубконтоПараметрамУчета(ТолькоМПЗ = Ложь) Экспорт
	...
	// В BASERP добавлены **.ДР для нераспределенных доп.расходов
...
Функция СобытиеЖурналаРегистрацииПараметрыУчета() Экспорт
	...
	Возврат НСтр("ru='Настройка параметров учета';uk='Настройка параметрів обліку'",ОбщегоНазначения.КодОсновногоЯзыка())
...
|[ОписаниеОшибки]'
|;uk='Помилка при зміні параметрів обліку:
|[ОписаниеОшибки]'");
...
|Подробности см. в Журнале регистрации.'
|;uk='Помилка при зміні параметрів обліку. 
|Подробиці див.в Журналі реєстрації.'");
		ОбщегоНазначения.СообщитьПользователю(Текст);
