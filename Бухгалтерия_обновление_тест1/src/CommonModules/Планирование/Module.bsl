...
Процедура ЗаполнитьПланыПоДефициту(Сценарий, ВидПлана, НачалоПериода, ОкончаниеПериода, ПланОбъект = Неопределено) Экспорт
	...
	Попытка
		...
		Если ПланОбъект = Неопределено Тогда
			...
			//++ НЕ УТ
			ИначеЕсли ВидПлана.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
		...
		Если ПараметрыВидаПлана.ТипПлана = Перечисления.ТипыПланов.ПланПроизводства Тогда
			...
			ЭтоРазборка = ПараметрыВидаПлана.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка;
			...
			Для каждого Строка Из МассивСтрок Цикл
				...
				ДанныеОбИзделии.НачалоПроизводства = ?(ПараметрыВидаПлана.ЗаполнятьСпецификациюПоПериодам,
													Строка[?(ЭтоРазборка, "ДатаЗапуска", "ДатаВыпуска")],
													ПланОбъект.НачалоПериода);
