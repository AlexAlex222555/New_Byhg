...
Процедура ПолучитьДанныеДляРаспределенияТрудозатратНаВыпуск(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		...
		ОстаткиМатериалов                             = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ОстаткиМатериалов");
		ПартииПроизводстваСводно                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ПартииПроизводстваСводно");
		...
		ВтПроизводственныеЗатратыСебестоимостьТоваров = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВтПроизводственныеЗатратыСебестоимостьТоваров");
		втДолиИтог                                    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "втДолиИтог");
		ДолиВыпуска                                   = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ДолиВыпуска");
		втРасчетноеКоличество                         = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "втРасчетноеКоличество");
		Расчетное                                     = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "Расчетное");
		...
		втПродукция                                   = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "втПродукция");
		Продукция                                     = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "Продукция");
		ЗамкнутыеПартии                               = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ЗамкнутыеПартии");
		НачальныеОстаткиТрудозатратНЗП                = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "НачальныеОстаткиТрудозатратНЗП");
		РаботыДляДавальца                             = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "РаботыДляДавальца");
		ТрудозатратыПоЭтапам                          = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ТрудозатратыПоЭтапам");
		ОстаткиВидовРаботПоЭтапам                     = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ОстаткиВидовРаботПоЭтапам");
		ПартииПроизводстваПоЭтапам                    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ПартииПроизводстваПоЭтапам");
		втСписаноВПрошлыхМесяцах                      = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "втСписаноВПрошлыхМесяцах");
		РасчетноеСводно                               = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "РасчетноеСводно");
		ПланФактТрудозатратПоЭтапам                   = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ПланФактТрудозатратПоЭтапам");
...
Функция ТекстТекущиеВыпускиПродукции()
	Возврат 
        ...
        |	ДД.НалоговоеНазначение КАК НалоговоеНазначениеДокумента,
		|	0 КАК НДСУпр,
...
Функция ТекстТекущиеРасходыТрудозатратыНЗП()
	Возврат 
        ...
        |	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначениеДокумента,
		|	0 КАК НДСУпр,
        |	0 КАК НДСРеглРеквизит,
		|	ДД.СтатьяКалькуляции,
		|	НЕОПРЕДЕЛЕНО КАК ДокументИсточник,
		...
		|	И (ДД.Регистратор ССЫЛКА Документ.ПроизводствоБезЗаказа
...
Процедура ЗаполнитьРасчетнуюПартиюСтоимостьТрудозатрат(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
    ...
    ЗаполнитьЗначенияСвойств(РасчетнаяПартия, Приход, "ВидДвижения, НалоговоеНазначение, АналитикаУчетаПоПартнерам, Партия, АналитикаУчетаПартий, 
    	|НалоговоеНазначениеПартии, АналитикаФинансовогоУчета, ДокументИсточник");
    ...
    РасчетнаяПартия.НалоговоеНазначение = Расход.КорНалоговоеНазначение;
...
Процедура ЗаполнитьРасчетнуюПартию(ПараметрыРасчета, Контекст, РасчетнаяПартия, Расход, Приход, ПартияЗаполнена) Экспорт
	...
	ИначеЕсли Контекст = "РаспределениеТрудозатратНаВыпуск" Тогда
		...
		ПартияЗаполнена = Истина;
	ИначеЕсли Контекст = "РаспределениеТрудозатратНаВыпускУстаревшее" Тогда
		// Этап 3
...
Процедура ДополнитьЭтапыСРаспределениемПартий(СписокЭтапов) Экспорт
	...
	СписокЭтапов.Добавить("РаспределениеТрудозатратНаВыпускУстаревшее");
