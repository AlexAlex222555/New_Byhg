...
//  ФИОПодписейНаДатуДокумнета -- получать ФИО подписей на датку документа, указанной в Форма
Процедура ВедомостьОрганизацияПриИзмененииНаСервере(Форма, ФИОПодписейНаДатуДокумнета = Ложь) Экспорт
		...
		ЗарплатаКадры.ЗаполнитьЗначенияВФорме(Форма, ЗначенияДляЗаполнения, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("Организация"), ФИОПодписейНаДатуДокумнета);
...
Процедура ВедомостьУстановитьДоступностьЭлементов(Форма) Экспорт
	...
	Форма.ТолькоПросмотр = 
		...
		ИЛИ Форма.Объект.ВыплатаОтраженаВБухучете;
