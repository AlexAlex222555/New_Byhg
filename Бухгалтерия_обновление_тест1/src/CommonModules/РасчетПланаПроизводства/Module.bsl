...
Функция СледующиеДействия(Действия)
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru='В процессе расчета плана производства произошло зацикливание процедуры обработки данных. Выполнение прервано.';uk='У процесі розрахунку плану виробництва відбулося зациклення процедури обробки даних. Виконання перервано.'");
		ВызватьИсключение ТекстОшибки;
	...
	ПорядокРасчета = Действия.Владелец["ПорядокРасчета"]; // см. АналогиМатериалов.СлужебнаяСтруктураТаблицыЗначений
...
Процедура ВыполнитьДействие(ИмяПроцедуры, ПараметрыПроцедуры, ПараметрыРасчета, ЗапрещеноВыполнятьВОтдельномПотоке = Ложь)
	...
	НаименованиеФоновогоЗадания = СтрШаблон(НСтр("ru='Расчет плана производства <%1> (%2)';uk='Розрахунок плану виробництва < %1> (%2)'"), ПараметрыРасчета.ПланПроизводства, (МестоВОчереди + 1));
...
//
// Параметры:
//  ПорцияРасчета - ТаблицаЗначений - содержит:
//                   * РасчетнаяБазаКоличество -
//                   * РасчетнаяБазаКЗаказу -
//  ПараметрыРасчета - 
//
...
Функция РазузловатьИзделия(ПорцияРасчета, ПараметрыРасчета)
	...
	СтруктураДанных = РазузлованиеИзделий.Разузловать(
		...
		ПараметрыРасчета.ПараметрыВыборкиДанных); // см. АналогиМатериалов.СлужебнаяСтруктураСтруктуры
	...
	Если ПорцияРасчета.Найти(Справочники.РесурсныеСпецификации.ПустаяСсылка(), "Спецификация") <> Неопределено Тогда // поддержка режима "без спецификации"
		...
		Для каждого Данные Из СтруктураДанных.ДанныеСпецификаций Цикл
			...
			Для каждого Строка Из НайденныеСтроки Цикл
				...
				НоваяСтрока.ДанныеУпаковки = УправлениеДаннымиОбИзделиях.СтруктураДанныхУпаковки();
...
Функция ОжидатьЗавершениеРасчета(ПараметрыРасчета, ОжидатьЗавершениеРасчетаВсехПотоков = Истина)
	...
	РасчетЗавершен = Ложь;
	ОчередьРасчета = ПараметрыРасчета.ОчередьРасчета; // см. АналогиМатериалов.СлужебнаяСтруктураМассиваИзСтруктур
...
//  ЗначенияЗаполнения - 
//  Данные - 
//  Таблица - ТаблицаЗначений - содержит:
//             * НомерСтроки - Число - 
//  ТипВыпуска -
//  Периоды - 
//
Процедура ДобавитьДвиженияВПланыВыпускаИзделий(Движения, ЗначенияЗаполнения, Данные, Таблица, ТипВыпуска, Периоды)
	...
	Для каждого Строка Из Таблица Цикл
		...
		Если (Строка.СпособАвтовыбораНоменклатуры <> Перечисления.СпособыАвтовыбораНоменклатуры.УказываетсяВНСИ
			...
			Шаблон = НСтр("ru='При разузловании изделия по спецификации ""%1"" в строке №%2 списка ""Выходные изделия"" не удалось выполнить автовыбор номенклатуры по причине: %3.';uk='При разузлование вироби по специфікації ""%1"" в рядку № %2 списку ""Вихідні вироби"" не вдалося виконати автовибір номенклатури з причини: %3.'");
	...
	#КонецОбласти
		...
		Если Строка.КоличествоИзделие > 0
			И НЕ Данные.ЕстьНекратныйВыпуск Тогда
...
Процедура РассчитатьРесурс(Строка, СтрокаСпецификации, Данные, ИмяПоляКоличество, ИмяРесурса)
	...
	Если ИмяРесурса = "КЗаказу" Тогда
		КоэффициентЧислитель = Данные.РасчетнаяБазаКЗаказу;
		КоэффициентЗнаменатель = Данные.КоличествоКРасчету;
	Иначе
		КоэффициентЧислитель = Данные.РасчетнаяБазаКоличество;
		КоэффициентЗнаменатель = Данные.КоличествоКРасчету;
	КонецЕсли;
...
Процедура ДобавитьДвиженияВПланыПотребленияМатериалов(Движения, ЗначенияЗаполнения, Данные, Таблица, Периоды)
	...
	Для каждого Строка Из Таблица Цикл
		...
		Если (Строка.СпособАвтовыбораНоменклатуры <> Перечисления.СпособыАвтовыбораНоменклатуры.УказываетсяВНСИ
			...
			ИЛИ Строка.СпособАвтовыбораХарактеристики <> Перечисления.СпособыАвтовыбораХарактеристики.УказываетсяВНСИ
				И Не ЗначениеЗаполнено(Строка.Характеристика)) Тогда
			Шаблон = НСтр("ru='При разузловании изделия по спецификации ""%1"" в строке №%2 списка ""Материалы и работы"" не удалось выполнить автовыбор номенклатуры по причине: %3.';uk='При разузлование вироби по специфікації ""%1"" в рядку № %2 списку ""Матеріали і роботи"" не вдалося виконати автовибір номенклатури через: %3.'");
			ТекстОшибки = СтрШаблон(Шаблон, СпецификацияПредставление(Данные.Спецификация), Строка.НомерСтроки, Строка.СведенияАвтовыбора);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
// Добавляет движения в планы занятости трудовых ресурсов
//
// Параметры:
//  Движения - Структура - содержит:
...
//  Неопределено
//
Функция ДобавитьДвиженияВПланыЗанятостиТрудовыхРесурсов(Движения, ЗначенияЗаполнения, Данные, Таблица, Периоды)
...
Функция ТекстЗапросаОперацииРасчетаПланаПроизводства()
	...
	|				ТОГДА Обороты.Период
	|			КОГДА Обороты.Период < &НачалоПериода
	|				ТОГДА Обороты.Период
	|			ИНАЧЕ &НачалоПериода
	|		КОНЕЦ, ДЕНЬ) КАК НачалоПроизводства,
	|	
	...
	|	И (НЕ ИзмененныеИзделия.ЭтоПолуфабрикат ИЛИ ИзмененныеИзделия.Спецификация <> ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка))
	|	
	|УПОРЯДОЧИТЬ ПО
	|	Спецификация
	|;
...
Функция ОбщиеНастройки(ПроверитьРежимЭксплуатации = Ложь) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось получить параметры расчета плана производства, значения будут приведены к настройкам по умолчанию';uk='Не вдалося отримати параметри розрахунку плану виробництва, значення будуть приведені до настройок за замовчуванням'");
	...
	Если ПараметрыРасчета.ИскатьЦиклыПосле < ПараметрыРасчета["ПротоколРасчета"].КоличествоУровней Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Отказ Тогда
				...
				ТекстШаблона = НСтр("ru='В процессе расчета плана производства произошло зацикливание спецификаций: %1. Выполнение прервано.';uk='У процесі розрахунку плану виробництва відбулося зациклення специфікацій: %1. Виконання перервано.'");
...
Функция СпецификацияПредставление(Ссылка)
	...
	Если Выборка.Следующий() Тогда
		Результат = СтрШаблон(НСтр("ru='""%1 %2""';uk='"" %1 %2""'"), Ссылка.Код, Ссылка.Наименование);
	Иначе
		Результат = "";
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ПараметрыРасчета)
	...
	|
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПланПроизводства.ВидПлана.ЗаполнятьСпецификациюПоПериодам, ЛОЖЬ)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЗаполнятьСпецификациюПоПериодам,
	|
...
Процедура ДобавитьТаблицыПодбораСпецификаций(Запрос, ТекстыЗапросов)
	...
	ПараметрыВыбораСпецификаций.ПараметрыСмещенияПериода =
		Документы.ПланПроизводства.ПараметрыСмещенияПериодаВыбораСпецификаций(Запрос.Параметры.Сценарий);
	
	Запрос.Текст = УправлениеДаннымиОбИзделиях.ТекстЗапросаСпецификацийНоменклатуры(
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Расчет плана производства';uk='Розрахунок плану виробництва'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция РазложитьПакетЗапросовВСтруктуруТекстовЗапросов(ТекстЗапроса)
	...
	Для каждого Пакет Из Пакеты Цикл
		...
		Если Позиция > 0 Тогда
			Для Курсор = Позиция + 10 По СтрДлина(Пакет) - 1 Цикл
				...
				Если (КодСимвола >= 48 И КодСимвола <= 57) // цифры
					...
					Продолжить;
				КонецЕсли;
				ИмяТаблицы = Сред(Пакет, Позиция + 10, Курсор - Позиция - 10);
				Прервать;
			КонецЦикла;
		КонецЕсли;
