...
Процедура СообщитьОбОшибкахКорректностьЗаполненияМестХранения(Знач Выборка, Знач ДокументЗакупки, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ШаблонТекстаОшибки = НСтр("ru='Не заполнена колонка ""Место хранения"" в строке %1 списка ""%2""';uk='Не заповнена колонка ""Місце зберігання"" в рядку %1 списку ""%2""'");
...
Процедура СообщитьОбОшибкахСоответствияВалютыПоАгентскойЗакупке(Выборка, ДокументЗакупки, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='Для номенклатуры %Номеклатура%, закупаемой по агентской схеме, валюта, указанная в договоре назначения, должна совпадать с валютой документа';uk='Для номенклатури %Номеклатура%, що закуповується за агентською схемою, валюта, зазначена в договорі призначення, повинна збігатися з валютою документа'");
...
Процедура СообщитьОбОшибкахКонтрольЗаказаКлиента(Выборка, ДокументЗакупки, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='Не заполнен Заказ клиента';uk='Не заповнене Замовлення клієнта'");
...
Процедура СообщитьОбОшибкахКонтрольНазначения(Выборка, ДокументЗакупки, Отказ);
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru='Не заполнено Назначение';uk='Не заповнене Призначення'");
