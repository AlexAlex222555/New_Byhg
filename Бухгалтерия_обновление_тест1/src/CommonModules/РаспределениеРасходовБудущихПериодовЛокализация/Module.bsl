...
#Область ТекстРаспределениеРБПНаРасходыСтарыеДвижения
	...
	|ВЫБРАТЬ // Распределение РБП на расходы (Дт 23, 9X :: Кт 9X)
	|
	...
	ТекстФрагментаПроводок = СтрШаблон(ТекстФрагментаПроводок, 
		НСтр("ru='Распределение РБП на расходы';uk='Розподіл витрат майбутніх періодів на витрати'",ЯзыкСодержания));
...
#Область ТекстРаспределениеРБПНаДопРасходыПоТоварамСтарыеДвижения
	...
	ТекстФрагментаПроводок = "
	|ВЫБРАТЬ // Распределение РБП на доп. расходы по товарам (Дт 2ХДР :: Кт 9Х)
	|
	|	Строки.Ссылка КАК Ссылка,
	|	Строки.Период КАК Период,
	|	Строки.Организация КАК Организация,
	...
	|		И КорСтроки.КорСтатьяРасходов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|ГДЕ
	|	(Строки.СтоимостьРегл <> 0 ИЛИ Строки.Стоимость <> 0)
	|";

	ТекстФрагментаПроводок = СтрШаблон(ТекстФрагментаПроводок, 
		НСтр("ru='Распределение РБП на доп. расходы по товарам';uk='Розподіл ВМП на дод. витрати на товари'",ЯзыкСодержания));
...
#Область ТекстРаспределениеРБПНаПрочиеАктивы
	...
	|
	|ГДЕ
	|	(Строки.СуммаРегл <> 0 ИЛИ Строки.СуммаУпр <> 0)
	|";

	ТекстФрагментаПроводок = СтрШаблон(ТекстФрагментаПроводок, 
		НСтр("ru='Распределение РБП на прочие активы';uk='Розподіл витрат майбутніх періодів на інші активи'",ЯзыкСодержания));
