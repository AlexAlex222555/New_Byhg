...
#Область ТекстОприходованиеТоваровПоУказаннойСтоимости
	...
	|	Строки.Номенклатура КАК СубконтоКт1,
	|	ВЫБОР КОГДА Строки.Склад ССЫЛКА Справочник.Партнеры ИЛИ Строки.Склад ССЫЛКА Справочник.Организации ТОГДА
	|		Строки.Контрагент ИНАЧЕ Строки.Склад КОНЕЦ КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	Строки.Количество КАК КоличествоКт,
...
#Область ТекстОприходованиеТоваровПоУказаннойСтоимостиСкладПроизводства
	ТекстЗапроса = "
	|ВЫБРАТЬ ///// Оприходование товаров по указанной стоимости на склад производства (Дт 281 :: Кт 281, 68ОК)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимости
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Отклонение в стоимости собственных товаров';uk='Відхилення у вартості власних товарів'",ЯзыкСодержания));
	ТекстыОтражения.Добавить(ТекстЗапроса);
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимостиДоходы
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Отклонение в стоимости товаров';uk='Відхилення у вартості товарів'",ЯзыкСодержания));
	ТекстыОтражения.Добавить(ТекстЗапроса);
...
#Область ТекстОприходованиеТоваровОтклонениеВСтоимостиДоходыСкладПроизводства
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Отклонение в стоимости товаров (Дт 23 :: Кт 7X)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
