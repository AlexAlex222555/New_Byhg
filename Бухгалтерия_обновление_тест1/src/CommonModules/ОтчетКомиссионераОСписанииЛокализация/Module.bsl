...
#Область ТекстОтражениеВзаиморасчетовПоКомиссионномуТовару
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Взаиморасчеты по комиссионному товару';uk='Взаєморозрахунки за комісійного товару'",ЯзыкСодержания));
	ТекстыОтражения.Добавить(ТекстЗапроса);
...
#Область ТекстОтраженияНДССписанногоТовара
	ТекстЗапроса = "
	...
	|	СУММА(ЕСТЬNULL(СуммыДокументовВВалютахУчета.СуммаНДСРегл, Строки.СуммаНДС) / КурсВалютыУпрУчета.Курс ) КАК СуммаУУ,
	|
