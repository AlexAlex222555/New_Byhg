...
// 	*ВариантСумм - ПеречислениеСсылка.ВариантыВыводаСуммВБюджетныхОтчетах -
...
|;uk='Перевищено допустиму кількість запусків процедури розрахунку змін в комірках. 
|Виконання перервано для запобігання зациклення, дані заповнені по зв''язаних комірках можуть бути некоректними.'");
				...
				Если ЗначениеЗаполнено(Область) Тогда
					...
					Если Не ЗначениеИзменений = Неопределено Тогда
						...
						Для Каждого Периодичность Из Перечисления.Периодичность Цикл
							ПериодСтрокой = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
...
Функция ТаблицаПериодов(Периодичности, НачалоПериода, КонецПериода) Экспорт
	...
	Для Каждого Периодичность Из Периодичности Цикл
		ИмяПериода = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
...
Функция ТаблицаПериодовПоИсточникамДанных(ОписаниеОтчета, ИсточникиПериодовГруппировок, НачалоПериода, КонецПериода, ГраницаФактДанных, ТекстПериодов)
		...
		Для Каждого Источник Из ИсточникиПериодовГруппировок Цикл
			...
			Запрос.Текст = "ВЫБРАТЬ "
				+ СтрЗаменить(Источник.ВыражениеНачалоПериодаГруппировки,
				              "ЛеваяТаблицаПериодов.Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность),
				              "&НачалоПериода")
				+ " КАК НачалоПериода, "
	...
	Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
		ТекстЗапроса = ТекстЗапроса + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				" ТаблицаПериодов.Период%1 КАК Период%1Отчет,
				| ТаблицаПериодов.Период%1 КАК Период%1Соединение,",
	...
	Для Каждого Источник Из ИсточникиПериодовГруппировок Цикл
		...
		Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
			ТекстЗапроса = ТекстЗапроса + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					" ЛеваяТаблицаПериодов.Период%1 КАК Период%1Отчет,
					| ПраваяТаблицаПериодов.Период%1 КАК Период%1Соединение,",
					ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность));
		КонецЦикла;
		...
		|ПО ПраваяТаблицаПериодов.Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность)
...
Функция ВспомогательныеДанныеРасчетаДанныхЯчеек(ПараметрыДанных, ОписаниеОтчета,
	...
	Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
		...
		ТекстПериодов = ТекстПериодов + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"<ИмяТаблицы>.Период%1 КАК Период%1,",
				ИмяПериода)
			+ Символы.ПС;
		
		ТекстПериодовСоединение = ТекстПериодовСоединение + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"<ИмяТаблицы>.Период%1Соединение КАК Период%1Соединение,",
				ИмяПериода)
			+ Символы.ПС;
		
		ТекстФлагиПериодов = ТекстФлагиПериодов + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"<ИмяТаблицы>.ФлагПериод%1 КАК ФлагПериод%1,",
				ИмяПериода)
			+ Символы.ПС;
		
		ТекстВыраженияПериодов = ТекстВыраженияПериодов + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"<ИмяТаблицы>.ВыражениеПериода%1 КАК ВыражениеПериода%1,",
				ИмяПериода)
			+ Символы.ПС;
...
Процедура РазбитьЯчейкиПоПериодам(ТекстЗапроса, ОписаниеОтчета, ВспомогательныеДанные, ИмяТаблицы)
	...
	Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
		...
		ТекстПериодичностей = ТекстПериодичностей + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"	ВЫБОР КОГДА СтруктураЯчеек.ФлагПериод%1 = ""заполнить""
				|		ТОГДА ТаблицаПериодов.Период%1Отчет
				|		ИНАЧЕ НЕОПРЕДЕЛЕНО
				|	КОНЕЦ КАК Период%1,",
				ИмяПериода)
			+ Символы.ПС;
		...
		ТекстПериодичностей = ТекстПериодичностей + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"	ВЫБОР КОГДА СтруктураЯчеек.ФлагПериод%1 = ""заполнить""
				|		ТОГДА ТаблицаПериодов.Период%1Соединение
				|		ИНАЧЕ НЕОПРЕДЕЛЕНО
				|	КОНЕЦ КАК Период%1Соединение,",
				ИмяПериода)
			+ Символы.ПС;
	...
	Для Каждого Измерение Из ОписаниеОтчета.Периодичности Цикл
		...
		Если Измерение = Перечисления.Периодичность.Неделя Тогда
			ТекстСоединенияПериодичностей = ТекстСоединенияПериодичностей + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"	И (СтруктураЯчеек.ФлагПериод%1 = """"
					|			ИЛИ
					|			СтруктураЯчеек.ВыражениеПериода%1 = -1
					|			ИЛИ
					|			(СтруктураЯчеек.ВыражениеПериода%1 = 0
					|				И ДобавитьКДате(&НачалоПериода, %1, СтруктураЯчеек.ВыражениеПериода%1) = 
					|					ТаблицаПериодов.Период%1Отчет)
					|			ИЛИ
					|			(СтруктураЯчеек.ВыражениеПериода%1 > 0
					|				И ДобавитьКДате(НачалоПериода(&НачалоПериода, %1), %1, СтруктураЯчеек.ВыражениеПериода%1) = 
					|					ТаблицаПериодов.Период%1Отчет)
					|		)",
					ИмяИзмерения);
		Иначе
	...
	Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
		ТекстЗапроса = ТекстЗапроса + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"	ТаблицаПериодов.Период%1Отчет,
				|	ТаблицаПериодов.Период%1Соединение,",
				ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность));
	КонецЦикла;
...
Процедура ВыполнитьСоединениеСтруктурыСДанными(ТекстЗапроса, ОписаниеОтчета, ВспомогательныеДанные, ИмяТаблицы, ВспомогательныеПараметры, РежимФормирования)
	...
	Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
		...
		ТекстЗапроса = ТекстЗапроса + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"	И ( (СтруктураЯчеек.ФлагПериод%1 = ""отобрать""
				|			И СтруктураЯчеек.Период%1Соединение = ТаблицаДанных.Период%1)
				|		ИЛИ
				|		(СтруктураЯчеек.ФлагПериод%1 <> ""отобрать""
				|			И ТаблицаДанных.Период%1 = НЕОПРЕДЕЛЕНО) )",
				ИмяПериода)
			+ Символы.ПС;
...
// 
// Параметры:
// 	ДеревоВывода - ДеревоЗначений -
// 		см. БюджетнаяОтчетностьРасчетКэшаСервер.ПолучитьПараметрыВыводаВидаБюджета.ДеревоВывода
// 	ДанныеЯчеек - см. ДанныеЯчеек
// 	ОписаниеОтчета - см. БюджетнаяОтчетностьРасчетКэшаСервер.КолонкиТаблицыДанных
// Возвращаемое значение:
...
Функция ПараметрыГруппировокКолонок(ПараметрыВывода, ИндексыПоискаИзмерений, ДеревоВывода,
									...
									НакопленныйОтбор = Неопределено) Экспорт
...
Функция РассчитатьСвязанныеСтатьиБюджетов(ПараметрыВыводаВидаБюджета, АдресДанныхЯчеек, ПараметрыДанных,
	...
	Для Каждого СтрокаТаблицы Из СтрокиКРасчету Цикл
		...
		Для Каждого Периодичность Из СтруктураКолонок.Периодичности Цикл
			ИмяПериода = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
			Если ЗначениеЗаполнено(СтрокаТаблицы["Период" + ИмяПериода]) Тогда
				Расшифровка.Вставить("Период" + ИмяПериода, СтрокаТаблицы["Период" + ИмяПериода]);
			КонецЕсли;
		КонецЦикла;
...
Функция ПреобразоватьПараметрыДанныхВДанныеПериодов(СтруктураКолонокТаблицыДанных, ПараметрыДанных) Экспорт
	...
	Для Каждого Периодичность Из УпорядоченныеПериодичности Цикл
		...
		Если СтруктураКолонокТаблицыДанных.Периодичности.Найти(Периодичность) <> Неопределено Тогда
			Результат.УпорядоченныеПериодичностиОтчета.Добавить("Период" + ИмяПериода);
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьПараметрыВыводаПримечаний(СтруктураКолонокТаблицыДанных, ДанныеЯчеек, ДеревоВывода)
	...
	Для Каждого Периодичность Из СтруктураКолонокТаблицыДанных.Периодичности Цикл
		СписокКолонок = СписокКолонок + ",Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
	КонецЦикла;
...
Функция НовыйСтруктураПримечания(ДанныеЯчеек, СписокКолонок)
	...
	ТаблицаДанных = ДанныеЯчеек.СкопироватьКолонки(СписокКолонок); // см. ДанныеЯчеек
...
Процедура ПроверитьФормулу(Знач Формула, Операнды, Отказ) Экспорт
	...
	Для Каждого Операнд Из Операнды Цикл
		...
		Если Не ЗначениеЗаполнено(Операнд.Идентификатор) Тогда
			ТекстСообщения = НСтр("ru='Ошибка формирования отчета: Неверный операнд формулы';uk='Помилка формування звіту: Невірний операнд формули'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	...
	Исключение
		ТекстСообщения = НСтр("ru='В формуле обнаружены ошибки';uk='У формулі виявлені помилки'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецПопытки;
...
Процедура ПроверкаВозможностиЗаполненияИзмеренийВСтруктуреЯчеек(ОписаниеОтчета, ИсточникиЗначенийИзмерений)
	...
	Если ПроизведениеАналитик > 1000 Тогда
		ВызватьИсключение НСтр("ru='Превышено максимальное количество строк для добавления аналитик. Обработка прервана.';uk='Перевищено максимальну кількість рядків для додавання аналітик. Обробка перервана.'");
	КонецЕсли;
	...
	Для Каждого ВидАналитики Из ОписаниеОтчета.ВидыАналитик Цикл
		...
		Если Не ИсточникВсеЭлементы Тогда
			...
			Для Каждого Периодичность Из ОписаниеОтчета.Периодичности Цикл
				ИмяПериода = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
				ТекстЗапроса = ТекстЗапроса + "
				|	И (	(СтруктураЯчеек.ФлагПериод" + ИмяПериода + " = ""отобрать""
				|			И СтруктураЯчеек.Период" + ИмяПериода + "Соединение = ТаблицаДанных.Период" + ИмяПериода + ")
				|		ИЛИ
				|		(СтруктураЯчеек.ФлагПериод" + ИмяПериода + " <> ""отобрать""
				|			И ТаблицаДанных.Период" + ИмяПериода + " = НЕОПРЕДЕЛЕНО)	)
				|";
			КонецЦикла;
			...
			ТекстЗапроса = ТекстЗапроса + ТекстСоединенияСотборомПоИсточникам;
...
// 	* СтруктураКолонок  - см. БюджетнаяотчетностьВыводСервер.КолонкиТаблицыДанных
// 	* ДанныеЯчеек       - см. ДанныеЯчеек
// 	* ДеревоВывода      - см. БюджетнаяОтчетностьРасчетКэшаСервер.НовыйДеревоВывода
