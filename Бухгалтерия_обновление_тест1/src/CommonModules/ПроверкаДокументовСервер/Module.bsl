...
Процедура ПередИзменениемДокумента(Источник, Отказ, Удаление = Ложь) Экспорт
	...
	Если ДанныеПроверки.Проверен И Не ИзменениеПроверенных Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Операция не выполнена.';uk='Операція не виконана.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Источник.Метаданные(), Источник.Ссылка, СообщениеОбОшибке);
...
Функция ИсключаемыеТипыДляПроверки() Экспорт
	...
	МассивВозврата.Добавить(Тип("ДокументСсылка.КорректировкаНалоговогоНазначенияКапитальныхИнвестиций"));
	...
	Иначе
		...
		Иначе
			Если ДанныеОПроверке.Проверен Тогда
				Если ПолучитьФункциональнуюОпцию("РазрешатьИзменятьПроверенныеДокументыПоРеглУчету") Тогда
					Комментарий = НСтр("ru='изменения не рекомендуются.';uk='зміни не рекомендуються.'");
				Иначе
					Комментарий = НСтр("ru='изменение невозможно.';uk='зміна неможлива.'");
				КонецЕсли;
				ПодсказкаКнопки = НСтр("ru='Документ проверен';uk='Документ перевірений'") + ", " + Комментарий;
