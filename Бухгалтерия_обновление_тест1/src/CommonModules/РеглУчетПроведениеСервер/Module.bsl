...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	Если ТаблицыДляДвижений.Свойство("ТаблицаОтражениеДокументовВРеглУчете")
		И ЗначениеЗаполнено(ТаблицыДляДвижений.ТаблицаОтражениеДокументовВРеглУчете) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицыДляДвижений.ТаблицаОтражениеДокументовВРеглУчете, ТаблицаДанные);
...
Процедура ОтразитьВсе(ПериодРасчета, Организация = Неопределено, ДанныеПоОтражениюВУчете = Неопределено) Экспорт
	...
	Если ДанныеПоОтражениюВУчете.ОшибкиПриОтражении > 0 Тогда
		ВызватьИсключение НСтр("ru='При отражении документов в регл. учете возникли ошибки. Подробнее см. в журнале регистрации.';uk='При відображенні документів в регл. обліку виникли помилки. Детальніше див. у журналі реєстрації.'");
	КонецЕсли;
...
Процедура ЗарегистрироватьОтражениеДокумента(Документ, СтатусОтражения, Комментарий = Неопределено, ТаблицаОтражения = Неопределено) Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = СтрШаблон(
					НСтр("ru='Не удалось установить статус отражения в учете ""%1"" для документа ""%2"" по причине: %3';uk='Не вдалося встановити статус відображення в обліку ""%1"" для документа ""%2"" по причині: %3'"),
					СтатусОтражения,
					Документ,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Установка нового статуса отражения в учете';uk='Встановлення нового статусу відображення в обліку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработкаУстановкиДатыНачалаУУНаПланеСчетовХозрасчетный(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось вернуть документы к отражению по причине: %1';uk='Не вдалося повернути документи до відображення через: %1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
		ЗаписьЖурналаРегистрации(
				НСтр("ru='Установка даты начала УУ на плане счетов Хозрасчетный.';uk='Встановленя дати початку УО на плані рахунків Госпрозрахунковий.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ОбновитьДанныеЗависимыхРегистров(МассивДокументов = Неопределено)
	...
	Если МассивДокументов.Количество() Тогда
		...
		РегистрыСведений.СуммыДокументовВВалютахУчета.ОбновитьДвиженияПоОборотнымРегистрамНепересчитываемыхДокументов(МассивДокументов);
	КонецЕсли;
...
Функция ВыполнитьОтражение(ТипДокумента, ВременныеТаблицы, ПакетноеОтражение = Ложь)
	...
	Пока ВыборкаСтатусов.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Отражение в регламентированном учете';uk='Відображення в регламентованому обліку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
...
Функция ВыборкиОтраженияДокументов(ИмяДокумента, ВременныеТаблицы)
	...
	Если ОбщегоНазначения.РежимОтладки() Тогда
		ВтСтроки   	= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ВременныеТаблицы, "ВтСтроки"); 
		ВтСтоимости = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ВременныеТаблицы, "ВтСтоимости");
		Партии 		= ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ВременныеТаблицы, "Партии"); 
	КонецЕсли;
	...
	Попытка
		...
		Для каждого СтрокаОшибки Из ТаблицаОшибокОтражения Цикл
			...
			Если ТипЗнч(СтрокаОшибки.ВидСчета) <> Тип("ПеречислениеСсылка.ВидыСчетовРеглУчета") Тогда
				ТекстСообщения = НСтр("ru='При записи счетов требующих настроек, ошибка: некорректно задан вид счета ""%1"".';uk='При запису рахунків що вимагають настройок, помилка: неправильно заданий вид рахунку ""%1"".'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаОшибки.ВидСчета);
				ЗаписьЖурналаРегистрации(НСтр("ru='Отражение в регламентированном учете';uk='Відображення в регламентованому обліку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Функция КомментарийОшибки(СтрокаОшибки)
	...
	СоответствиеКодовОшибокПредставлениям.Вставить("СЧЕТ_ЗАПРЕЩЕН", НСтр("ru='указан запрещенный счет ""%Счет%""';uk='вказано заборонений рахунок ""%Счет%""'"));
	СоответствиеКодовОшибокПредставлениям.Вставить("СЧЕТ_ЗАБАЛАНСОВЫЙ", НСтр("ru='указан забалансовый счет ""%Счет%""';uk='вказано позабалансовий рахунок ""%Счет%""'"));
	СоответствиеКодовОшибокПредставлениям.Вставить("СЧЕТ_ПУСТОЙ", НСтр("ru='не указан счет';uk='не вказано рахунок'"));
	...
	КомментарийОшибки = НСтр("ru='Для учета %ВидСчета% %Измерение%%Место%%Аналитика% %КодОшибки%.';uk='Для обліку %ВидСчета% %Измерение%%Место%%Аналитика% %КодОшибки%.'");
	...
	Если ЗначениеЗаполнено(СтрокаОшибки.ДопИнформацияПоРасчетамСПартнерами) Тогда
		РазделУчета = СтрокаОшибки.ДопИнформацияПоРасчетамСПартнерами;
	Иначе
	...
	Если СтрокаОшибки.ВидСчета = Перечисления.ВидыСчетовРеглУчета.ПрочиеАктивыПассивы Тогда
		Если ЗначениеЗаполнено(СтрокаОшибки.ИдентификаторСтроки) Тогда
			СтрокаИзмерениеНастройки = НСтр("ru='в строке ""%ИдентификаторСтроки%"" документа';uk='в рядку ""%ИдентификаторСтроки%"" документа'");
		...
		Иначе
			СтрокаИзмерениеНастройки = НСтр("ru='в документе';uk='в документі'");
		КонецЕсли;
	Иначе
		СтрокаИзмерениеНастройки = НСтр("ru='в организации ""%Организация%""';uk='в організації ""%Организация%""'");
	...
	Если РазделУчета = Неопределено Или НастройкаСчетовУчета.НастройкиАналитикиУчетаПоРазделуУчета(РазделУчета).Используется Тогда
		КомментарийОшибки = СтрЗаменить(КомментарийОшибки, "%Аналитика%", 
			РеглУчетВыборкиСерверПовтИсп.СтрокаАналитикаУчета(СтрокаОшибки.АналитикаУчета));
	Иначе
		КомментарийОшибки = СтрЗаменить(КомментарийОшибки, "%Аналитика%", "");
	КонецЕсли;
	Если РазделУчета = Неопределено Или НастройкаСчетовУчета.НастройкиМестаУчетаПоРазделуУчета(РазделУчета).Используется Тогда
