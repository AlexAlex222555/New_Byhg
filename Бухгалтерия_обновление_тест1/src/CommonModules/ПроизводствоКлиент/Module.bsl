...
Процедура ОткрытьФормуВыбораПолучателя(Форма, Объект, ПараметрыТЧ, ТипыНоменклатуры = Неопределено, ОписаниеОповещения) Экспорт
	...
	Если Объект[ИмяТЧОбъект].Количество() = 0 Тогда
		...
		ТекстОшибки = НСтр("ru='В список ""%1"" не введено ни одной строки.';uk='У список ""%1"" не введено жодного рядка.'");
	...
	Иначе
		...
		Если НаличиеЗапрещенногоТипа Тогда
			...
			ТекстОшибки = НСтр("ru='Для выбранной номенклатуры (тип ""%1"") операция не может быть выполнена.';uk='Для вибраної номенклатури (тип ""%1"") операція не може бути виконана.'");
		...
		Если ВыборПодразделение И ВыборСклада Тогда
			...
			ТекстОшибки = НСтр("ru='Выбраны строки с получателями разных типов. Операция не может быть выполнена.';uk='Вибрані рядки з одержувачами різних типів. Операція не може бути виконана.'");
...
Функция ПроверитьЗаполнениеПередВводомДолиСтоимости(ДанныеСтроки, ПараметрыРаспределенияЗатрат, ИмяОбъекта = "Объект") Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеСтроки.Номенклатура) Тогда
		Текст = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Номенклатура';uk='Номенклатура'"));
	...
	Если (ДанныеСтроки.КоличествоУпаковок = 0 ИЛИ ДанныеСтроки.Количество = 0) 
		...
		Текст = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Количество';uk='Кількість'"));
	...
	Если Не ДоступнаРасшифровкаРасчетаДолиСтоимости(ДанныеСтроки, ПараметрыРаспределенияЗатрат) Тогда
		Текст = НСтр("ru='Недостаточно прав доступа для открытия формы расшифровки расчета доли стоимости.';uk='Недостатньо прав доступу для відкриття форми розшифровки розрахунку частки вартості.'");
...
Функция СоздатьВыпускПродукцииБезЗаказаНаОсновании(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru='Не сформировано ни одного документа. Отсутствует продукция, требующая передачи на склад.';uk='Не сформовано жодного документа. Відсутня продукція, що потребує передачі на склад.'"));
...
Процедура ЗаполнитьПризнакЦеховаяКладовая(ТекущаяСтрока, Параметры, КэшированныеЗначения) Экспорт
	...
	Если ЗначениеЗаполнено(ЗначениеСклада) И ТипЗнч(ЗначениеСклада) = Тип("СправочникСсылка.Склады") Тогда
		...
		Если СвойстваСклада = Неопределено Тогда
			...
			ТекстИсключения = 
				НСтр("ru='Попытка заполнения служебных реквизитов на основании выбранного склада на клиенте.';uk='Спроба заповнення службових реквізитів на підставі вибраного складу на клієнті.'");
