...
Процедура ЗаполнитьРеквизитРасчетПартийВДвижениях(ПараметрыРасчета) Экспорт
	...
	//++ НЕ УТ
	|			И НЕ Т.Регистратор ССЫЛКА Документ.КорректировкаНалоговогоНазначенияКапитальныхИнвестиций
	|			И НЕ Т.Регистратор ССЫЛКА Документ.КорректировкаНалоговогоНазначенияОС
	//-- НЕ УТ
	|		)
	|		ТОГДА ИСТИНА 
	|		ИНАЧЕ ЛОЖЬ 
...
Функция ПартионныйУчетВерсии22НельзяПонизитьДоВерсии21(ЕстьДвиженияПУ22) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.КодПричины = 2 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
                |    - " + НСтр("ru='используется управление производством версии 2.5';uk='використовується управління виробництвом версії 2.5'",ОбщегоНазначения.КодОсновногоЯзыка());
		//-- НЕ УТ
		...
		ИначеЕсли Выборка.КодПричины = 4 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
                |    - " + НСтр("ru='используется учет необоротных активов версии 2.5';uk='використовується облік необоротних активів версії 2.5'",ОбщегоНазначения.КодОсновногоЯзыка());
		//-- НЕ УТ
		Иначе
		...
		ИначеЕсли СтрокаОстатка.Регистратор = Выборка.Регистратор
			...
			Если Ложь
			 ...
			 Тогда
                ...
                СуммыРаспределения.НДСРеглРеквизит = 0;
...
Процедура СкорректироватьСтоимостьСписанияНезавершенногоПроизводстваПоФИФОСкользящая(ПараметрыРасчета)
	...
	|		И ДолиСтоимости.НалоговоеНазначение = Таб.НалоговоеНазначение
...
Процедура СформироватьДвиженияСебестоимостьТоваровСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения, Суммы, НалоговоеНазначениеВыпуска)
	...
	Запись.НалоговоеНазначение21  = НалоговоеНазначениеВыпуска;
	Запись.КорНалоговоеНазначение = ДанныеДвижения.НалоговоеНазначение;
	Запись.НДСРегл = Суммы.НДСРегл;
	Если  (ЗначениеЗаполнено(Запись.НалоговоеНазначение21) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.НалоговоеНазначение21, "ВидДеятельностиНДС") = Перечисления.ВидыДеятельностиНДС.Необлагаемая) 
...
Процедура СформироватьДвиженияПрочиеРасходыСписаниеНЗП(ПараметрыРасчета, ДанныеДвижения, Суммы)
	...
	КопируемыеПоля = "
    |Период, Организация, НалоговоеНазначение, СтатьяРасходов, АналитикаРасходов, НаправлениеДеятельности, ДокументДвижения, КорПодразделение, КорНаправлениеДеятельности";
...
Процедура ПолучитьСебестоимостьПартийТоваров(ПараметрыРасчета, ДляСтоимостиПродаж = Ложь)
	...
	|		) 
	|       ТОГДА  
	|			ВЫБОР КОГДА Затраты.Знаменатель < 0 ТОГДА
	|				0 - Затраты.НДСРегл
	|			ИНАЧЕ
	|				Затраты.НДСРегл
	|			КОНЕЦ
...
Процедура ВедомостьПоПартиямТоваров_ДополнитьТекстЗапроса(ТекстЗапроса) Экспорт
	ТекстЗапросаПУ21 = "
		...
		|			Затраты.НДСРеглНачальныйОстаток
        //++ НЕ УТ
        |		КОГДА ТИПЗНАЧЕНИЯ(Затраты.ДокументВыпуска) = ТИП(Документ.ВыпускПродукции) 
        |			  ИЛИ ТИПЗНАЧЕНИЯ(Затраты.ДокументВыпуска) = ТИП(Документ.ПоступлениеОтПереработчика) 
        |			  ИЛИ ТИПЗНАЧЕНИЯ(Затраты.ДокументВыпуска) = ТИП(Документ.ОтчетПереработчика) 
        |			ТОГДА ВЫБОР КОГДА 
        |				(Затраты.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Облагаемая) 
        |				    И Затраты.ДокументВыпуска.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая))
        |				ИЛИ (Затраты.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) 
        |				    И Затраты.ДокументВыпуска.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность))
...
Функция АнализСебестоимостиВыпущеннойПродукции_ТекстЗапросаМатериальные() Экспорт
	ТекстЗапроса = "
		...
		|				ИНАЧЕ 0 КОНЕЦ)
        |		КОГДА &ДанныеПоСебестоимости = 5 ТОГДА
        |			Партии.СтоимостьБезНДС
        |		КОГДА &ДанныеПоСебестоимости = 6 ТОГДА
        |			Партии.СтоимостьРегл
		|		ИНАЧЕ
		|			Партии.СтоимостьБезНДС
		|		КОНЕЦ) КАК Стоимость
		...
		|	      	            И Партии.НалоговоеНазначениеВыпуска.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.Необлагаемая))
		|	      	          ИЛИ (Партии.АналитикаУчетаПартий.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая)
		|	        	        И Партии.НалоговоеНазначениеВыпуска = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность))
		|			    ТОГДА Партии.НДСРегл
		|				ИНАЧЕ 0 КОНЕЦ)
        |		КОГДА &ДанныеПоСебестоимости = 5 ТОГДА
        |			Партии.СтоимостьБезНДС
