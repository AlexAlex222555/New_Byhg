...
Процедура ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	...
	Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков = "ТаблицаВидыЗапасов"
		...
		ТекстИсключения = НСтр("ru='ТаблицаВидыЗапасов нужна для заполнения видов запасов по умолчанию, поэтому ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию должен быть выстален в ИСТИНА.';uk='ТаблицаВидыЗапасов потрібна для заповнення видів запасів за замовчуванням, тому ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию повинен бути виставлений в ІСТИНА.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
...
|є надлишкові витратні руху в коригування регістрів за вимірюваннями 
|%Организация%-% АналитикаУчетаНоменклатуры%-% ВидЗапасов% -%НомерГТД%, 
...
|реквізитів видів запасів за замовчуванням, але таблиці не передана.'");
					ВызватьИсключение ТекстИсключения;
				КонецЕсли;
...
Процедура УстановитьБлокировкуРегистров(ДокументОбъект, МенеджерВременныхТаблиц)
	...
	//++ НЕ УТ
	Если ЕстьДвижениеВКоллекции(ДокументОбъект,"ТоварыПереданныеПереработчику") Тогда
...
Процедура ТаблицаОстатковДляВозвратаПереданныхПереработчикуТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков)
	...
	|	ЕСТЬNULL(ТаблицаНомеровГТД.НомерГТД, ЗНАЧЕНИЕ(Справочник.НоменклатураГТД.ПустаяСсылка)) КАК НомерГТД,
