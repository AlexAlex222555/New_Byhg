...
Процедура ДобавитьЭтап_СписаниеЗатратНаВыпускБезРаспоряжений(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.ТекстВыполнить = НСтр("ru='Списать';uk='Списати'");
...
Процедура Использование_СписаниеЗатратНаВыпускБезРаспоряжений(ПараметрыОбработчика) Экспорт
	...
	Если РазмерыВременныхТаблиц.ВТКОформлению = 0 И РазмерыВременныхТаблиц.ВТОбороты = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет незакрытых оборотов по регистру ""Распоряжения на списание по нормативам""';uk='Немає незакритих оборотів по регістру ""Розпорядження на списання за нормативами""'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура Оформление_СписаниеЗатратНаВыпускБезРаспоряжений(ПараметрыОбработчика) Экспорт
	...
	Если ИспользуетсяПроизводство22 Тогда
		ПараметрыОбработчика.ДанныеЭтапа.Наименование = НСтр("ru='Списание затрат на выпуск без распоряжений (2.1)';uk='Списання витрат на випуск без розпоряджень (2.1)'");
	ИначеЕсли ПолучитьФункциональнуюОпцию("КомплекснаяАвтоматизация") Тогда
		ПараметрыОбработчика.ДанныеЭтапа.Наименование = НСтр("ru='Списание затрат на выпуск';uk='Списання витрат на випуск'");
	КонецЕсли;
	
	ПараметрыОбработчика.ДанныеЭтапа.ТекстВыполнить = НСтр("ru='Списать';uk='Списати'");
...
Процедура ОписаниеПроверок_СписаниеЗатратНаВыпускБезРаспоряжений(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не оформленные списания материалов (работ) на выпуски продукции без распоряжений';uk='Не оформлені списання матеріалів (робіт) на випуски продукції без розпоряджень'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
|;uk='Повинні бути оформлені всі списання матеріалів (робіт) та розподілено трудовитрати на всі документи ""Випуск продукції""
|за підрозділами, для яких списання витрат виконується документом ""Списання витрат на випуск"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиСписанияЗатратНаВыпускБезРаспоряжений(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Распоряжение", 	НСтр("ru='Документ выпуска';uk='Документ випуску'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТКОформлению",
		НСтр("ru='Обнаружены не оформленные списания материалов (работ) организации ""%1"" на конец периода %2';uk='Виявлено не оформлені списання матеріалів (робіт) організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Распоряжение");
...
Процедура Использование_РаспределениеМатериаловИРаботНаСебестоимостьПродукции21(ПараметрыОбработчика) Экспорт
	...
	Если НЕ НастройкиПроизводства.ИспользуетсяПроизводство21 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Не используется управление производством (версия 2.1)';uk='Не використовується управління виробництвом (версія 2.1)'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если РазмерыВременныхТаблиц.ВТНераспределенныеМатериалыИРаботы = 0
	 И РазмерыВременныхТаблиц.ВТРаспределенныеМатериалыИРаботы = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет материалов и работ для распределения на производство';uk='Немає матеріалів та робіт для розподілу на виробництво'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_РаспределениеМатериаловИРаботНаСебестоимостьПродукции21(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Не распределенные материалы (работы) на производство';uk='Не розподілені матеріали (роботи) на виробництво'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Должно быть оформлено распределение всех материалов и работ на производство документами ""Распределение материалов и работ"".';uk='Повиннен бути оформлений розподіл усіх матеріалів та робіт на виробництво документами ""Розподіл матеріалів та робіт"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиРаспределенияМатериаловИРаботНаСебестоимостьПродукции21(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Обнаружены не распределенные на производство материалы (работы) организации ""%1"" на конец периода %2';uk='Виявлено не розподілені на виробництво матеріали (роботи) організації ""%1"" на кінець періоду %2'",ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
